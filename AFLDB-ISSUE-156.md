...
| P4 | **AFLDB-ISSUE-167** | Special records — first-kick / after-siren (family deferred pending decision D-1) | Phase E (§23, §12) | medium-high | suppress operation proven reload-safe — see §P4 handoff contract below. **Allocated and planned 2026-09-13; Stages 0–8 run as at 2026-09-14. Stages 0–7 are committed and pushed on `opus/issue-167-special-records-admin` (Stage 6 `077bf2a`, Stage 7 `c847b88`), not merged to `main`. Stage 8 applied migration 102 to `afldb_dev` (migration before code, D-5 honoured with no `db:privileges` step) and ran the §18 acceptance matrix against real Contributor / Admin / Super Admin accounts: GREEN, including durable-authority replay proven through BOTH real importers, the CAS zero-write and focus contract, bounded revalidation, and both match-delete refusals. Two defects were found by rendered acceptance and fixed — a `'use server'` module exporting const arrays, which made every mutation return HTTP 500 and which no build or unit test could catch, and a read-only notice shown to Super Admins. **P4 is NOT closed: the Stage 8 fix is uncommitted and DEV runs it as a working-tree overlay, so the operator commit is the sole remaining blocker.** PROD was not contacted** |
