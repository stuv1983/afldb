#!/usr/bin/env node
/** AFLDB-ISSUE-212: exploratory V2, correcting three confirmed V1 corpus/scorer
 * oracle defects (AFLDB-ISSUE-206 §"Corpus and scorer errors found during
 * triage") in a new versioned contract. V1 (`/home/arm/nl-exploratory-v1.csv`)
 * is frozen and untouched; this is a separate file and a separate id space
 * is not needed because generation is deliberately unchanged (same seed, same
 * template axes, same RNG consumption order) -- only the three affected
 * families' EXPECTATION fields differ, so V1 and V2 rows are directly
 * comparable id-for-id.
 *
 * The three corrections, each additive/optional so the shared scorer
 * (tools/nl/corpus.ts) still scores V1 exactly as before:
 *
 *   1. team_match_result's "versus" template (template 3) now sets
 *      exploratory_support's sibling `expected_scope_kind=matchup`, asserting
 *      the real unordered scope.matchup contract instead of directional
 *      clubFor/clubAgainst (496-row CORPUS_ORACLE_DEFECT).
 *   2. achievement_summary no longer asserts expected_aggregation (plan.agg
 *      is a vestigial default for this grain with no behavioural effect) and
 *      instead sets the new expected_achievement_kind to the builder's own
 *      achievementSummary.kind (283-row CORPUS_ORACLE_DEFECT).
 *   3. Player identity (Gary Ablett Jnr/Snr, 351-row SCORER_DEFECT) needed no
 *      generator change -- it is a scorer-only fix (corpus.ts now prefers a
 *      resolved player id over display-name text). Listed here for the
 *      record: the PLAYERS pool and every question are unchanged.
 *
 ...
 *
 * node tools/nl/generate-exploratory-corpus-v2.mjs --baseline /path/to/v5.csv \
 *   --out /path/to/exploratory-v2.csv --manifest /path/to/exploratory-v2.manifest.json
 */
import { createHash } from 'node:crypto';
import { readFileSync, writeFileSync } from 'node:fs';

const VERSION = 'issue-212-exploratory-v2';
...
const HEADER = [
  ...
  // AFLDB-ISSUE-212: new, optional (see tools/nl/corpus.ts StressExpectation
  // and tools/nl/README.md) -- absent on every V1 row, so the shared scorer
  // needs no version flag to keep scoring V1 exactly as it always did.
  'expected_scope_kind','expected_achievement_kind',
];
...
function row(family, template, question, expected = {}, extra = {}) {
  ...
  return {
    ...
    verification_source: 'ISSUE-212 corrected exploratory scorer contract (AFLDB-ISSUE-206 root causes)',
    ...
    // AFLDB-ISSUE-212 corrections. Blank for every family except the two
    // whose V1 oracle was confirmed wrong.
    expected_scope_kind: extra.scopeKind ?? '',
    expected_achievement_kind: extra.achievementKind ?? '',
  };
}
...
const families = [
  ...
  { name:'team_match_result', n:2200, make() {
    ...
    // AFLDB-ISSUE-212: template 3's "for C versus O" is the exact adjacent
    // "clubA versus clubB" text extractClubs always reads as the unordered
    // scope.matchup, regardless of the earlier "for" -- the 496-row
  ...
  { name:'achievement_summary', n:650, make() {
    ...
    // AFLDB-ISSUE-212: no expected_aggregation here -- plan.agg is a
    // vestigial default for this grain (the 283-row CORPUS_ORACLE_DEFECT
    // confirmed on AFLDB-ISSUE-206: the executor never reads it, and the
    // parser's own default for a cue-free question is {kind:'max'}, not
    // 'count'). The real answer-shape contract is achievementSummary.kind,
    // asserted via expected_achievement_kind and still carried in
    // expected_plan_json for the diagnostic triage tool.
    const e=temporal({grain:'achievement_summary',intent:{achievementSummary:{achievementKey:'first_kick_goal',kind:kind[1]}}},t);
    if(template>=2)e.club=c[1];
    return row('achievement_summary',template,q,e,{depth:e.club?3:2,scope:'achievement+club/time',achievementKind:kind[1]});
  }},
...
const manifest={
  ...
  // AFLDB-ISSUE-212: this generator's own identity, distinct from the row's
  // shared V1 id space (kept intentionally so v1/v2 rows are diffable
  // id-for-id -- see the file header). scopeKindCounts/achievementKindCounts
  // are the direct evidence that exactly the two intended families carry the
  // new columns and nothing else does.
  supersedes:'issue-206-exploratory-v1',
  correctedFamilies:{
    team_match_result:'expected_scope_kind=matchup replaces directional club/opponent for the "versus" template (AFLDB-ISSUE-206 496-row CORPUS_ORACLE_DEFECT)',
    achievement_summary:'expected_aggregation dropped; expected_achievement_kind asserts achievementSummary.kind (AFLDB-ISSUE-206 283-row CORPUS_ORACLE_DEFECT)',
    player_identity_scoring:'scorer-only fix in tools/nl/corpus.ts (id-preferring player match); no generator change (AFLDB-ISSUE-206 351-row SCORER_DEFECT)',
  },
  scopeKindCounts:counts(r=>r.expected_scope_kind||'(none)'),
  achievementKindCounts:counts(r=>r.expected_achievement_kind||'(none)'),
};
