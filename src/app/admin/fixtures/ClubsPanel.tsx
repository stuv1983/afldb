...
export function ClubsPanel({
  fixtureKey, homeClubId, awayClubId, clubs, expectedUpdatedAt,
}: {
  fixtureKey: string;
  homeClubId: number;
  awayClubId: number;
  ...
  return (
    <section className="section">
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr auto 1fr', gap: '0.6rem', maxWidth: '28rem', alignItems: 'end' }}>
        ...
        <button type="button" className="btn btn-secondary" onClick={handleSwap} disabled={change.isPending} title="Swap home and away">
          ⇄
        </button>
        <label style={{ display: 'grid', gap: '0.2rem', fontSize: '0.85rem' }}>
          Away
