...
export async function answerAchievementSummary(plan: NlQueryPlan): Promise<NlAnswerPayload> {
  ...
  const [{ total }] = await sql<{ total: number }[]>`
    SELECT count(*)::int AS total FROM player_achievements
     WHERE achievement_type = ${type} AND ${LINKED}
  `;
  ...
  switch (summary.kind) {
    case 'by_club': {
      ...
      const rows = await sql<{ label: string; value: number; slug: string }[]>`
        SELECT o.name AS label, count(*)::int AS value, o.slug
         ...
         WHERE a.achievement_type = ${type} AND a.${LINKED}
         GROUP BY o.id, o.name, o.slug
         ORDER BY count(*) DESC, o.name
      `;
      return { ...base, groupBy: 'club', rows: rows.map((r) => ({ label: r.label, value: r.value, href: `/clubs/${r.slug}` })) };
    }
    ...
    case 'clubs_without': {
      ...
      // history, not just the 18 current teams.
         ...
         WHERE NOT EXISTS (
           SELECT 1 FROM player_achievements a
            ...
            WHERE cl.organization_id = o.id
              AND a.achievement_type = ${type} AND a.${LINKED}
         )
         ORDER BY o.name
      `;
      return { ...base, groupBy: 'club', rows: rows.map((r) => ({ label: r.label, value: 0, href: `/clubs/${r.slug}` })) };
    }

    case 'by_decade': {
      const rows = await sql<{ decade: number; value: number }[]>`
        SELECT (season / 10) * 10 AS decade, count(*)::int AS value
    ...
    case 'by_season': {
      const rows = await sql<{ season: number; value: number }[]>`
        SELECT season, count(*)::int AS value
          FROM player_achievements
         WHERE achievement_type = ${type} AND ${LINKED}
         GROUP BY season
         ORDER BY count(*) DESC, season DESC
      `;
      return { ...base, groupBy: 'season', rows: rows.map((r) => ({ label: String(r.season), value: r.value, href: `/seasons/${r.season}` })) };
    }
    ...
    case 'latest': {
      ...
      const mapped: NlAchievementGroupRow[] = rows.map((r) => ({
        ...
        value: r.season,
        href: `/players/${r.playerSlug}-${r.playerId}`,
