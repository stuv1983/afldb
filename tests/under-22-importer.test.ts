...
describe('22 Under 22 awards import contract', () => {
  ...
  it('makes every destructive awards reload restore the independent team data', () => {
    ...
    // all_australian (AFLDB-ISSUE-112 phase 5), club_bf (phase 6),
    // named_medals (phase 7), under_22 and rising_star (phase 4) are all
    // independently sourced legacy-free manifest reloads: a full 'awards'
    // refresh still closes over them, but none is a GROUP_REQUIRES key, so
    // each may run alone without dragging in the legacy 'awards' group.
    ...
    expect(dependencies).not.toMatch(/^\s*"named_medals":/m);
    const shared = [
    ...
    expect(expandGroups('named_medals')).toEqual(['named_medals']);
