...
describe('POST /api/internal/revalidate-season', () => {
  ...
  it('refuses anything that came through the reverse proxy, secret or no secret', async () => {
    // Annotated rather than inferred: without it TypeScript widens the two
    // cases into a union of object literals with mutually optional keys,
    // which is not assignable to the Record<string, string> `post` takes.
    const proxied: Record<string, string>[] = [
      { 'x-forwarded-for': '203.0.113.9' },
      { 'x-forwarded-host': 'beta.afldb.com' },
    ];
  ...
  it('checks the proxy headers BEFORE the secret, so a public probe learns nothing', async () => {
    const response = await POST(post({ season: 2026 }, {
