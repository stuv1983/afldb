...
describe('deleteAccessCode removes a retired code with a durable record', () => {
  ...
  it('writes access.code_deleted naming what was destroyed, and no secret', async () => {
    ...
    expect(auditInTransaction).toHaveBeenCalledTimes(1);
    const [, action, detail, actor] = vi.mocked(auditInTransaction).mock.calls[0]!;
  ...
  it('audits inside the deleting transaction, not after it', async () => {
    ...
    // and "the database cannot hold the deletion without the log".
    // auditInTransaction (AFLDB-ISSUE-119) is the only form that joins the
    // caller's transaction; audit() binds the module-level pool and would
    // commit on its own connection whatever this transaction went on to
    // do. Swapping one for the other still satisfies the assertion above
    // and silently drops the guarantee, so both the function used AND the
    // handle it was given are checked.
    state.deleteRows = deletedRow();

    await deleteAccessCode({}, form(42));

    const tx = vi.mocked(auditInTransaction).mock.calls[0]![0];
    expect(tx, 'the audit must be given the transaction handle').toBe(state.lastTx);
    expect(audit, 'the pooled form commits independently and must not be used here')
