...
|---|---|---|---|---|
| `AFLDB-ISSUE-119` | Medium | Admin / Security / NL search / Telemetry / Database | Stage 1 maps three NL relations plus the incoming app-health FK. Disposable log rows may be cleared; durable admin reviews, immutable reader feedback, their required log/ancestor context, app-health rows and audit history must remain. Migration `081_nl_search_telemetry_clear.sql`, its `tools/maintenance/privileges.sql` reconciliation and the guarded `_test` integration tests (deeper-than-one-parent ancestry fixture with a deleted mid-chain sibling, orphaned feedback, app-health `SET NULL`, sequence non-reset, rollback, lock-cutoff, restricted `afldb_auth` credential) are written and source-reviewed 2026-09-01; migration `081` is **applied to `afldb_test`** (runbook §23) and the database security boundary is now **validated** there (runbook §24) — `npm run db:privileges:test` succeeded and `tests/integration/privileges.test.ts` passes 34/34 — while **the clear function has still never been called** and its runtime behaviour is entirely unevidenced; no Server Action or UI exists yet. Runbook: `issues/open/AFLDB-ISSUE-119.md`. Renumbered from `AFLDB-ISSUE-118` on 2026-09-01: the Gridley corpus project holds committed ISSUE-118 on `opus/gridley-corpus`, while this claim was still uncommitted (runbook §0). | **Stage 2 in progress (§20–§24). Next, per runbook §24.8: (1) establish the restricted `_test` auth DSN — set `AFLDB_TEST_AUTH_DATABASE_URL` to the same endpoint and same `afldb_test` database with role `afldb_auth` and its own password, never the owner credential; (2) then run only `npx vitest run tests/integration/nl-search-telemetry-clear.test.ts`, the first call of the function ever, confirming the restricted describe ran rather than skipped. A skipped restricted describe is not a pass, so the restricted-role halves of acceptance criteria 5 and 10 stay pending. No Server Action or UI until that evidence passes.** |
