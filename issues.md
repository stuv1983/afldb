...
<!-- UPDATE 2026-09-04 (ISSUE-131 closeout): `AFLDB-ISSUE-131` is **Resolved** — runbook §8's
     ...
     (`issues/closed/AFLDB-ISSUE-131.md` §16): merged `657a875`, deployed, timer **active**, settle
     batches 735-739 on the fixed code, **0 duplicate 2026 fixtures** measured 2026-09-04, 0 refusals,
     0 findings, source completeness `complete`, three identical no-op reruns. Removed from this count
     and the Open Issues table; runbook moved to `issues/closed/AFLDB-ISSUE-131.md`. Its number stays
     allocated and it claims **NO migration number** (`086` still next free) — §7's optional hardening
     index was never measured (§9.4) and is not adopted. Deferred, non-blocking follow-up recorded in
     §16.8, not carried as an open issue. No code, migration, test or deployment change in the
     closeout. **Next free issue ID is still `AFLDB-ISSUE-138`.** -->
...
<!-- RETIRED 2026-09-04 — `AFLDB-ISSUE-131` (an upstream match rekey duplicates the canonical match)
     ...
     `issues/closed/AFLDB-ISSUE-131.md` §16 from persisted production evidence: settle batches 735-739
     on the fixed code (735 applied 2 matches / 2 period sets / 83 player rows, source completeness
     `complete`; 736-738 identical no-op reruns; 739 the nightly timer, 0 writes), **0 duplicate 2026
     fixture groups** measured read-only on 2026-09-04, 0 rekey/apply/authority refusals, 0 findings,
     `afldb-settle-afltables.timer` **active**. `repair-match-rekeys` shipped and was correctly not
     needed (production held 0 duplicates before the deploy). Not retained: the R preflight,
     repair-tool dry-run and settle dry-run transcripts — each superseded by stronger state (§16.5).
...
- **Status:** **Resolved 2026-09-04** — merged (`657a875`), deployed, and runbook §8's production acceptance reconstructed and accepted from persisted production evidence (`issues/closed/AFLDB-ISSUE-131.md` §16). Stage 1 complete 2026-09-03; Stage 2 (implementation + validation) complete 2026-09-03; independent review answered the same day. The former "unmerged; production untouched; `afldb-settle-afltables.timer` remains STOPPED" state was first corrected at the ISSUE-133 closeout (`issues/closed/AFLDB-ISSUE-133.md` §3–§4, §11.3) and is superseded in full by §16.
...
- **Resolved:** 2026-09-04 — production acceptance reconstruction (`issues/closed/AFLDB-ISSUE-131.md` §16). Five settles have run on the fixed code (batches 735-739, all `completed`); **0 duplicate 2026 fixture groups** measured read-only on production 2026-09-04 13:27 AEST; batch 735 applied 2 `matches` + 2 `match_period_scores` + 83 `player_match_stats` with source completeness `complete` and 0 refusals/0 findings; batches 736-738 were identical no-op reruns (0 canonical writes, 0 ledger rows) and 739 was the nightly timer, also 0; `afldb-settle-afltables.timer` **active**. `repair-match-rekeys` shipped and was correctly not needed — production held 0 duplicates before the deploy (runbook §15.2) and holds 0 now. Not retained: the R preflight, repair-tool dry-run and settle dry-run transcripts (§16.5) — each superseded by stronger persisted state. **`canonicalMatchesRekeyed = 0` on every production run: the rekey path itself has never fired in production and remains proven by the `afldb_test` suite, not by a field firing (§16.6).**
...
- **Files (closeout, 2026-09-04):** `issues/open/AFLDB-ISSUE-131.md` -> `issues/closed/AFLDB-ISSUE-131.md` (moved; header and §0 posture corrected, superseded markers added to §12 and §15.5, new **§16** final acceptance), `issues.md`, `IssuesIndex.md`. **No application code, test, migration, deployment or `CHANGELOG.md` change; no settle, repair or write on any environment.**
- **Runbook:** `issues/closed/AFLDB-ISSUE-131.md` (moved from `issues/open/` at closeout) — the authoritative design and evidence ledger; root cause **§3**, reconciliation contract **§5**, remediation design **§8**, read-only evidence **§9**, risks **§10**, Stage 2 **§13**, review fixes **§14**, environment evidence **§15**, **final production acceptance §16**
...
>
> **Superseded in full 2026-09-04 (ISSUE-131 closeout):** §8's acceptance is reconstructed and
> accepted in `issues/closed/AFLDB-ISSUE-131.md` §16. Production is deployed on the fix, the timer
> is **active** and has fired on schedule (batch 739), five settles have run on the fixed code, and a
> read-only measurement on 2026-09-04 returned **0 duplicate 2026 fixture groups**. The paragraph
> below is Stage-1 history and must not be read as current.
...
### Resolution (2026-09-04) — §8 production acceptance, reconstructed

§8's supervised production sequence was carried out by the operator on 2026-09-03, but no
transcript of it was ever written into the repository. That missing transcript — not a missing
step — was the only thing keeping this issue Open. The acceptance is reconstructed in
`issues/closed/AFLDB-ISSUE-131.md` §16 from state the pipeline itself persists, which is stronger
evidence than a transcript: `import_batches.validation_result` (each settle run's own counters),
`canonical_applications` (every canonical mutation) and `matches` (the result).

**Evidence, all already in the repository except the confirming read:** `issues/closed/AFLDB-ISSUE-133.md`
§3-§5 and §11.1 (deployed revision `657a875`, `BUILD_ID w9ce2qfWBViW-3wnIRGzt`, service restart
22:16:18 AEST, timer **active**, both canonical `wildcard_final` rows and their query path, the live
page rendering them); `issues/closed/AFLDB-ISSUE-132.md` §1/§11 (production holding 2 Wildcard
Finals and **0 duplicate fixtures** after the settle); runbook §15.2 (production held **0** duplicate
fixtures *before* the deploy, so the repair plan was necessarily empty); and one read-only
production confirmation on **2026-09-04 13:27 AEST** (`PROD afldb-prod`, `afldb_owner`,
`default_transaction_read_only = on`, SELECT only).

**What that read returned:** timer `active`; **0** duplicate 2026 fixture groups; 209 canonical 2026
matches (207 `home_and_away` + 2 `wildcard_final`); settle batches **735** (09-03 22:37:47), **736**,
**737**, **738** and **739** (the nightly timer, 09-04 04:31:39) all `completed` with no `failed` or
`running` batch row anywhere; `canonical_applications` rows for batch **735 only** (`matches` insert 2,
`match_period_scores` insert 2, `player_match_stats` insert 83 = 87 ledger rows) and **none** for 736-739;
no `data_issues` opened since 2026-09-03 20:00 AEST and none mentioning a rekey. Batch 735's own
counters: `snapshotMatches` 209, `snapshotPlayerMatchRows` 9,614, `snapshotRejections` 0,
`snapshotUnkeyedRejections` 0, `absenceSweepSkipped` 0 (so `assessSourceCompleteness()` returns
**`complete`** over 9,823 enumerated records — the ISSUE-128 gate, on the production run itself),
`canonicalRowsInserted` 101 (= 2 + 16 + 83, reconciling exactly with the ledger), `canonicalRowsUpdated` 0,
**`canonicalMatchesRekeyed` 0, `canonicalRekeyRefusals` 0**, `canonicalApplyFailures` 0,
`canonicalApplyRefusals` 0, `manualAuthorityRefusals` 0, `dataIssuesOpened` 0. Batches 736 and 739
carry every write counter at zero; 737 and 738 show `records_rejected` 0 and zero ledger rows.

**Verdict:** the fix is merged, deployed, and has run on production five times without creating a
duplicate canonical match, without a single refusal or finding, with source completeness `complete`,
with three identical no-op reruns (SC3) and one unsupervised nightly timer firing that wrote nothing.
The remediation tool shipped and was correctly not needed. **§8 is substantively satisfied.**

**Not retained, and not material (§16.5):** the `afldb-r-preflight.sh` transcript (superseded — two
acquisitions completed under the systemd unit, and `acquire_core.R` fails closed on a missing
`jsonlite`/`fitzRoy`, on any fitzRoy version other than `fitzroy-contract.json`'s `pinned_version` since
the settle script passes no `--allow-version-mismatch`, and on a missing SHA-256 provider); the
`repair-match-rekeys` dry-run transcript (superseded — an empty plan is entailed by 0 duplicate
groups measured before and after); and whether the settle was first run `--dry-run --auto-apply`
(superseded — by the apply's own outcome and three identical no-op reruns).

**Honest limit (§16.6):** `canonicalMatchesRekeyed = 0` on every production run. Production's 2026
rows already stood on the identities AFL Tables currently publishes, so the rekey path has never
fired in the field; what production accepts is the **prevention**. The rekey-in-place behaviour
itself remains proven by the `afldb_test` regression suite (runbook §13.3, §14.8).

**Observation recorded, not acted on (§16.7):** batch 735 wrote 83 of the source's 92 Wildcard Final
`player_match_stats` rows (42 + 41); its `records_rejected = 9` counts `import_rejections` rows for
targets refused on **unresolved identity**, with `candidatesCreated = 9` opening the matching review
candidates. Nothing was silently dropped (`snapshotRejections` / `snapshotUnkeyedRejections` are 0).
That is identity resolution on the ISSUE-122 path, consistent with the four production canonical
player splits `AFLDB-ISSUE-137` tracks, and is worth re-measuring on the first settle after ISSUE-137
completes. Not investigated here and not a new issue on this evidence.

### Follow-up (deferred, non-blocking — not ISSUE-131 acceptance)

1. **§9.4 / §7 — the optional hardening index** (UNIQUE on `season, match_date, home_club_id,
   away_club_id`) was never measured over full history and is **unwritten and not adopted**.
   ISSUE-131 claims **NO migration number**; `086` remains next free, and any later adoption must
   re-derive its number by scanning every live branch tip.
2. **§9.6 / §5.2 — `game_id` as a stable identity** was never measured and no cross-snapshot `Game`
   comparison was made. §5.2 stays undecided, §10 risk 1 stands, and the two `source_record_id`
   conventions (§3.6) remain as they are.
3. **Dev's 17 empty historical duplicate rows** (runbook §15.1, §15.3 item 4) — dev-only, no dependent
   data, outside the repair tool's candidate set by construction, a separate supervised cleanup
   decision. **No DELETE is proposed** anywhere in this issue.
4. **`AFLDB-ISSUE-134`** — settle-triggered ISR invalidation, already tracked.
5. **The nine unresolved-identity rejections** above, to be re-measured after `AFLDB-ISSUE-137`.
---
