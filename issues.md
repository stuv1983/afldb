...
|---|---|---|---|---|
| `AFLDB-ISSUE-119` | Medium | Admin / Security / NL search / Telemetry / Database | Stage 1 maps three NL relations plus the incoming app-health FK. Disposable log rows may be cleared; durable admin reviews, immutable reader feedback, their required log/ancestor context, app-health rows and audit history must remain. Migration `081_nl_search_telemetry_clear.sql` is allocated to this issue, written and source-reviewed 2026-09-01, but is untracked, unexecuted and database-unvalidated; no grant reconciliation, TypeScript, test, Server Action, UI or database mutation exists yet. Runbook: `issues/open/AFLDB-ISSUE-119.md`. Renumbered from `AFLDB-ISSUE-118` on 2026-09-01: the Gridley corpus project holds committed ISSUE-118 on `opus/gridley-corpus`, while this claim was still uncommitted (runbook §0). | **Stage 2 in progress (§20). Next: reconcile the function `EXECUTE` grant and owner in `tools/maintenance/privileges.sql`, write the guarded `_test` integration tests (mandatory deeper-than-one-parent ancestry fixture with a deleted mid-chain sibling, restricted-role and rollback), then run `npm run db:migrate:test` — the migration's first execution — followed by the focused integration suite. No Server Action or UI until that evidence passes.** |
...
- **Status:** In progress — Stage 1 complete; §5/§6 retained-evidence boundary approved by the operator 2026-09-01; Stage 2 started the same day. Migration `081` is allocated, written and source-reviewed but **unexecuted and database-unvalidated**
...
- **Files:** `src/db/migrations/081_nl_search_telemetry_clear.sql` (new, untracked), `issues/open/AFLDB-ISSUE-119.md`, `IssuesIndex.md`, `issues.md`
