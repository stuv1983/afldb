...
|---|---|---|---|---|
| `AFLDB-ISSUE-119` | Medium | Admin / Security / NL search / Telemetry / Database | Stage 1 maps three NL relations plus the incoming app-health FK. Disposable log rows may be cleared; durable admin reviews, immutable reader feedback, their required log/ancestor context, app-health rows and audit history must remain. Migration `081_nl_search_telemetry_clear.sql`, its `tools/maintenance/privileges.sql` reconciliation and the guarded `_test` integration tests (deeper-than-one-parent ancestry fixture with a deleted mid-chain sibling, orphaned feedback, app-health `SET NULL`, sequence non-reset, rollback, lock-cutoff, restricted `afldb_auth` credential) are written and source-reviewed 2026-09-01; migration `081` is **applied to `afldb_test`** (runbook §23) and the database security boundary is now **validated** there (runbook §24) — `npm run db:privileges:test` succeeded and `tests/integration/privileges.test.ts` passes 34/34 — and on 2026-09-01 the clear function was **executed for the first time** (runbook §25): with `AFLDB_TEST_AUTH_DATABASE_URL` established, `tests/integration/nl-search-telemetry-clear.test.ts` passes **9/9, 0 skipped**, proving arbitrary-depth ancestor retention with mid-chain sibling deletion, review and orphaned-feedback preservation, app-health `SET NULL` detachment, sequence non-reset, whole-clear rollback, the `pg_blocking_pids`-proven lock cutoff, and live `afldb_auth` EXECUTE success with `DELETE`/`TRUNCATE` refused at SQLSTATE `42501`. The **query layer now exists and is validated too** (runbook §26, §27): `src/db/queries/nl-search-telemetry-clear.ts` wraps the function on a required transaction handle and converts all five `bigint` counts through a rejecting `toCount()`; `npx vitest run tests/nl-search-log.test.ts` passes **16/16** (the `clearNlSearchTelemetry count boundary` describe **11/11**) and `npm run typecheck` passes, so risk R3 is evidenced at the typed query boundary. The **transaction-aware audit helper now exists and is validated too** (runbook §28): `src/lib/auth/session.ts` extracts its single `auth_audit_log` INSERT into one private writer typed on `postgres.ISql` — the genuine common supertype of the `authSql` pool and a `begin()` handle, so no cast and no suppression were needed — leaving `audit()` unchanged and adding `auditInTransaction(tx, …)`; `npx vitest run tests/auth.test.ts` passes **31/31** (the new DB-free `auth_audit_log writer` describe **6/6**) and `npm run typecheck` passes over all **67** existing `audit()` call sites. Neither helper has a **production caller** yet, and no Server Action or UI exists. Runbook: `issues/open/AFLDB-ISSUE-119.md`. Renumbered from `AFLDB-ISSUE-118` on 2026-09-01: the Gridley corpus project holds committed ISSUE-118 on `opus/gridley-corpus`, while this claim was still uncommitted (runbook §0). | **Stage 2 in progress (§20–§28); the database layer is fully validated end to end, and the query layer and the transaction-aware audit helper are validated too. Next, per runbook §28.9: (1) the Server Action in `src/app/admin/nl-search/actions.ts` (§6, §8, §11) — `requireSuperAdmin()` FIRST, before parsing the confirmation input and before opening any transaction; then the exact phrase `CLEAR SEARCH TELEMETRY` validated server-side, a wrong or missing phrase opening no transaction and deleting and auditing nothing; then ONE `authSql.begin()` holding both `clearNlSearchTelemetry(tx)` and `auditInTransaction(tx, 'nl_search.telemetry_cleared', counts, { userId: admin.id, label: admin.email })` with the payload being exactly the five returned counts, since R5 makes the caller's transaction load-bearing for the §8 cutoff and D6 means `clearNlSearchTelemetry` accepts nothing else; then, on committed success only, `revalidatePath('/admin/nl-search', 'layout')` and `revalidatePath('/admin/app-health')`, verified against this repository's known Next 15.5 hazard where `revalidatePath` inside an action has hung a client; and a result state reporting deleted/retained counts rather than an empty dashboard; (2) `tests/admin-nl-search-actions.test.ts` (§12); (3) the UI (§10), `docs/search.md` and `CHANGELOG.md`. Acceptance criteria 3, 4, 6, 9 and the UI half of 10 remain unstarted. Open risks: R4, R5, R6, R7, R8. Do not re-run or edit migration `081`: it is checksum-locked in `afldb_meta.schema_migrations`, so any repair is a new migration or a test-DB rebuild. `npm run db:privileges:test` stays idempotent and safe to re-run.** |
...
- **Status:** In progress — Stage 1 complete; §5/§6 retained-evidence boundary approved by the operator 2026-09-01; Stage 2 started the same day. Migration `081` was **applied to `afldb_test` on 2026-09-01** (runbook §23), and the **database security boundary is now validated** there (runbook §24): the `privileges.sql` reconciliation ran successfully and `tests/integration/privileges.test.ts` passes 34/34, proving owner, `SECURITY DEFINER`, pinned `search_path`, the exact two-grantee ACL and the absence of any direct `DELETE`/`TRUNCATE`. The **clear function’s runtime behaviour is now validated too** (runbook §25): `AFLDB_TEST_AUTH_DATABASE_URL` is established against the same `afldb_test` endpoint as `afldb_auth`, and `tests/integration/nl-search-telemetry-clear.test.ts` passes **9/9 with 0 skipped** — the restricted describe **ran rather than skipped**, so `afldb_auth` is proven to invoke the function successfully while its direct `DELETE`/`TRUNCATE` are refused live with SQLSTATE `42501`. The **query layer now exists and is validated** (runbook §26, §27): `src/db/queries/nl-search-telemetry-clear.ts` wraps the function on a required transaction handle and converts all five `bigint` counts through a rejecting `toCount()`; `npx vitest run tests/nl-search-log.test.ts` passes **16/16** — the `clearNlSearchTelemetry count boundary` describe **11/11** — and `npm run typecheck` passes, so **risk R3 is evidenced at the typed query boundary**: the driver's `bigint`/string counts are explicitly converted or rejected before they can enter the application result type or the §9 audit contract. The **transaction-aware audit helper now exists and is validated too** (runbook §28): `src/lib/auth/session.ts` extracts its single `auth_audit_log` INSERT into one private writer typed on `postgres.ISql` — the genuine common supertype of the `authSql` pool and a `begin()` handle — so `audit()` keeps its exact signature, pool and behaviour while the new `auditInTransaction(tx, …)` writes the same row on the caller's transaction, with no cast, no TypeScript suppression and no duplicated SQL; `npx vitest run tests/auth.test.ts` passes **31/31** (the new DB-free `auth_audit_log writer` describe **6/6**) and `npm run typecheck` passes, compiling all **67** existing `audit()` call sites against the refactored signature. Neither helper has a **production caller** yet. **Server Action and UI not started**
- **Renumbered:** 2026-09-01, from `AFLDB-ISSUE-118` — the Gridley compatibility-corpus project holds committed ISSUE-118 on branch `opus/gridley-corpus` (`9ecc6fc`, `28fdb2f`, `6e3b38a`); this issue's claim was still uncommitted. Evidence in `issues/open/AFLDB-ISSUE-119.md` §0. Branch and worktree have since been renamed to `codex/issue-119` / `D:\dev\afldb-issue-119`.
- **Created:** 2026-08-31
- **Resolved:** N/A
- **Severity:** Medium
- **Area:** Admin / Security / Natural-language search / Telemetry / Database
- **Files:** `src/db/migrations/081_nl_search_telemetry_clear.sql` (new), `tools/maintenance/privileges.sql` (function reconciliation section), `src/db/queries/nl-search-telemetry-clear.ts` (new — the typed query helper), `tests/integration/nl-search-telemetry-clear.test.ts` (new), `tests/integration/privileges.test.ts` (ISSUE-119 capability describe), `tests/nl-search-log.test.ts` (`clearNlSearchTelemetry count boundary` describe), `src/lib/auth/session.ts` (the `auth_audit_log` INSERT extracted into one `postgres.ISql`-typed private writer; `audit()` unchanged; new `auditInTransaction`), `tests/auth.test.ts` (`auth_audit_log writer` describe), `.env.example` (`AFLDB_TEST_AUTH_DATABASE_URL`), `issues/open/AFLDB-ISSUE-119.md`, `IssuesIndex.md`, `issues.md`
...
- **Runbook:** `issues/open/AFLDB-ISSUE-119.md` (authoritative Stage 1 inventory, retention/security/deletion contract, tests, acceptance criteria, Stage 2 gate, and the §20–§28 Stage 2 implementation and validation records)
