...
- **Status:** In progress — Stage 1 complete; §5/§6 retained-evidence boundary approved by the operator 2026-09-01; Stage 2 started the same day. Migration `081` was **applied to `afldb_test` on 2026-09-01** (runbook §23), its first execution anywhere. Its `privileges.sql` reconciliation and the guarded `_test` integration tests (runbook §22) are written and source-reviewed but **still unexecuted**, and the clear function has never been called
...
- **Runbook:** `issues/open/AFLDB-ISSUE-119.md` (authoritative Stage 1 inventory, retention/security/deletion contract, tests, acceptance criteria, Stage 2 gate, and §20 Stage 2 implementation record)
