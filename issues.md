...
- **Files:** `src/db/migrations/081_nl_search_telemetry_clear.sql` (new), `tools/maintenance/privileges.sql` (function reconciliation section), `tests/integration/nl-search-telemetry-clear.test.ts` (new), `tests/integration/privileges.test.ts` (ISSUE-119 capability describe), `.env.example` (`AFLDB_TEST_AUTH_DATABASE_URL`), `issues/open/AFLDB-ISSUE-119.md`, `IssuesIndex.md`, `issues.md`
...
- **Runbook:** `issues/open/AFLDB-ISSUE-119.md` (authoritative Stage 1 inventory, retention/security/deletion contract, tests, acceptance criteria, Stage 2 gate, and the §20–§24 Stage 2 implementation and validation records)
