...
expected to report an empty plan there. Dev's 17 pairs fall to §8 action 2, **report only** — the
stale halves carry no dependent data, retiring an empty row needs a DELETE that `afldb_import` must
not perform and that §8 rule 3 forbids ad hoc, so **the contract stays report-only for this shape
and no DELETE is proposed**; disposing of dev's 17 empty halves is a separate supervised decision
outside this issue's authorised scope. A further rekey attempt over that shape is already specified
and tested: it resolves to `rekey_would_merge`, writes nothing, and withholds the whole fixture
family. **§9.4 and §9.6 were not run**, so §7's hardening index remains unmeasured and unwritten
