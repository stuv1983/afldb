...
**Current next action (supersedes the retained S6 handoff above): S8 only** — provision R and
the pinned fitzRoy, then implement the bounded service/timer and supervised real-run contract in
runbook §16. Do not start S9. S7 remains uncommitted; never include the stray `must` file.
