...
- **Status:** Open — Stage 1 (investigation and design) complete; **nothing implemented**
- **Severity:** Medium
...
- **Resolved:** N/A
- **Files (expected, none changed yet):** `deploy/afldb-r-env.sh` (new), `deploy/afldb-r-preflight.sh` (new), `deploy/afldb-settle-afltables.sh`, `docs/deployment.md`, `tests/current-season-import.test.ts`
...
Whether `/home/arm/R/library` is intended on the dev host or the host should be reconciled to
the documented `/usr/local/lib/R/site-library` (recommended). Either answer leaves the
repository implementation identical; it decides the host action and one documentation
paragraph. Production's actual R library location is also unconfirmed and must be checked.
