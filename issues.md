...
### Exact next action
**S0 through S4 complete (2026-09-02); all uncommitted.** Both S1/S2 closeout gaps are closed:
`canonical_applications` was added to the post-045 exception list in
`tests/reference-data.test.ts` (migration 083 untouched, `grant_import_write()` not used, no
privilege widened), and the 5 §8 manual-authority integration cases were executed green against
`afldb_test` through an SSH local forward — `settle-afltables` integration is 24 passed / 1
skipped, the skip being the pre-existing `AFLDB_TEST_IMPORT_DATABASE_URL` conditional. No S1/S2
design problem was exposed.

Operator: review and commit S1–S4; never the stray `must` file. Committing makes the `083` claim
visible to other branches. Then a fresh session, worktree `D:\dev\afldb-issue-122`, branch
`claude/issue-122`, carry-over `issues/open/AFLDB-ISSUE-122.md`, **start at S5** (the applier:
`src/lib/acquisition/canonical-apply.ts`, the four target writers, the `canonical_applications`
ledger, savepoint isolation, `targetNeedsApply` retry, wired into `recordOutcome()`;
`SettleCounters`' literal-`0` types become `number`).

**Binding on S5.** The S2 manual-authority snapshot is synchronous and is loaded **once** inside
`sql.begin()`, so a `data_overrides` change later in the same run would be decided against a
stale snapshot. This was deliberately not solved in S3 or S4. S5 must re-read both the manual
authority and the canonical target state **immediately before the canonical mutation, inside the
savepoint** — E4 and E5 together — and must feed the S3 predicate `autoApplyOwnership()`
freshly-read ownership rather than the value `resolveTarget()` computed earlier in the pass.
