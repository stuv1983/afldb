...
|---|---|---|---|---|
| `AFLDB-ISSUE-119` | Medium | Admin / Security / NL search / Telemetry / Database | Stage 1 maps three NL relations plus the incoming app-health FK. Disposable log rows may be cleared; durable admin reviews, immutable reader feedback, their required log/ancestor context, app-health rows and audit history must remain. Migration `081_nl_search_telemetry_clear.sql`, its `tools/maintenance/privileges.sql` reconciliation and the guarded `_test` integration tests (deeper-than-one-parent ancestry fixture with a deleted mid-chain sibling, orphaned feedback, app-health `SET NULL`, sequence non-reset, rollback, lock-cutoff, restricted `afldb_auth` credential) are written and source-reviewed 2026-09-01 but are unexecuted and database-unvalidated; no Server Action, UI or database mutation exists yet. Runbook: `issues/open/AFLDB-ISSUE-119.md`. Renumbered from `AFLDB-ISSUE-118` on 2026-09-01: the Gridley corpus project holds committed ISSUE-118 on `opus/gridley-corpus`, while this claim was still uncommitted (runbook §0). | **Stage 2 in progress (§20–§22). Next: first execution of any of this SQL, operator-run per runbook §22.7 — `npm run db:migrate:test`, `npm run db:privileges:test`, then the two focused vitest suites with both `_test` DSNs set. No Server Action or UI until that evidence passes.** |
...
- **Status:** In progress — Stage 1 complete; §5/§6 retained-evidence boundary approved by the operator 2026-09-01; Stage 2 started the same day. Migration `081`, its `privileges.sql` reconciliation and the guarded `_test` integration tests (runbook §22) are written and source-reviewed but **unexecuted and database-unvalidated**
