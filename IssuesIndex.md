...
**Open issues:** 5 tracked here — `AFLDB-ISSUE-104`, `AFLDB-ISSUE-110`,
`AFLDB-ISSUE-113`, `AFLDB-ISSUE-116`, `AFLDB-ISSUE-122`.
...
<!-- RETIRED 2026-09-02 — `AFLDB-ISSUE-112` is **Resolved** and is NO LONGER an open issue.
     ...
     that mechanism. -->
| `AFLDB-ISSUE-122` | Medium | Data acquisition / Import architecture / Data integrity | Valid new AFL Tables games are acquired, validated, persisted and projected but **never become canonical** — `AFLDB-ISSUE-099` built the pipeline and deliberately stopped at zero canonical writes. **First wrong layer: `recordOutcome()` (`src/lib/acquisition/settle-afltables.ts:2353-2487`)** — a valid promotable observation hits the non-refusal `else` at `:2439` and falls into the function's only terminal action, the `promotion_candidates` upsert at `:2458-2484`; no branch writes canonical data, and `canonicalRowsInserted`/`Updated` are literal-`0` **types** at `:1327-1329`. Four blocks behind it: no canonical writer in `src/lib/acquisition/*`; accept unrepresentable (`promotion-review.ts:29-36`); the `UNAVAILABLE_MANUAL_AUTHORITY` stub (`observations.ts:398`); and `match_period_scores`/`brownlow_round_votes` having no `source_id`. **Nothing consumes the queue** — no promotion-review UI, route or handler exists. Operator superseded the "canonical promotion is reviewed by default" policy for AFL Tables. Also confirmed in scope: `current-season-import.ts:922-943` (Squiggle/Kali) updates canonical `matches` with **no ownership predicate and no `data_overrides` check** — silent ownership transfer and silent revert of admin score edits; its canonical write is retired. Planned: a savepoint-isolated auto-apply stage reusing `reconcile()`'s nine gates, a real `data_overrides`-backed authority provider (satisfying ISSUE-099 A4 **without** widening migration 073's CHECK), provenance columns for the two source-less targets (A1/A2/A3), and a new append-only `canonical_applications` ledger — `promotion_decisions` stays human-only and **no auto-applied record ever creates a candidate that is then marked accepted**. Key files: `src/lib/acquisition/{settle-afltables,reconciliation,observations}.ts`, `src/lib/external-afl/current-season-import.ts`, `data/reference/source-families.json`, `tools/maintenance/privileges.sql`, `deploy/`. | **S0-S4 COMPLETE 2026-09-02 (runbook §23); ALL UNCOMMITTED. Runbook: `issues/open/AFLDB-ISSUE-122.md`.** S1 closeout: `tests/reference-data.test.ts` post-045 exception list now carries `canonical_applications` (migration 083 untouched, `grant_import_write()` not used, no privilege widened). S2 closeout: the 5 §8 manual-authority integration cases EXECUTED and GREEN on `afldb_test` via an SSH local forward — `settle-afltables` integration **24 passed / 1 skipped** (the skip is the pre-existing `AFLDB_TEST_IMPORT_DATABASE_URL` conditional). No S1/S2 design problem was exposed. S3: `TARGETS_WITHOUT_SOURCE_ID` **removed**; `resolveTarget()` reads the real `source_id` for all four targets (`match_period_scores` ownership is the shared owner of its rows, a mixed set being indeterminate, and `source_id` is stripped from the compared `period_scores` value so a run cannot look like a correction); new pure E3 predicate `autoApplyOwnership()` — `new_target` insertable, afltables-owned updateable, foreign-owned refused, and `source_id IS NULL` refused as `ownership_indeterminate` for the AUTOMATIC path only. The generic `evaluateOwnership()` / `evaluateTargetOwnership()` still answer `ok` for a NULL owner and are unchanged; a test pins the divergence on the same input. **No canonical write and no S9 adoption.** S4: `corroboration_policy` (`blocking` | `advisory`) added to the source-family contract, defaulting to `blocking` for every undeclared family, declared `advisory` on `afltables`/`match` and `afltables`/`player_match_stats` only; `reconcile()` gate 7 honours it. `classifyCorroboration()` still runs, the report still travels, and the `source_disagreement` `data_issues` row is now opened/deduplicated from the REPORT rather than the refusal verb — required, because keying it off the verb would have silently stopped opening findings the moment the veto was removed. Evidence preserved, only the veto withdrawn; Squiggle/Kali can neither block an AFL Tables write nor become a prerequisite for one. Gates: `reference-data` 42/42, `current-season-import` 201/201, `settle-afltables` integration 24+1 skipped, five other registry consumers 99+1 skipped, `tsc --noEmit` exit 0, `eslint` delta zero (24 pre-existing `no-explicit-any` in `reference-data.test.ts`, 24 at HEAD too), `git diff --check` clean. **Next action: S5 (the applier)** — fresh session, same worktree and branch, carry-over the runbook; `src/lib/acquisition/canonical-apply.ts`, the four target writers, the `canonical_applications` ledger, savepoint isolation, `targetNeedsApply` retry, wired into `recordOutcome()`. **BINDING on S5: the S2 authority snapshot is loaded once inside `sql.begin()`, so S5 must re-read manual authority AND canonical state immediately before the canonical mutation, inside the savepoint (E4 + E5), and must feed `autoApplyOwnership()` freshly-read ownership.** Still open from S0: the production §15.1 read-only query that closes the S9 decision. |
