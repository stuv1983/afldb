...
- **State:** Open / **Planning — Approved for Stage 1 (2026-09-11). NOT implemented, NOT resolved.** No application code, migration, privilege, test or deployment change exists; the only artefact is the plan `AFLDB-ISSUE-159.md` on branch `opus/issue-159-coach-admin` (worktree `D:\dev\afldb-issue-159`). AFLDB has **no** coach admin surface today (`EDITABLE_ENTITIES` is players/matches/draft_picks only and nothing under `src/app/admin/` mentions coaches), so P3 ADDS a surface rather than replacing a "coach slice" of `/admin/data-editor` — a correction to ISSUE-156 §1. **Stop condition C-1 DECIDED:** a manually created coach is an ordinary `coaches` row whose `afltables_coach_path` AND `name_key` are both a synthetic `'manual:' || <token>` (opaque, minted once, never name-derived) under the EXISTING `manual_admin_edit` source row (057:36-42), with the durable record carried by a `data_overrides` row (`entity_type='coaches'`, `entity_key='manual_admin_edit:<token>'`, `field_group='identity'`) that reload and promotion replay reconstructs — **no `NOT NULL` relaxed, no `UNIQUE` dropped, no new `sources` row, no merge tooling**. The nullable-path alternative was rejected because `data_edits.row_id` is lineage-bound (`promotion-inventory.ts:322-337`) and name-derived identity is forbidden (`:78-82`), so a NULL path makes a manual coach's `data_edits` rows unremappable and the promotion stops; the manual `name_key` must be namespaced too because the importer's upsert conflict target is the path alone (`import_match_coaches.py:398-412`), so a real `"Surname, Given"` string would abort the whole coach batch on `coaches_name_key_key` the first time AFL Tables published that coach. **Highest-risk planning finding, not in C-1's text:** `src/lib/acquisition/manual-authority.ts` pins the live `data_overrides.entity_type` CHECK as an EXACT SET (`:191-197`) and `Object.keys(EDITABLE_ENTITIES)` likewise (`:103-107`); both feed `overrideScopeProven` (`:250-253`), and when it is false `manualAuthorityVerdict()` returns `'indeterminate'` for `match_period_scores`, `player_match_stats` and `brownlow_round_votes` (`:146-149`) — so widening the CHECK to admit `'coaches'` **degrades the nightly settle to propose-only in EITHER deploy order**. **Operator decisions approved 2026-09-11:** D-1 rewrite the proof to an order-independent, strictly-equivalent four-condition form (no deploy window may allow a silent apply→propose switch); D-2 the admin UI reads override state through a narrow server-side SELECT-only import-role helper — no direct `afldb_import` authority in browser/admin code and no broadened `afldb_auth`, so `privileges.sql` is untouched; D-3 do not widen `coaches_link_ck` unless Stage 1 evidence proves it necessary (then stop and report); D-4 an explicit human decision wins visibly and durably over a later source refresh. **Migration 095 allocated** (`data_overrides.entity_type` += coaches/match_coaches; `data_edits.table_name` += `'coaches'` only, since `match_coaches` has a composite PK and no row id and an assignment is audited against `'matches'`; plus `coaches_path_namespace_ck` and `coaches_manual_identity_ck`). **Coach reconciliation is REPORTED OUT OF SCOPE and stays P9-class** — the manual row can never be deleted (its `data_edits` ids stop the promotion), so a merge needs a durable supersession record, mandatory preview, second review and a reversible merge record, and no merge tooling exists anywhere in the repository; P3 ships duplicate PREVENTION only and reconciliation takes its own ID when P9 starts.
- **Key files/subsystems:** `AFLDB-ISSUE-159.md` (implementation contract — §16 Stage 1 deliverables, §17 Stage 1 gates); `src/lib/acquisition/manual-authority.ts`; `src/db/migrations/095_coach_admin_overrides.sql` (to create); `tools/migration/common.py` (`replay_admin_overrides`), `tools/migration/import_match_coaches.py` (two ordered replay call sites); `tools/db/promotion-inventory.ts` (new `afltables_coach_path` identity rule + `data_edits` coach target); `src/db/queries/audit-log.ts`; `docs/production-promotion.md` §8; Stage 2 only: `src/lib/auth/capabilities.ts`, `src/app/admin/nav-model.ts`, `src/app/admin/coaches/**`, `src/db/queries/admin-coaches.ts`.
- **Next action:** **Stage 1 in a fresh Opus 5 / high-effort session** (proof rewrite → migration 095 → replay branches → promotion lineage; no UI). Gates in order: G0 implementation preflight + migration-number re-check; G1 the rewritten proof in BOTH deploy orders; G2 §5.3 pre-check (`bad_path=0`, `bad_name_key=0`, `total=383`) then migration on `afldb_test`; G3 reload safety on `afldb_test`; **G4 the HARD GATE — a real DEV settle run proving the three unrepresentable targets still APPLY, not merely propose**; G5 the 086 source contract; G6 `db-promotion-check` incl. `assertContractCoherent()` vs the DEV `historicalOnly` declaration; G7 `coach-reconciliation`; G8 `tsc`. **Stage 2 (the `/admin/coaches` surface) must not begin until G4 passes.**
