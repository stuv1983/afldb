...
**Last updated:** 2026-09-12 (ISSUE-160 blocking DEV render defect fixed, uncommitted and unvalidated)
...
- **State:** **Stage 1 and Stage 2 IMPLEMENTED and COMMITTED 2026-09-11 (Stage 1 `91935b9`, Stage 2 `a947e52`), not deployed, not merged. Locally audited 2026-09-11: one real atomicity defect (a refusal returned after a write COMMITTED the partial mutation in `attachAflTablesIdentity` / `adoptLegacyPick`) and eight new Stage 2 ESLint errors found, both fixed with regression coverage and left uncommitted for operator review.** Stage 2 delivers `/admin/draft`, `/admin/draft/new`, `/admin/draft/[id]` and `/admin/draft/revalidate`; capabilities `data.draft.read` (Admin+) / `data.draft.edit` (Super Admin), enforced at every page/route/action boundary and validated by `tests/auth.test.ts`'s source contract; the Data-group nav entry; the D-9 shared `src/lib/admin/revalidate-route.ts` + `src/components/admin/action-submit.ts` extraction (coaches switched to them, behaviour-preserving, its own suite still green); and the `/admin/data-editor` draft-slice UI removal (D-5 UI half). Validated against `afldb_test` (24 new integration/unit tests) plus the full previously-green suite (`auth`, `edit-spec`, `data-overrides-source-contract`, `player-link-mutations`, `db-promotion-check`, `admin-coach-actions`, `integration/admin-coaches`); `tsc --noEmit` clean. **DEV/Playwright acceptance (gate 16) deferred by the operator's Admin Centre release-batching decision, not run.** Operator decisions D-1..D-9 decided 2026-09-11 and implemented as decided. **D-8 resolved to the HARD-REFUSAL branch of J-3**: gate-2 probe (a) measured ZERO `(draft_year, draft_kind, pick_number)` collisions across all 6,810 source selections on the rebuilt `afldb_test` and zero on `afldb_dev`, so the confirmation branch is deliberately not implemented. Gate 2 also measured: legacy `source_id IS NULL` picks = **0** on both `afldb_test` and `afldb_dev` (so D-7 adoption has no backlog on either, and probe (d) on PROD is still OPEN); DEF-1 residue (`null|%` override keys) = **0** on both (probe (e) on PROD still OPEN); `data_edits` rows with `table_name='draft_picks'` = **0** on both (probes (g)/(h) on PROD still OPEN); NULL-pick-number kinds = `free_agency`, `post_draft`, `pre_draft`, `trade` (100% NULL each), every other kind zero; same player+year+kind duplicates = 0. Delivered: `createPlayerInTransaction` mints a `manual_admin_edit` identity + whole-row `identity` record and derives `search_name`/`slug`/`sort_name` in SQL (this also FIXED a real bug -- the JS template literal `'\s+'` was the letter `s`, so every run of `s` in a name became a hyphen); new `src/db/queries/admin-draft.ts` holds 6.1-6.8 and is the only `INSERT INTO draft_picks` in `src/`; `replay_admin_overrides` gains fail-closed manual branches for `players` and `draft_picks` (binding order `players` -> `draft_picks`), with the debut BIND case that prevents a twin; D-2 symmetric name+DOB refusal in `import_fitzroy_core.import_players()`; `manual_admin_edit` ledger target in the DraftGuru exporter/importer; D-3 `draft_pick_key` lineage rule + `data_edits` target and the widened `players` rule; `/admin/data-editor` draft writer removed. **No migration (096 still free), no privilege change.** W-1, W-2, W-3, W-16 cleared and verified; W-18 (S-1) still OPEN by design. **2026-09-12 — DEV browser acceptance found a BLOCKING universal render defect on `/admin/draft/[id]` (every id, both roles, digest `4230168678`): J-18's revision is `max(data_edits.id)`, and it was read as `afldb_app` on the page and as `afldb_import` in the compare-and-swap, neither of which may SELECT that audit table (057 grants it to afldb_auth alone; 066 gives import `INSERT` only; 039 makes afldb_app's read registry-only and `data_edits` is not registered). Fixed by one `readDraftPickRevision()` on `authSql` shared by the page and all three CAS call sites, with a unit regression pinning the pool and an integration J-18 round trip; no migration, grant, capability, authority-model, durable-identity or replay/promotion change. UNCOMMITTED and UNVALIDATED — no tsc, vitest, ESLint or `npm run build` run.**
- **Key files/subsystems:** `AFLDB-ISSUE-160.md` (runbook); `src/db/queries/players.ts` (`createPlayerInTransaction`), `src/db/queries/admin-draft.ts` (6.1-6.8), `src/db/queries/data-edits.ts`, `src/lib/edit/spec.ts`, `src/app/admin/data-editor/{page.tsx,CreatePlayerForm.tsx}` (draft slice removed), `src/app/admin/draft/**` (list/detail/new pages, 6 mutation panels, actions/validation/submit-helper/revalidate route+allowlist), `src/lib/admin/revalidate-route.ts` + `src/components/admin/action-submit.ts` (D-9, shared with coaches), `src/lib/auth/capabilities.ts`, `src/app/admin/nav-model.ts`, `tools/migration/common.py` (`replay_admin_overrides`), `tools/migration/import_fitzroy_core.py` (D-2), `tools/rebuild/draftguru/{import_draftguru.py,export_link_decisions.py}`, `tools/db/promotion-inventory.ts`, `docs/production-promotion.md` §8; tests `player-link-mutations`, `edit-spec`, `auth`, `data-overrides-source-contract`, `db-promotion-check`, `integration/draftguru-import`, `admin-coach-actions`, `integration/admin-coaches`, new `admin-draft-actions` + `integration/admin-draft`.
- **Next action:** operator reviews and commits the audit fixes plus the §18 closure (one uncommitted working tree); Stage 1 + Stage 2 are already committed at `91935b9` / `a947e52`. ISSUE-160 is CODE-COMPLETE and locally validated: both §18 list affordances the audit found undelivered -- the `state` filter (override | duplicate | awaiting-identity) and the unresolved-row deep link to `/admin/player-links` -- are now implemented, read-only, with no migration and no new mutation path. Only deliberately deferred external gates remain: PROD probes (d)/(e)/(g)/(h), S-1, the real-importer half of gate 9, DEV deploy/build (gate 15) and the three-role Playwright / rendered responsive acceptance (gate 16). NOT resolved: DEV acceptance has not happened. ISSUE-160 stays OPEN pending deferred DEV acceptance: gate 15 (DEV deploy/build) and gate 16 (three-role Playwright at 320/768/1000/1280/1920) run only when the operator updates DEV for the Admin Centre batch, per the operator's explicit hold. Still open from Stage 1: gate 2 PROD read-only probes (d)/(e)/(g)/(h) -- not run, PROD deliberately untouched -- and gate 9's real-importer half, which SKIPS in this worktree (no `.venv`, no accepted DraftGuru Stage A snapshot, no `AFLDB_TEST_IMPORT_DATABASE_URL`); both are written and run on a host that has them. Operator decision **S-1** (§11.1, sequencing the `promotion-inventory.ts` change against the paused ISSUE-151 PROD promotion `20260907-234124`) is required before the promotion-tooling change DEPLOYS, not before this commit. **NOW FIRST: validate the 2026-09-12 `/admin/draft/[id]` crash fix** — `npx tsc --noEmit`; `npx vitest run tests/admin-draft-actions.test.ts`; `npx vitest run tests/integration/admin-draft.test.ts` (needs `AFLDB_TEST_DATABASE_URL`; the suite now redirects `AFLDB_AUTH_DATABASE_URL` to it as well); ESLint over `src/db/queries/admin-draft.ts`, `src/app/admin/draft/[id]/page.tsx`, `src/app/admin/fixtures/SingleFixtureForm.tsx` and the two test files; then `npm run build`. Then commit, redeploy DEV and **re-test `/admin/draft/[id]` in the browser** — a local build cannot accept a rendered-route crash. Nothing else about ISSUE-160 changed.
...
- **State:** **Stage 1 AND Stage 2 code-complete 2026-09-11 (Stage 1 Opus 5 high, Stage 2 Sonnet 5 high); NOT committed, NOT deployed.** Stage 2 delivers `/admin/season-lists`, `/admin/season-lists/[season]`, `/admin/season-lists/[season]/[club]`, capabilities `data.seasonLists.read/.edit`, the Data-group nav entry, and the ISSUE-160 handoff link (`/admin/draft/[id]` header + `NewPickWizard` success panels) — no mutation writes `season_list_members` from the draft side. No revalidate route (`revalidatePaths: []` always). One additive Stage 2 read, `readDraftSuggestions()`. **Both stages are now committed (`97af605` Stage 1, `1441a5b` Stage 2) and the full local validation has been RUN and passed** (local completion audit 2026-09-11, Opus 5 high, operator-authorised command execution): preflight READY (0 blockers), `tsc` clean, 204 unit + 85 integration + 420 contract + 2 FK-index tests pass, eslint clean over every ISSUE-161-authored file, `git diff --check` clean. The audit found and narrowly fixed ONE defect — `CopyForwardPanel` could confirm a copy-forward the operator never previewed (the club checkboxes stayed live after a dry run while only Confirm was offered); Confirm is now gated on the preview still describing the live selection, with 3 regression tests added. Client-only: no action, query, transaction, invariant or migration changed. The **D-3 evidence gate PASSED**: modern-era probe (`season >= 2000`) returned **zero** rows and all 249 historical multi-club player-seasons end at **1992**, so `UNIQUE (season, player_id)` stands. **Migration 096 allocated** (re-checked free) and applied to `afldb_test` only: `season_list_members`, `afldb_season_list_clubs(season)` (the one eligibility rule — 18 clubs for 2027 by forward extension, era identities before the register's last season), `data_overrides.entity_type` widened, both role registries. `admin-season-lists.ts` ships add / multi-select add / remove+tombstone / transfer / copy-forward (preview + write) / the non-authoritative appearances review projection / diagnostics, all one import-role transaction with `RollbackRefusal` for any post-write refusal; `player-identity.ts` extracted behaviour-preservingly from `admin-draft.ts`+`players.ts`; fail-closed `replay_admin_overrides('season_list_members')` (tombstones delete first, `NOT EXISTS` never `ON CONFLICT`) + fitzRoy call site + promotion doc/checklist. Tests: 25 pure, 40 integration (incl. the real Python replay), ISSUE-160 regressions 45+120 green, tsc/lint/`git diff --check` clean; one pre-existing Windows-CRLF-only failure in an untouched file. Fixture-independence proved: 2027 has 0 matches and 0 `club_seasons` rows and every mutation still works. Deviations recorded (replay deletes tombstones first; the derived-rebuild proof is an exact source contract; the test-scoped season ceiling is `NODE_ENV=test`-gated, never a `seasons` row). Stacked on ISSUE-160 (`opus/issue-161-season-lists` from `opus/issue-160-draft-admin` @ `1295d3d`); ISSUE-160 is not merged first and both deploy to DEV together as the Admin Centre batch. Findings: AFLDB has no list concept — every player–club/season table is DERIVED from matches and truncate-rebuilt, so none can hold a listed-but-never-played player; chosen model is a new canonical registry table `season_list_members` (`UNIQUE (season, player_id)`) with the ISSUE-159/160 durability shape (whole-row `data_overrides` keyed `<club_slug>|<season>|<player identity>`, per-mutation import-role transaction, `data_edits` on the player, fail-closed replay branch after `players`, inactive override = removal tombstone). No `retired` flag: not-current = no membership in the current list season. Season boundary solved without writing `seasons`/`clubs`/`club_seasons` (all reference-loaded or derived): list season ≤ `max(seasons.year)+1`, eligible clubs = `is_current_afl_club` identities for seasons at/beyond the register. One migration required (next free 096, not allocated). No public-consumer change required. ISSUE-160 integration = handoff link + draftee suggestions, explicit add, never automatic.
- **Key files/subsystems:** `AFLDB-ISSUE-161.md` (runbook); migration 096, `src/db/queries/admin-season-lists.ts`, `src/db/queries/player-identity.ts` (extracted from `admin-draft.ts`), `src/app/admin/season-lists/**` (page/actions/validation/submit-helper/5 panels), `src/lib/auth/capabilities.ts` (`data.seasonLists.read/.edit`), `src/app/admin/nav-model.ts`, `src/app/admin/draft/{submit-helper.ts,actions.ts,NewPickWizard.tsx,[id]/page.tsx}` (handoff link), `tools/migration/common.py` (`replay_admin_overrides('season_list_members')`), `tools/migration/import_fitzroy_core.py` (call site), `docs/production-promotion.md` §8, tests `admin-season-list-actions`, `integration/admin-season-lists`, `auth`, `data-overrides-source-contract`, `reference-data`, `db-promotion-check`.
- **Next action:** **code-complete and locally validated — operator reviews and commits the audit's two uncommitted files** (`src/app/admin/season-lists/CopyForwardPanel.tsx` and `tests/admin-season-list-actions.test.ts`, plus tracking). The 2026-09-11 repair pass findings (2 stale nav expectations, a real `generateMetadata` guard defect on both dynamic-route pages, an internal-link lint error, and the missed mandatory `+added/-departed vs S-1` / "departed since S-1" contract item, §11/§13/§15.1/§20) are all fixed AND now re-validated green; the local completion audit added the one further `CopyForwardPanel` preview/confirm fix. Nothing further is required before the next Admin Centre issue. DEV deploy order when the Admin Centre batch ships: **migration 096 → `npm run db:privileges` → code** (privileges could not be reconciled from the workstation — no `psql` — and the table's app read is fail-closed until they are), then combined ISSUE-160 + ISSUE-161 DEV acceptance and role-based Playwright. Neither ISSUE-160 nor ISSUE-161 is resolved.
...
- **State:** **OPEN. 2026-09-12 — RELEASE-BLOCKING DEV BUILD DEFECT FOUND AND FIXED, UNCOMMITTED AND NOT RE-VALIDATED.** The combined DEV rollout applied migrations 096/097/098 and reconciled privileges successfully (98/98, 0 pending), then `npm run build` refused the batch before any restart: `SingleFixtureForm.tsx` (and `RoundPanel.tsx`, `RoundBatchForm.tsx`) value-imported `MAX_HOME_AND_AWAY_ROUND` / `MAX_BATCH_ROWS` from `src/db/queries/admin-fixtures.ts`, which carries `import 'server-only'`. The pure §9 vocabulary now lives in new server-neutral `src/lib/fixtures/spec.ts` and is re-exported unchanged by `admin-fixtures.ts` (no server consumer changed, no constant duplicated, no query/type/SQL moved, no schema change, 096-098 untouched). All 88 `'use client'` files in `src/` audited: only those three; every other client `@/db/` import is `import type` and erased. One narrow boundary regression added to `tests/admin-fixture-actions.test.ts`. **`npm run build` is the acceptance criterion and has not been run.** Earlier: Stage 1 committed `cb98c67`; Stage 2 admin surface committed `6a9fbc4`; both validated locally. Final whole-issue audit 2026-09-12 (Opus 5 high, 1M): NO stop condition fired, NO Stage 1 file changed, 8 Stage 2 defects fixed — those fixes are UNCOMMITTED and NOT RE-RUN. NOT merged, NOT deployed; DEV and PROD untouched throughout.** Stage 2 validation (operator, 2026-09-11): tsc clean; `tests/auth.test.ts` 138/138; fixture + contract bundle 452 passed / 4 skipped; `tests/integration/admin-fixtures.test.ts` 36/36; ESLint clean; `preflight` READY; worktree clean. **Audit fixes** (`AFLDB-ISSUE-162.md` §39): a **void** fixture was offered Reschedule/Venue/Round/Clubs panels the backend can only refuse `invalid_transition` — hiding them is convenience, the backend gate is unchanged and still refuses a forged request; clearing a date left an invisible, disabled start time that still submitted and refused `invalid_schedule`, and in the round batch refused the WHOLE round; `ClubsPanel` rendered its first option when a fixture's club was no longer in the eligible list, so it could display one club and save that one; duplicate React keys on the per-round `round_byes` diagnostics; `badge-danger` used in three files and defined nowhere in `globals.css`; the batch parser coerced `venueId` where it held club ids strictly; batch row controls had no accessible names; `minWidth: 0` added to the two-column grids against 320px overflow. The batch-fingerprint regression now pins season, round type, both club ids, date, venue id, unmapped venue name, notes and row count. **Audit confirmations:** the capability contract is clean (`tests/auth.test.ts`'s ISSUE-158 walker requires `requireCapability()` as the first awaited call of every fixture page and all ten actions, and no role helper is used in the fixture tree); preview/confirm staleness rests on two independent gates, the server fingerprint being the real one because the client snapshot omits the season; CAS runs on every edit before the lifecycle gate; the played resolution is deterministic, fail-closed, links nothing on ambiguity and never locks on it, and no `match_id`/`match_key` exists or is compared; venue mapped/unmapped/TBC are never collapsed and an unrelated edit does not destroy `venue_raw`; the audit link is the numeric `fixtures.id` the generic viewer expects, matching the `coaches/[id]` precedent. **Earlier history —** first operator validation run found 4 Stage 1 defects — all repaired 2026-09-11, none re-run yet.** The repairs (`AFLDB-ISSUE-162.md` §37.8): `DATA_EDIT_TABLE_LABELS` was missing its `fixtures` entry (tsc failure); `normaliseSchedule()` accepted impossible calendar dates because `Date.parse()` normalises rather than rejects them (`2027-02-30` was accepted) — now a real-calendar round trip; `resolvePlayed()` returned `played` on `exactCount === 1` even with a competing swapped candidate, violating D-6 — the truth table is now one-candidate-or-ambiguous; and `PLAYED_RESULT_FACTS` picked a result row with `ORDER BY id LIMIT 1` independently of that resolution, so an ambiguous fixture could be compared against an arbitrary match — now guarded to the uniquely resolved row. Brittle source-contract extractions repaired without weakening any guarantee (ISSUE-159's `match_coaches` branch was reading later branches; the ISSUE-162 contracts were reading the module's own comments). **Second run 2026-09-11 with the `.env` in place: tsc, 69/69 fixture unit, 383 contract/promotion/current-season, 124/124 ISSUE-161 regressions, ESLint and `git diff --check` all GREEN; migration 097 is applied to `afldb_test`. `tests/integration/admin-fixtures.test.ts` was 31 passed / 5 failed — all five a TEST-ORDERING defect, now repaired:** the played-resolution tests wrote the played `matches` row BEFORE creating the fixture, so §13's `already_played` guard correctly refused the create and the tests died before reaching the resolution. They now follow the real chronology (fixture while scheduled → result later → read resolves), the seeding helper is renamed `insertPlayedMatch()` with the ordering rule documented, and no production file was touched. Awaiting the operator's rerun, expecting 36/36. Migration **097** re-checked free and allocated: `fixtures` (schedule facts only — no score, result, margin, attendance, `match_id` or `match_key` column), `data_overrides.entity_type` += `'fixtures'`, `data_edits.table_name` += `'fixtures'`, `grant_app_read` + `grant_import_write`. `src/db/queries/admin-fixtures.ts` ships create / round-batch preview+fingerprint-gated commit / reschedule / venue / round / clubs / notes / cancel / reinstate / void / the read-time played resolution / season+diagnostics readers, each one import-role transaction taking `pg_advisory_xact_lock(717275, season)`, every precondition before the first write and `RollbackRefusal` for any post-write refusal. Fail-closed `replay_admin_overrides('fixtures')` added to `tools/migration/common.py` (+ `FIXTURE_STATUSES`/`FIXTURE_ROUND_TYPES`), fitzRoy call site after `matches`, promotion doc §8 loop, and the `fixture_key` lineage rule + `data_edits` target + acceptance-checklist text in `promotion-inventory.ts`. Tests written but **not yet run**: `tests/admin-fixture-actions.test.ts` (pure) and `tests/integration/admin-fixtures.test.ts` (`afldb_test`, incl. the real Python replay and the derived-safety proof), plus contract updates in `db-promotion-check`, `current-season-import` and `data-overrides-source-contract`. **Validation is the operator's next step** (§ Next action). Original planning finding, unchanged: `matches` requires NOT NULL scores/result/margin (003) and every ladder/season/venue/NL/Grid consumer reads "row exists" as "played", so an unplayed fixture cannot live there (S-1/S-5 fire for any `matches`-based option); AFLW already separates `staging_aflw.fixtures` from `staging_aflw.matches` (025). Chosen: new canonical registry table `fixtures` (schedule facts only, no score column), `matches` untouched, "played" derived by season + round code + club-pair resolution, identity = minted `fixture_key` under `manual_admin_edit`, whole-row `data_overrides` + fail-closed `replay_admin_overrides('fixtures')`, `data_edits` widened with a `fixture_key` lineage rule, never-delete lifecycle (`scheduled`/`cancelled`/`void`), no `seasons`/`clubs`/`club_seasons` write (window `max(seasons.year)..max+1`, clubs via `afldb_season_list_clubs()`), no settle/current-season/rekey change, no public exposure. One additive migration (next free 097, not allocated). Capabilities `data.fixtures.read`/`.edit`; routes `/admin/fixtures`, `/[season]`, `/[season]/new`, `/[season]/[fixtureKey]`; round-at-a-time batch entry with fingerprint-gated all-or-nothing confirm. Stacked on ISSUE-161 (`opus/issue-162-fixture-admin` from `34858ce`); deploys to DEV only with the Admin Centre batch.
- **Key files/subsystems:** `AFLDB-ISSUE-162.md` (runbook); **new** `src/db/migrations/097_fixtures.sql`, `src/db/queries/admin-fixtures.ts`, `tests/admin-fixture-actions.test.ts`, `tests/integration/admin-fixtures.test.ts`; **changed** `src/db/queries/audit-log.ts` (`DataEditTableName` += `'fixtures'`), `src/lib/acquisition/manual-authority.ts` (`OVERRIDE_ENTITY_TYPES`), `tools/migration/common.py` (replay branch + two frozen enumerations), `tools/migration/import_fitzroy_core.py` (call site), `tools/db/promotion-inventory.ts` (`fixture_key` rule, `data_edits` target, checklist), `docs/production-promotion.md` §8, `tests/db-promotion-check.test.ts`, `tests/current-season-import.test.ts`, `tests/data-overrides-source-contract.test.ts`. **Stage 2 (built, `6a9fbc4`):** `src/app/admin/fixtures/**` (4 pages, `actions.ts` with 10 Server Actions, 6 panels, 2 entry forms, `validation.ts`, `submit-helper.ts`, `labels.ts`), `src/lib/auth/capabilities.ts`, `src/app/admin/nav-model.ts`, `tests/auth.test.ts`. **`src/db/queries/data-edits.ts` was NOT changed and needs no change** — it is the generic `/admin/data-editor` read/write module, not an entity-link extension point; the audit viewer's entity page is already generic over `DataEditTableName` and every detail page builds the href inline (`AFLDB-ISSUE-162.md` §38.2 item 1). **Audit (2026-09-12, uncommitted):** `[season]/[fixtureKey]/page.tsx`, `ClubsPanel.tsx`, `ReschedulePanel.tsx`, `RoundBatchForm.tsx`, `RoundPanel.tsx`, `SingleFixtureForm.tsx`, `[season]/page.tsx`, `actions.ts`, `src/styles/globals.css` (additive `.badge-danger` only), `tests/admin-fixture-actions.test.ts`. **DEV build fix (2026-09-12, uncommitted):** **new** `src/lib/fixtures/spec.ts` (the server-neutral §9 vocabulary); **changed** `src/db/queries/admin-fixtures.ts` (imports + re-exports it; `import 'server-only'` retained), `SingleFixtureForm.tsx`, `RoundPanel.tsx`, `RoundBatchForm.tsx`, `LifecyclePanel.tsx` (import from the spec), `tests/admin-fixture-actions.test.ts` (client/server boundary regression).
- **Stage 1 is committed** at `cb98c67` ("Implement AFLDB-ISSUE-162 Stage 1 fixture backend") and **Stage 2 at `6a9fbc4`** ("Implement AFLDB-ISSUE-162 Stage 2 fixture admin UI") — neither merged, neither deployed. **Stage 2 admin surface implemented 2026-09-11 (Sonnet 5 high), validated and committed.** `/admin/fixtures`, `/[season]`, `/[season]/new`, `/[season]/[fixtureKey]`; capabilities `data.fixtures.read`/`data.fixtures.edit`; single-fixture and round-batch (preview/fingerprint-gated confirm) entry; one panel per §15 field group; cancel/reinstate/void as three visibly distinct controls with mandatory-reason confirmation for cancel and void; played fixtures show only identity/schedule/notes. `tests/auth.test.ts` extended (capability table + nav order); no Stage 1 file touched; no new test file (matches the ISSUE-161 Stage 2 precedent). Full record `AFLDB-ISSUE-162.md` §38.
- **Next action:** **FIRST, validate the build fix — it is the release blocker.** `npx tsc --noEmit`; `npx vitest run tests/admin-fixture-actions.test.ts`; `npx vitest run tests/data-overrides-source-contract.test.ts`; ESLint over `src/lib/fixtures/spec.ts`, `src/db/queries/admin-fixtures.ts`, the four `/admin/fixtures` components and the test; then the decisive gate **`npm run build`** — `tsc` and vitest both stub `server-only`, so only the production build proves the layer. The fix is uncommitted and was made in the `opus/issue-163-club-leadership` worktree; if the rollout builds from `main` at `db96948`, land it there first. **Then, unchanged, operator re-gates the audit fixes and commits them** (`AFLDB-ISSUE-162.md` §39.7). In order: `npm run preflight -- --mode implementation --issue 162`; `npx tsc --noEmit`; `npx vitest run tests/auth.test.ts`; `npx vitest run tests/admin-fixture-actions.test.ts tests/data-overrides-source-contract.test.ts tests/db-promotion-check.test.ts tests/current-season-import.test.ts`; `npx vitest run tests/integration/admin-fixtures.test.ts` (expect 36/36 — no Stage 1 file changed); the ISSUE-160/161 regression set (`admin-season-list-actions`, `integration/admin-season-lists`, `reference-data`, `admin-match-mutations`), since `globals.css` is the only shared file the audit touched; eslint over the eight changed TS/TSX files; `git diff --check` and `git status --short`. The Stage 1 all-refs migration-097 collision check (`git log --all --oneline -- src/db/migrations/097_*.sql`) has still never run and remains binding. **`npm run db:privileges` is required on any database that runs the app against `fixtures`** (app read is fail-closed since 039) — deploy order is binding: **migration 097 → `db:privileges` → code**. After that, ISSUE-162 is locally complete and waits on the operator's confirmation that no further Admin/Super Admin addition joins the batch, then combined ISSUE-160 + 161 + 162 DEV deployment and acceptance (three-role Playwright + responsive 320/768/1000/1280/1920). Do not merge ISSUE-160/161 first, do not update DEV, do not deploy, do not touch PROD. ISSUE-162 is **not resolved**. **2026-09-12 DEV acceptance defect (non-blocking, fixed, uncommitted, unvalidated):** on `/admin/fixtures/[season]/new`, clearing the date disabled the start-time control but left its old value visible; storage was already correct (TBC date, TBC time), so this was presentation only. `SingleFixtureForm`'s time input is now controlled and cleared with the date — the rule `RoundBatchForm.updateRow()` already applied. One file, no backend change, no automated regression (there is no DOM test harness for these forms); verified by the focused browser re-test.
...
- **State:** **Stage 1 AND Stage 2 BUILT, VALIDATED and COMMITTED 2026-09-12 (`ea9f3dd`); FINAL LOCAL AUDIT DONE 2026-09-12 (three Stage 2 fixes, uncommitted) — not merged, not deployed; migration 098 applied to `afldb_test` ONLY; DEV and PROD untouched.** Operator decisions D-1…D-18 SIGNED OFF 2026-09-12 with four clarifications (`AFLDB-ISSUE-163.md` §30-§32). Stage 2 record is §33; the audit record is §34. Finding: AFLDB already holds `captaincies` (migration 005) — 1,774 Wikipedia rows 1897–2026 loaded from `data/awards/captaincies.csv`, `Captain`-only vocabulary (30 rows annotated co-captain, 0 vice/acting), natural UNIQUE on `player_name_raw`, `season` FK to the register, free-text `period` — already rendered as the club page "Captains" table and in player honours, and a `player_link_resolutions` target with promotion identity `'none'`. It is a historical honours import; extending it fails on the season FK (no 2027 row until the rollover), name-keyed identity, the reload-collision refusal in `import_awards.py`, and the frozen count gates. Chosen model: new canonical registry `club_leadership` — one row per **appointment** (`appointment_key` UUID minted once; `season` no FK; `club_id`; `player_id NOT NULL`; `role IN ('captain','vice_captain')`, co-captaincy = concurrent captain rows; `status IN ('active','ended','void')`, never deleted; optional `started_on`/`ended_on`, NULL = unknown; partial `UNIQUE (season, player_id) WHERE status='active'`); "current" = status, never a clock; public season = organisation-scoped `max(non-void season)`. Season-list precondition enforced at write time (`FOR KEY SHARE` on the membership), never a FK, not re-checked on replay; list removal/transfer warns (additive `activeLeadershipRole` column), never blocks — ISSUE-161 untouched. Durability = 162 shape: always-active whole-row `data_overrides` keyed `manual_admin_edit:<appointment_key>`, `data_edits.table_name` += `'club_leadership'` with an `appointment_key` lineage rule, fail-closed `replay_admin_overrides('club_leadership')` after `players` re-creating ended and void rows. Capabilities reused (`data.seasonLists.read/.edit`, no new capability, no new nav item). Admin: Leadership section + Appoint panel (player `<select>` over list members only; server-enforced co-captain confirmation; End / Replace / Reinstate / Correct dates / Void) on `/admin/season-lists/[season]/[club]`; Captain column on `/admin/season-lists/[season]`. Public: compact `ClubLeadership` block after the stat strip on `/clubs/[slug]` (continuing current identity only; omitted when nothing active) and the Captains history table becomes a season-boundary union (`captaincies` < 2027, canonical captain rows ≥ 2027). Revalidation: path-based `/clubs/<slug>` for every identity of the affected organisation, computed server-side, POSTed to a new capability-gated allowlisted `/admin/season-lists/revalidate` (the endpoint 161 reserved). No backfill: forward from 2027; `captaincies.py MAX_SEASON < 2027` pinned by a source-contract test; Wikipedia→canonical conversion is a follow-up. Migration **098** expected, not allocated.
- **Key files/subsystems:** `AFLDB-ISSUE-163.md` (runbook); planned migration 098; planned `src/db/queries/admin-club-leadership.ts`, `src/db/queries/club-leadership.ts`, `src/app/admin/season-lists/{leadership-actions.ts,revalidate/route.ts,revalidate-paths.ts,LeadershipPanel.tsx,LeadershipActions.tsx,AppointLeaderPanel.tsx}`, `src/components/ClubLeadership.tsx`; touched: `awards.ts` (`getClubCaptains` union), `admin-season-lists.ts` (additive column), `season-lists/[season]/page.tsx`, `[season]/[club]/page.tsx`, `MemberActions.tsx`, `clubs/[slug]/page.tsx`, `audit-log.ts`, `audit-view.ts`, `manual-authority.ts`, `tools/migration/common.py`, `import_fitzroy_core.py`, `tools/db/promotion-inventory.ts`, `docs/production-promotion.md` §8; tests `admin-club-leadership-actions` (new), `integration/admin-club-leadership` (new), `auth`, `data-overrides-source-contract`, `db-promotion-check`, `current-season-import`. Existing related: `captaincies` (005/042/044), `tools/migration/captaincies.py`, `import_awards.py:2320-2429`, `data/awards/captaincies.csv`.
- **Stage 1 state (2026-09-12, Opus 5 high — BUILT, UNCOMMITTED, NOT YET VALIDATED):** operator signed off D-1…D-18 with four clarifications (capability reuse; a HARD public source boundary ≤2026 legacy / ≥2027 canonical; explicit lifecycle/date consistency; and no silent 2027+ hole in **player honours**). Migration **098** allocated (`098_club_leadership.sql`; working-tree refs check clean — the `git log --all` half is an operator command, not run). Stage 1 delivered: the canonical table + CHECKs + partial unique index + both role registries; `src/db/queries/admin-club-leadership.ts` as the ONE writer (appoint / replace / end / reinstate / correct / void, CAS on `updated_at`, `FOR KEY SHARE` on the membership, `RollbackRefusal` for every post-write refusal, server-computed `revalidatePaths`); `src/db/queries/club-leadership.ts` (`getClubCurrentLeadership`, the one boundary constant); the season-boundary union in BOTH public projections — `getClubCaptains` **and** `getPlayerHonours`, so 2027+ captaincies appear as honours with no Stage 2 rendering change and no write to `captaincies`; the additive `activeLeadershipRole` column in `readClubSeasonList`; the fail-closed `replay_admin_overrides('club_leadership')` branch + frozen enums + the `import_fitzroy_core.py` call site after `players`; the `appointment_key` lineage rule, `data_edits` target, checklist and §8 runbook text; `audit-log.ts` / `audit-view.ts` / `manual-authority.ts`; `tests/integration/admin-club-leadership.test.ts` (new) plus contract extensions in `data-overrides-source-contract`, `db-promotion-check` and `current-season-import`. No stop condition fired.
- **Stage 2 state (2026-09-12, Sonnet 5 high — BUILT, UNCOMMITTED, NOT YET VALIDATED):** Leadership section + per-row lifecycle controls (Replace/End/Correct/Void/Reinstate) + Appoint panel (list-members-only player `<select>`) on the club-season page; Captain column on the season overview; co-captaincy confirmed only as a deliberate follow-up action, never a defaulted checkbox; `/admin/season-lists/revalidate` route + allowlist now exist and are requested for the first time; public `ClubLeadership` block on `/clubs/[slug]`; the Captains table's React key fixed from `${season}-${playerName}` to the row's own `id` (§32.12, now closed). New `tests/admin-club-leadership-actions.test.ts` (pure layer — key shape, vocab, date validation, revalidate allowlist; refusal-before-write proofs stay with Stage 1's integration suite). No new capability, no stop condition fired. Full record `AFLDB-ISSUE-163.md` §33.
- **Final audit state (2026-09-12, Opus 5 high — three Stage 2 fixes, UNCOMMITTED):** whole implementation read against §30 D-1…D-18 with no command executed. Backend integrity, lifecycle/CAS, co-captaincy, replacement, the season-list invariant, replay/durability, the public source boundary, player honours, Server Action/auth, revalidation security, Admin UI and public UI all hold; **no deviation from D-1…D-18, no stop condition**. Fixed: (1) `AppointLeaderPanel` no longer replaces its form with a receipt on success (the `AddPlayerPanel` precedent — a club names a captain and several vice-captains in one sitting); (2) the co-captaincy confirm step is bound to the exact `(role, playerId)` that was refused, so a confirmation cannot be spent on a different appointment; (3) the unreachable Replace co-captaincy branch removed with the reason recorded. Three narrow tests added to the existing pure suite (`parsePositiveInt` never makes a number from a blank/zero/junk field; an omitted optional field reads as absent; `club_leadership` is an accepted `data_edits` entity so the per-row audit link cannot become a 404). Recorded, not fixed (§34.4): appoint-path audit `new_values` omits the derivable `entity_key`; the public Captains table renders no `period` column (and never has); the replay validates date SHAPE not the calendar (unreachable via the writer, still fails closed); a successful per-row mutation leaves a terminal message until reload (deliberate); `readLeadershipDiagnostics()` is unused by any page.
- **Next action:** Stage 2-only re-validation — `npx tsc --noEmit`; `npx vitest run tests/admin-club-leadership-actions.test.ts`; `npx vitest run tests/auth.test.ts`; `npx eslint src/app/admin/season-lists/AppointLeaderPanel.tsx src/app/admin/season-lists/LeadershipActions.tsx tests/admin-club-leadership-actions.test.ts`; `git diff --check`. Stage 1's expensive suites do **not** need re-running (no Stage 1 file was touched). Then commit the audit fixes. Stacked on ISSUE-162 at `f80a1df`; ships only with the combined 160+161+162+163 Admin Centre DEV batch (deploy order 096 → 097 → 098 → `db:privileges` → code), whose rendered Playwright acceptance is the remaining proof. No DEV, no PROD, no merge, no deploy. ISSUE-163 is **not resolved**.
