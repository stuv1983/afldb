...
**Open issues:** 17 tracked here — `AFLDB-ISSUE-117`, `AFLDB-ISSUE-137`, `AFLDB-ISSUE-138`, `AFLDB-ISSUE-139`, `AFLDB-ISSUE-140`, `AFLDB-ISSUE-142`, `AFLDB-ISSUE-144`, `AFLDB-ISSUE-147`, `AFLDB-ISSUE-148`, `AFLDB-ISSUE-149`, `AFLDB-ISSUE-150`, `AFLDB-ISSUE-151`, `AFLDB-ISSUE-152`, `AFLDB-ISSUE-153`, `AFLDB-ISSUE-155`, `AFLDB-ISSUE-156`, `AFLDB-ISSUE-159`.

<!-- UPDATE 2026-09-11 (ISSUE-156 P3 ALLOCATED AS `AFLDB-ISSUE-159` — PLANNING ONLY, NO CODE):
     `AFLDB-ISSUE-159` (Coach administration) added to this index, Planning / Approved for
     Stage 1 — NOT implemented, NOT resolved. Allocated after operator preflight verification
     ...
     OF SCOPE and stays P9-class — separate ID when P9 starts, not 159. Blocking prerequisite
...
- **Next action:** P1 and P2 are complete and merged (`3bbcab0`, `92a898f`). **P3 is allocated as `AFLDB-ISSUE-159`** (Coach administration) with stop condition C-1 DECIDED at its preflight — start ISSUE-159 **Stage 1** in a fresh Opus 5 / high-effort session against `AFLDB-ISSUE-159.md` §16/§17. P4–P12 remain unallocated placeholders and each receives an ID only at phase start.

## AFLDB-ISSUE-159 — Coach administration (ISSUE-156 P3)

- **Severity:** Medium
- **Area:** Admin / Data management / Acquisition / Promotion lineage
- **State:** Open / **Stage 1 COMPLETE and VALIDATED — G0 through G8 all PASSED (2026-09-11).** Committed `dbef4c2`, deployed to DEV (migration 095 applied, 95/0 pending, service healthy), not merged to `main`. G4 passed on real `afldb_dev` as legs A+B+C under operator decision D-5 (213 resolved `match_period_scores` + 8,974 resolved `player_match_stats` targets all already equal to canonical DEV, live authority `clear` for all three unrepresentable targets, Brownlow absence proven to be a source absence); G5 12/12, G6 97/97, G7 13/13, G8 typecheck clean. **NOT resolved: Stage 2 (the `/admin/coaches` surface) is UNBLOCKED but NOT STARTED.** Written on `opus/issue-159-coach-admin` (worktree `D:\dev\afldb-issue-159`), contract `AFLDB-ISSUE-159.md` §16: the order-independent `manual-authority.ts` proof rewrite (D-1); migration **095** (`data_overrides.entity_type` += `coaches`/`match_coaches`, `data_edits.table_name` += `coaches` only, `coaches_path_namespace_ck`, `coaches_manual_identity_ck`); the `coaches` and `match_coaches` `replay_admin_overrides` branches plus their two ordered call sites in `import_match_coaches.py`; the `afltables_coach_path` promotion lineage rule and the `data_edits` coach target; `DataEditTableName` += `'coaches'`; the promotion runbook replay step; four existing test suites extended in place, no new test file. **Untouched as required:** `src/lib/edit/spec.ts`, `privileges.sql`, migration 087, `coaches_link_ck`, `PROMOTION_CONTRACT` — and no route, page, component, Server Action or capability. AFLDB has **no** coach admin surface today, so P3 ADDS one rather than replacing a "coach slice" of `/admin/data-editor` (a correction to ISSUE-156 §1). **C-1 as approved:** a manually created coach is an ordinary `coaches` row whose `afltables_coach_path` AND `name_key` are both a synthetic `'manual:' || <token>` (opaque, minted once, never name-derived) under the EXISTING `manual_admin_edit` source row, with the durable record a `data_overrides` row that reload and promotion replay reconstructs — no `NOT NULL` relaxed, no `UNIQUE` dropped, no new `sources` row, no merge tooling. The nullable-path alternative was rejected because `data_edits.row_id` is lineage-bound and name-derived identity is forbidden; the manual `name_key` must be namespaced too because the importer's upsert conflict target is the path alone, so a real `"Surname, Given"` string would abort the whole coach batch on `coaches_name_key_key` the first time AFL Tables published that coach. **D-1 resolved the blocking coupling:** `manual-authority.ts` pinned the live `data_overrides.entity_type` CHECK as an EXACT SET and `Object.keys(EDITABLE_ENTITIES)` likewise, so widening the CHECK degraded the nightly settle to propose-only in EITHER deploy order; the proof is now four order-independent conditions (readable + unambiguous CHECK; no `UNREPRESENTABLE_OVERRIDE_ENTITIES` literal in it; none among the editor keys; editor ⊆ CHECK), every original refusal retained, and `OVERRIDE_ENTITY_TYPES` demoted to documented inventory the proof never consults. **D-2/D-3/D-4 stand unchanged** and are Stage 2 obligations. **S-4 verified explicitly:** the coach target is a third target on the existing `row_id` column, not a new column, so the DEV `historicalOnly` declaration stays coherent and `assertContractCoherent()` is unaffected — pinned by a test, not left to inspection. **Coach reconciliation remains REPORTED OUT OF SCOPE and P9-class**: the manual row can never be deleted (its `data_edits` ids stop the promotion), so a merge needs a durable supersession record, mandatory preview, second review and a reversible merge record, and no merge tooling exists anywhere in the repository. P3 ships duplicate PREVENTION only.
- **Key files/subsystems:** `AFLDB-ISSUE-159.md` (implementation contract — §16 Stage 1 deliverables, §17 Stage 1 gates); `src/lib/acquisition/manual-authority.ts`; `src/db/migrations/095_coach_admin_overrides.sql` (written); `tools/migration/common.py` (`replay_admin_overrides`), `tools/migration/import_match_coaches.py` (two ordered replay call sites); `tools/db/promotion-inventory.ts` (new `afltables_coach_path` identity rule + `data_edits` coach target); `src/db/queries/audit-log.ts`; `docs/production-promotion.md` §8; Stage 2 only: `src/lib/auth/capabilities.ts`, `src/app/admin/nav-model.ts`, `src/app/admin/coaches/**`, `src/db/queries/admin-coaches.ts`.
- **Next action:** **Operator commits and pushes the Stage 1 closeout on `opus/issue-159-coach-admin` (no `main` merge), then Stage 2 in a fresh session** against `AFLDB-ISSUE-159.md` §9/§15 (Sonnet 5 / medium, escalating to Opus 5 for the assignment transaction and the permission matrix), with D-2/D-3/D-4 binding and the P9-class reconciliation stop out of scope. Merge readiness is its own step (`npm run merge:ready -- --issue 159`). G4 PASSED on real DEV (A+B+C) and no apply was required: every resolved target already equalled canonical DEV. **G4-D is an open dated follow-up** — when the 2026 Brownlow votes publish, the next real DEV settle must show `brownlow_round_votes` traversing the canonical apply path. Operator decision **D-5** (2026-09-11) revised G4: it proves post-095 manual authority stays `clear` for all three unrepresentable targets, because all three share one `overrideScopeProven` boolean and AFL Tables publishes no 2026 Brownlow votes until the count (`projectable_round_vote_rows = 0` measured offline). Not a Brownlow skip — leg C positively proves SOURCE absence, and G4-D carries the vote-bearing verification forward. G0 PASSED (READY, 0 blockers, 095 free — re-confirmed at Stage 1 start). G1 PASSED (254 passed, 4 pre-existing POSIX skips, 0 failures; both deploy orders). G3 PASSED A–E after the first G3-B run failed closed on a composite-key decode defect — `match_coaches` override keys decode on the LAST `|` because `matches.match_key` is itself pipe-delimited — fixed, single-sourced for refusal and write, regression coverage updated, `afldb_test` restored to 386 sourced coaches; do not re-run. G4 Steps 1–2 PASSED read-only on `afldb_dev`: 095 is the only pending migration, 386 coach rows with 0 namespace/identity violations, both CHECK names present, nothing mutated. G2 pre-check PASSED on `afldb_test`: `bad_path=0`, `bad_name_key=0`, `total=386` — **the plan's `total=383` was wrong**, a mis-transcription of `afltables-contract.json`'s count of accepted-baseline fitzRoy `Coach` *strings*; that file pins `measured.coaches = 386` and the tracked `coaches-20260905` snapshot holds 386 pages, so 386 is the correct expectation and both gating invariants were 0. G2 PASSED (migration 095 applied to `afldb_test`). Remaining: **G4 the HARD GATE — a real DEV settle run proving the three unrepresentable targets still APPLY, not merely propose**; G5 the 086 source contract on the new replay branches; G6 `db-promotion-check` incl. `assertContractCoherent()` vs the DEV `historicalOnly` declaration; G7 `coach-reconciliation`; G8 `tsc`. **Stage 2 (the `/admin/coaches` surface) must not begin until G4 passes**; a propose-only result is S-1, a stop, not expected drift.
