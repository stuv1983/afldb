...
| `AFLDB-ISSUE-118` | Medium | Grid Solver / External datasets / Data acquisition | **REOPENED 2026-09-05; Stage AA3 and Stage H2 (heights) done the same day.** Heights: migration `086_player_height_evidence.sql` + `tools/migration/enrich_heights.py` map the AFL Tables `player_details` register to canonical players through the snapshot's own per-match profile URLs (16,713 / 16,731 rows, 18 fail closed, 0 conflicts) and fill only NULL heights with evidence — **12,487 on `afldb_test`, 11,740 on `afldb_dev`**, idempotent. Gridley coverage complete (941 bridged answer-key players, 0 missing); `grid-solver` 189/189. Still open: 83 bridged players whose **Gridley height differs from AFL Tables** (Wikipedia agrees with AFL Tables where probed) keep the corpus regression red as `incorrect known answer`; the other gaps (draft / marquee / captaincy) and the 26 unsupported criteria remain, now classified by family in runbook §23.17. Key files: `issues/open/AFLDB-ISSUE-118.md` §23.16–§23.18, `tools/migration/enrich_heights.py`, `src/db/migrations/086_player_height_evidence.sql`, `tests/integration/gridley-height-oracle.test.ts`, `tests/integration/gridley-oracle-bridge.ts`. | §23.18: acquire AFL API listed heights as a second evidence source and decide precedence; admit the height stage to `db:test:rebuild`; then named medals + the six missing captaincy clubs (transcription only). Production gets 086 + `enrich_heights.py` with the next deploy (ISSUE-137 sequencing). |
