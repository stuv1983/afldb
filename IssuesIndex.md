...
**Last updated:** 2026-09-13 (`AFLDB-ISSUE-166` **OPENED, ROOT CAUSE PROVEN, FIX IMPLEMENTED AND
VALIDATED — Low severity; UNCOMMITTED, UNDEPLOYED.** Found during `AFLDB-ISSUE-165` Stage 8
rendered acceptance, but **not** caused by it. **This supersedes the initial High-severity
triage in full.** Withdrawn: that `requireCapability()` was bypassed; that full privileged page
content was returned; that `/admin/admins` leaked other administrators' session IP/device data;
and that a per-worker/cache artifact was responsible. None of those were true. The real defect:
`src/app/admin/loading.tsx` put a Suspense boundary above every guarded admin page, so the 200
shell committed before any page-level `redirect()` ran, and Next 16 encoded the redirect as a
`__next-page-redirect` meta-refresh inside the already-committed 200 response. A browser still
denied (one second later); `fetch()`, curl, a crawler or a monitor recorded a denied admin route
as a success. Denied `.admin-main` contained only "Loading…", with zero
forms/inputs/selects/textareas/tables/rows/buttons and one Flight chunk against three on a
permitted `/admin/awards`. Authorization held throughout: every privileged page/query path guards
before privileged data access, Route Handlers return real HTTP redirects, and Server Actions
remain independently capability-guarded. Fix: delete `src/app/admin/loading.tsx`, use
`useLinkStatus()` in `AdminNav` for navigation feedback, and add structural regression tests that
prevent restoring a route-level loading boundary or a layout `<Suspense>` above the guards. Open
issue count 2 -> 3. **Stays OPEN until deployed to DEV and Probe 1/2 confirm real HTTP redirects
on denied admin pages** — no harness can assert the streamed HTTP status without a running
server, so that probe is mandatory acceptance evidence. Full record: the `AFLDB-ISSUE-166` entry
in `issues.md`.)
