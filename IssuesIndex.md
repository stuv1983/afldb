...
- **Next action:** operator reviews and commits Stage 1, then a fresh session runs Stage 2 (`/admin/draft` routes + wizard, `data.draft.read`/`data.draft.edit` capabilities and nav, D-9 shared revalidate/submit extraction, gate 16 Playwright when DEV is updated). **Capabilities are deliberately NOT declared in Stage 1**: `tests/auth.test.ts` fails on a capability declared but enforced at no page, route or action, and Stage 1 ships no route -- so gate 4 is Stage 2. Still open: gate 2 PROD read-only probes (d)/(e)/(g)/(h) -- not run, PROD deliberately untouched this session -- and gate 9's real-importer half, which SKIPS in this worktree (no `.venv`, no accepted DraftGuru Stage A snapshot, no `AFLDB_TEST_IMPORT_DATABASE_URL`); both suites are written and run on a host that has them. Operator decision **S-1** (§11.1, sequencing the `promotion-inventory.ts` change against the paused ISSUE-151 PROD promotion `20260907-234124`) is required before the promotion-tooling change DEPLOYS, not before the Stage 1 commit.
