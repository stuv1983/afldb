...
after the ranking mention). Both fixed; `PARSER_VERSION` 61 → 62. Local `tests/nl-parser.test.ts`
536/536, broader gates 402/402, `typecheck` clean, all uncommitted in worktree
`sonnet/issue-215-career-numeric-binding-phrasing`. **Next action:** operator host validation on
streamanator (frozen V5 + exploratory V2, then a v61-vs-v62 plan diff) — see `AFLDB-ISSUE-215.md` §10
for the exact commands. See `issues.md` for the full record.
