...
**Open issues:** 14 tracked here — `AFLDB-ISSUE-117`, `AFLDB-ISSUE-137`, `AFLDB-ISSUE-138`, `AFLDB-ISSUE-139`, `AFLDB-ISSUE-140`, `AFLDB-ISSUE-142`, `AFLDB-ISSUE-144`, `AFLDB-ISSUE-147`, `AFLDB-ISSUE-148`, `AFLDB-ISSUE-149`, `AFLDB-ISSUE-150`, `AFLDB-ISSUE-151`, `AFLDB-ISSUE-152`, `AFLDB-ISSUE-153`.
...
| `AFLDB-ISSUE-152` | Medium | Natural-language search / semantic coverage / `src/search/nl/*` | **OPEN — STAGE 0 COMPLETE; OPERATOR DECISIONS FINAL 2026-09-08; PHASE B TECHNICALLY COMPLETE 2026-09-08 (committed); PHASE C IMPLEMENTED 2026-09-08 — every focused gate green, M7 closed, NOT COMMITTED and NOT deployed; PHASES D/E/F NOT STARTED.** Branch `opus/issue-152-nl-record-expansion` (worktree `D:\dev\afldb-issue-152`), from merged `main` @ `c2761e6`; `PARSER_VERSION` **36** (Phase B 34→35, Phase C 35→36, one bump each). **PHASE B BUILT:** `coach_record` grain + `NlCoachRef` + `coachQualifier`, coach directory (386 rows, colliding surnames excluded), `src/db/queries/nl/coach-record.ts`, `CoachRecordTable`, `coachProfilePath`, and the **deletion of the false coaching decline — F2 CLOSED**. Coverage is a **floor at 1902 only** (operator §13.17b): 1899 refuses, a future season is an honest empty result, and no completeness claim is made. Validation (all executed 2026-09-08): `tsc` clean; focused Phase B gate **548/548 PASS** across 7 files; migration naming/collision/checksum contract **36/36 PASS**; the F5 telemetry-grain contract test **PASS** with 092 applied to `afldb_test`; `tests/integration/nl-answers-coaching.test.ts` **23/23 PASS**; new corpora 99 plan / 25 decline, all executed through the real coach directory; the 1,435 / 60 gates untouched. **Four `tests/integration/database.test.ts` dataset-count assertions FAIL as external, pre-existing `afldb_test` baseline drift — OUT OF ISSUE-152 SCOPE, not investigated, not edited:** player-match rows 685,471 expected / 694,445 actual; 200-249 games with 16+ finals 114 / 119; 50-199 goals and no Brownlow votes 261 / 268; 200+ games, 100+ goals, 15+ finals 219 / 223 — the first pinned by its own comment to the `full-history-20260827` snapshot, the rest counting the same population. None touches NL search, telemetry or migration 092. A separate dataset-baseline investigation owns them. Deviations recorded in runbook **§14.5**. **FINDING F5 — CLOSED 2026-09-08.** Fixed by migration `092_nl_search_log_coach_record_grain.sql`, authorised by the operator as Phase B work, applied to `afldb_test`, and **verified against the database** by the contract test in `tests/integration/database.test.ts`, which PASSES. `nl_search_log`'s `grain` CHECK (migration 079) listed only eight grains, so a `coach_record` telemetry row was rejected and `logNlSearch` — which deliberately swallows INSERT failures so telemetry cannot break a search — dropped it in silence: coaching answers rendered correctly and only the search log lost them. **The Stage-0 "no migration" assumption (§13.16) is superseded by implementation evidence — the ninth supported NL grain cannot be admitted without extending the telemetry schema contract.** Migration 092 is forward-only and strictly widening: every grain 079 accepted is retained verbatim, the CHECK is neither weakened nor dropped, and `logNlSearch` is unchanged (no separate telemetry defect was found — the swallow is deliberate and documented). `tests/integration/database.test.ts` now drives the accepted list from `Record<NlGrain, true>`, names `coach_record` explicitly, and additionally asserts that a random unsupported grain is still REJECTED, so the constraint cannot be widened into a formality. NOT DONE: applying migration 092 to `afldb_dev` and production (deploy-time, migration BEFORE code), rendered/browser run, `npm run build`, deploy. Five public record boards are unrepresented in the typed NL layer — `/records/coaches`, `/records/after-the-siren`, `/records/family`, `/records/father-son`, `/records/first-kick-goal` — and only first-kick-goal is substantially supported (list, club scope, season/decade scope, six `achievement_summary` kinds). The eight classic `RECORD_CATEGORIES` are already covered; no other new record family exists. **F1:** `/records/father-son` reads `player_relationships WHERE relationship='parent_child'` and never `father_son_selections`, while the Grid Solver's `father_son_selection` builder reads that draft-rule table — two meanings of one phrase; and `/records/family` groups every relationship type by `family_key` while its prose says siblings. Public-UI truthfulness. **Operator 2026-09-08: OUT OF ISSUE-152 SCOPE, tracked as `AFLDB-ISSUE-153`** and recorded here as an external dependency only — it does **not** block Phase B, C or E; only wording whose meaning depends on it (C1, FS1–FS3, FS6, decisions D6/D8) is deferred. **F2:** `UNANSWERABLE_TOPICS` (`src/search/nl/vocab.ts:882`) declines every coaching question with the reason "AFLDB has no coaching data at all" — untrue since migration 087, contradicted by five public surfaces, and it fires before entity extraction so it swallows future support. **F3:** `coached_by`, `premiership_coach`, `after_siren_winner`, `father_son_selection`, `father_son_father`, `has_brother` are implemented and grid-tested but unreachable from NL (the parser emits 8 of 179 `GRID_BUILDERS`) — much of the work is wiring, not SQL; `src/db/queries/coaches.ts` already holds the whole coaching answer surface, `organization_id`-lineage-correct on `(W + D/2)/G`, derived from `match_coaches ⋈ matches` and never from `coaches.source_games_coached`. Proposed: grains `coach_record` (with an `NlCoachRef` distinct from `NlPlayerRef` — coach-only people have no player row and names collide) and `after_siren` (keeping `kick_scored`/`kick_effect`/`kicker_result`/`siren` distinct), `family` provisional, 4 small new builders, 8 existing builders wired. Both rendered corpora (1,435 realistic / 60 decline) are untouched and contain no coaching/siren/family/father-son question, so the expansion is purely additive. **EVIDENCE GATE CLEARED 2026-09-08:** `ISSUE-152-nl-evidence.sql` was executed by the operator against `afldb_test` over the `55432` tunnel — read-only, `ROLLBACK` reached, no errors, production untouched → `ISSUE-152-nl-evidence-output.txt` (41/41 sections). Two evidence-pack-only SQL fixes were needed first (§7.4 `a.id` qualification, §2.8 `id AS event_id` restored); no product file touched. Measured results are in runbook **§5.3** and resolve **D1–D5, D7, D9**; **D6/D8** stay deferred to `AFLDB-ISSUE-153`. Headline measurements: coaches **386** (368 player-linked / **18 coach-only** — the exact D3 seam; the earlier "383" is corrected), `match_coaches` 32,034 spanning **1902–2025** only, Richmond org has 42 coaches with exhaustive threshold counts (≥100 wins → 3; ≥200 games → 3); `after_siren_kicks` 126 with a **metric ceiling of 2** (every superlative is a tie — Barry Hall and Gary Rohan), **no VFL/AFL Grand Final after-siren event** and a non-premiership Escort Championships row labelled `GF` that a naive round filter would return; `player_relationships` holds **only** `sibling` (498) and `parent_child` (127) — the other six enum members have **zero rows**, so D7 is an empty-set call, and "brothers" must stay label-backed (474 of 498; 8 `sisters` rows exist). **F1 is LATENT:** `father_son_selections` and `parent_child` are the same 127 rows with the same 96/31 split, and `parent_child` rows have no `family_key`, so neither page currently serves a wrong set — urgency down, scope unchanged. **New F4:** `afldb_test` has **zero** first-kick-goal rows (loaded from a gitignored extract via `npm run records:first-kick-goal`), so Phase E has no DB-backed witnesses — a test-fixture gap, not a product defect. Six measurement gaps M1–M6 named. **OPERATOR DECISIONS FINAL 2026-09-08:** **D1–D5, D7, D9 APPROVED**; **D6/D8 DEFERRED** to `AFLDB-ISSUE-153`; **F1 external-only** under ISSUE-153; **D10 PARTIALLY APPROVED** — a match-unlinked after-siren row may answer anything derivable from `after_siren_kicks` itself (list, count, player/club/opponent totals, goal/behind/none, won/drew/none, `kicker_result`, season, premiership flag) and `match_id` must **not** be required merely because the event is premiership-season, **but** it may not decide "most recent"/"first" until a deterministic chronology independent of canonical `matches` is proven (`season` + `round_raw` is not automatically a total ordering), and anything needing canonical match properties (finals/`round_type`, match date, venue, match identity) **must** require `match_id` — the ownership boundary is documented in runbook **§7.1**; **D11 ANSWERED — DB-backed verification REQUIRED**, parser/plan-only verification is refused for Phase E, the first-kick-goal family must be loaded into `afldb_test` via the existing supported loader first, **the loader is not to be run without explicit operator authorisation**, and Phase E stays sequenced after B and C. **PHASE B PLAN RECORDED — runbook §13** (typed `coach_record` grain + `NlCoachRef`, coach directory with unique-surname aliases only — Albert/Charlie Pannam and Len/Norm Smith prove why, `coachCuePresent` metric gating mirroring `clubSeasonCuePresent`, deletion of the false coaching decline in the same commit, reuse of `src/db/queries/coaches.ts` shapes, `organization_id` lineage, whole-career vs one-club semantics, the stated 50-game `(W+D/2)/G` qualifier, red-before-green tests, hand-written-SQL integration comparisons on exhaustive §1.4/§1.5/§7.2 witnesses, additive corpora, `PARSER_VERSION` 34→35 at one bump point, ten must-still-decline forms, and one open sub-decision on post-2025 seasons). **PHASE C IMPLEMENTED 2026-09-08 — runbook §16; the §15 plan as built, with three narrowing deviations recorded in §16.10.** `PARSER_VERSION` 36, migration **093** applied to `afldb_test`. **R0 (before any edit): all eight probes DECLINED — no live false-answer defect, so D19 did not fire**, though the metric extractor DID claim `goals`/`kicks` and was masked only by the unresolved-token penalty, which is why the step-5d precedence rule is asserted directly. **M7 CLOSED (D16): `club_unresolved = 0`, `opponent_unresolved = 0`, `events = 126`**, so §15.6's exclusion caveat was dead code and was not implemented; the 6-of-126 no-player-link and 10-of-126 no-match-link exclusions ARE wired and computed at answer time. **46 assertions observed RED across five suites before implementation**; R1 proved 093 was required by making `afldb_test` reject `after_siren` on `nl_search_log_grain_check`. Validation: `tsc` clean; DB-free NL **490/490 PASS**; new `tests/integration/nl-answers-after-siren.test.ts` **20/20 PASS** against hand-written SQL; six other NL integration suites **86 PASS / 3 skipped** (Phase B coaching still green); telemetry-grain contract PASSES with 093. Every §15.16 witness re-measured and matched: Hall + Rohan tied on 2, nine tied on 2 kicks, 3+ goals empty, 4 goals against the Richmond lineage including Bill Wood 1946 Footscray, Grand Final empty and never the `round_raw='GF'` Escort Championships row, 8 finals events, first = Billy Schmidt 1913 / match 1313, most recent = Nasiah Wanganeen-Milera 2025 / match 16792 and never an unlinked row, and R7 parity with `getAfterSirenRecords` for every player. Corpora **93 plan / 27 decline**, all executed through the real directory; 1,435/60 gates and both coaching corpora untouched and asserted siren-free. **NOT COMMITTED, not rendered, not built, not deployed.** The plan it was built from (one new `after_siren` event grain over `after_siren_kicks`; the three dimensions `kick_scored`/`kick_effect`/`kicker_result` kept independent so "goal after the siren" (71) and "goal after the siren to win" (62) are provably different answers and the existing `after_siren_winner` builder — which means a winning kick and admits a behind — is deliberately NOT wired; a single `siren_kicks` metric with every distinction as a typed dimension, deliberately **not** named `kicks` because `NL_COVERAGE.kicks` would impose the player-stat 1965 floor and decline 1913–1964; `event`/`player` subjects with two payload variants; D10 §7.1 encoded as one exported `afterSirenRequiresMatchLink()` so unlinked rows answer list/count/player/club/dimension/season questions and only finals-scope and first/most-recent require `match_id`; `round_raw` never read as a filter (Kerry Good 1980 `'GF'`, Escort Championships); Grand Final returns an honest empty result; every superlative returns a tie set (ceiling 2); `min` and `scope.playerIdIn` refused; 15 named decline forms; R0–R7 red-before-green including a probe of whether the CURRENT parser already false-answers "most goals after the siren" as a career-goals ranking; **migration 093 expected required as the F5 analogue**, proven by adding `after_siren` to `SUPPORTED_NL_GRAINS` and watching `database.test.ts` fail first — 093 verified free on every branch; `PARSER_VERSION` 35→36 at one bump point; two additive corpora) is at runbook §15; **D12–D19 are all ANSWERED** — D12 approved, D13/D14/D18 confirmed, D15 approved, D16/D17 authorised, D19 recorded (condition did not fire). Key files: `issues/open/AFLDB-ISSUE-152.md` (runbook, 49-subfamily matrix, §5.3 measured evidence, decisions D1–D11, **§7.1** D10 boundary, **§13** Phase B plan, **§15** Phase C plan), `ISSUE-152-nl-evidence.sql`, `ISSUE-152-nl-evidence-output.txt`. | **Operator:** Phase C is implemented and validated but **UNCOMMITTED** — review the working tree on `opus/issue-152-nl-record-expansion` and commit it. Then decide whether the four new corpora (2 coaching + 2 after-siren) are rendered through the UI (dev server + browser sweep) and whether `npm run build` / `nl:stress` run, and whether Phase D or E is authorised next. At deploy time migrations **092 AND 093 must both reach `afldb_dev` and production BEFORE the code** — 093 exists precisely because a missing grain in `nl_search_log`'s CHECK drops telemetry silently while the answer renders correctly. Phase E still cannot begin until the operator authorises `npm run records:first-kick-goal -- --apply` against `afldb_test` (state-changing, operator-only). The four failing `tests/integration/database.test.ts` dataset counts are external `afldb_test` baseline drift and need a separate investigation, not this issue. D6/D8 and F1 remain with `AFLDB-ISSUE-153`. |
