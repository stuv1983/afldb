...
`AFLDB-ISSUE-112`, `AFLDB-ISSUE-113`, `AFLDB-ISSUE-116`, `AFLDB-ISSUE-117`.
...
> here and no row is written for it — inventing one would be worse than omitting it. When the
> branches merge, ISSUE-110's own rows are authoritative: preserve them and correct this count
> then. Merge-sensitive; see `issues/open/AFLDB-ISSUE-102-HANDOFF.md`.
...
| `AFLDB-ISSUE-117` | Medium | Admin / Access management / Security | Revoked beta access keys cannot be removed from `/admin/access` — revoke sets `beta_access_codes.revoked_at` and the row then stays in the list forever. **Implemented; validation PASSED 51/51 on 2026-08-31 (0 failures).** Deletable = **retired** (revoked OR spent): a spent code deletes directly, no ceremonial revoke; a partly-used or unlimited code is still refused because it remains redeemable. Expiry deliberately excluded. Active -> Revoke -> Delete; the revoked-only rule is `revoked_at IS NOT NULL` inside the DELETE (`deleteRevokedAccessCode`), not a hidden button, and the `access.code_deleted` audit is written in the SAME `authSql.begin` as the delete so the row cannot outlive its trail. Checked before adding the DELETE: no FK references the table, and the beta cookie's `code:<id>` subject is never looked up, so deletion ends no session revocation would not. Key files: `src/db/queries/access-codes.ts` (new), `src/app/admin/access/actions.ts`, `AccessManager.tsx`, `src/lib/auth/session.ts` (`audit` takes an optional tx), `src/db/migrations/079_access_code_delete.sql` (new), `tools/maintenance/privileges.sql`. **Next action: redeploy the widening to dev.** Dev already has migration 079 + `privileges.sql` and runs the PRE-widening code, so the spent-code behaviour cannot be confirmed there yet. Commit/push, redeploy `claude/issue-116` (`-AllowDirtyServer` required), confirm all four states by hand, then restore dev to `dev`@`0c2100a`. **Deploy order is load-bearing:** migration 079 and `privileges.sql` BEFORE the code, or every delete fails closed on a permission error. Runbook: `issues/open/AFLDB-ISSUE-117.md`. Known gap, deliberate: a spent/expired code has `revoked_at IS NULL` and the UI offers Revoke only when `live`, so it remains undeletable. |
