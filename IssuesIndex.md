...
| `AFLDB-ISSUE-152` | Medium | Natural-language search / semantic coverage / `src/search/nl/*` | **OPEN — PHASES B AND C COMMITTED; PHASE E TECHNICALLY COMPLETE 2026-09-09, VALIDATED, UNCOMMITTED; PHASES D AND F NOT STARTED.** Branch `opus/issue-152-nl-record-expansion` (worktree `D:\dev\afldb-issue-152`) from `main` @ `c2761e6`. `PARSER_VERSION` **37** (B 34->35, C 35->36, E 36->37). Built: **B** `coach_record` grain + 386-coach directory, false coaching decline deleted (**F2 CLOSED**), migration **092**; **C** `after_siren` event grain over 126 curated events with three independent dimensions and a match-link boundary, migration **093** (**M7 CLOSED**); **E** first-kick-goal E7/E8 with **no grain, no builder, no SQL and no migration** — `first_kick_goal_consecutive_min` and `first_kick_goal_only_career_goal`, two Grid Solver builders the parser could never reach. E8 was a **misread**, not a decline: "only career goal" was left in the text and read as the ranking subject; the cue now owns its own negation and is consumed in step 5a. **D11 SATISFIED / F4 CLOSED:** loader authorised (**E-D2**) and applied to `afldb_test` from `D:\dev\afldb\data\records\first-kick-goal.csv` (main checkout; gitignored, not committed) — batch **230: 334 updated / 0 inserted / 0 deleted**. M-E1: **334 total / 330 linked / 44 multi-kick / max consecutive 6 / 23 only-career-goal / 4 no-further-kicks / 1911-2026**, pinned as a fixture contract. **Three deliberate fail-closed deviations (§18.5):** E9 (kick-level "never kicked the ball again", checked before the E8 cue), E-DEC-4/5 (modifier + `achievement_summary`, which carries no `careerPredicates`), E-DEC-8 (N outside 1..10) each **consume the span and return a named refusal** rather than leaving a leftover token — the leftovers are "goal" and "kick", both `METRIC_WORDS`, and Phase C's R0 proved only the 0.35 unresolved-token penalty was suppressing the wrong answer. **Runbook correction:** the export is **`getFirstKickGoalList`**, not `listFirstKickGoals`. **Validation 2026-09-09:** non-DB gate **485/485 PASS** (parser 253, plan 136, describe 49, audit 10, ui-corpus 37), `tsc --noEmit` clean; red-before-green vs `47a645f` **21 failed / 427 passed** (plan stayed green — Phase E adds no validation rule); **`nl-answers-first-kick-goal.test.ts` 20/20 PASS in 1.499s** vs hand-written SQL and the linked-filtered records board. **Separately recorded:** `first-kick-goal-reload-links.test.ts` **11 Windows timeouts = established Windows runtime pathology, NOT a Phase E regression** (authoritative Linux **16/16 PASS in 53.94s**); no timeout raised, no importer/reload change. Corpora additive: **20 realistic + 6 decline**; **1,435 / 60** gates and the coaching/after-siren corpora unchanged. Four `tests/integration/database.test.ts` dataset counts still fail as external `afldb_test` baseline drift — out of scope, untouched. D6/D8 and F1 sit with `AFLDB-ISSUE-153`. Key files: `issues/open/AFLDB-ISSUE-152.md` (§5.1 matrix, §5.3 evidence, D1-D11, §7.1, §14 B, §16 C, **§18 E**), `src/search/nl/{vocab,parser,plan,describe}.ts`, `ISSUE-152-nl-evidence-output.txt`. | **Operator:** commit the Phase E working tree (B and C are already committed at `e8f5f67` / `47a645f`). Then decide whether the six new corpora are rendered through the UI and whether `npm run build` / `nl:stress` run. **Deploy ordering is unchanged: migrations 092 AND 093 must both reach `afldb_dev` and production BEFORE the code.** Then Phase D: its F1-dependent half (C1, FS1-FS3, FS6, decisions D6/D8) is **blocked on `AFLDB-ISSUE-153`**, but **C2/C3/C4/FS4 do not depend on F1 and may proceed now**. **Phase F is gated on B-D being green** (§9) and cannot start ahead of D. No action needed on the Windows reload timeouts; re-prove that suite on Linux if wanted. |
