...
| `AFLDB-ISSUE-118` | Medium | Grid Solver / External datasets / Data acquisition | **REOPENED 2026-09-05; Stages H2–H4 (heights), Family A (medals) and Family B (captaincies) done the same day** (runbook §23.16–§23.22). Heights: AFL Tables register canonical (12,487 on `afldb_test`), the AFL API season rosters (`acquire_rosters.R`, 2012–2026, 1,824 players) and a tracked Wikipedia adjudication set (83 players) loaded as corroborating evidence; precedence decided on evidence (§23.19 H3.6), no canonical height changed; the corpus classifies height cells from the evidence: `incorrect known answer` 299 → 0, `external source disagreement` 197 cells / 6 players (informational), **`source conflict` 102 cells / 3 players still open**. Rebuild: three manifest-pinned height stages + five final gates in `db:test:rebuild`; heights reproduced by the runner (batches 7/8), remaining stages hand-run after an in-flight edit broke the awards stage, final validation 53/53 — a clean unattended full rebuild is the next action. Family A: 7 medals, 328 Wikipedia-cited winner rows, all linked on `afldb_test`; `unsupported` 375 → 309 (19 criteria). Family B: 399 captaincy rows for Geelong / Hawthorn / West Coast / Fitzroy / Brisbane Bears / University, 1,774 / 1,774 linked on `afldb_test`. | §23.22: (1) unattended `db:test:rebuild` of the 16-stage graph; (2) record the post-Family-B corpus counts; (3) operator decision on the 3 height source-conflict players; (4) DOB stage, coaches, siblings per §23.17; (5) production with the next deploy (ISSUE-137 sequencing). |
