...
<!-- UPDATE 2026-09-04 (ISSUE-134 security-boundary repair): the `404`-on-every-request defect
     found by the first DEV acceptance is FIXED on `claude/issue-134`. Gate 1 no longer requires
     the forwarding headers to be absent - a premise Next 16 falsifies, since
     `base-server.js:606-612` synthesises both on every request - and instead requires
     `x-forwarded-for` to RESOLVE TO LOOPBACK. That is sound on the tracked deployment contract:
     both Caddyfiles overwrite the header with `{remote_host}` (never append) on every proxy
     block, and `deploy/afldb.service` binds the app to `127.0.0.1`, the same property
     `src/lib/auth/session.ts` already stakes the audit trail on. Chains, non-loopback addresses
     and malformed values fail closed; `x-forwarded-host` is no longer a gate (it is client
     input on both paths). The secret, the timing-safe comparison, the failure-only limiter, the
     integer-only body and the server-composed path are all unchanged. Tests now use the request
     shape Next really delivers, and the Caddy/systemd/`??=` contract is asserted statically so
     an overwrite -> append change cannot silently invalidate the model. All repository gates
     green again. The issue stays **Open** pending the second DEV acceptance; NOT merged, NOT
     deployed. See `issues/open/AFLDB-ISSUE-134.md` §11. -->
...
| `AFLDB-ISSUE-134` | Low | Deployment / Operations / Frontend rendering (ISR) / Current-season settle | **IMPLEMENTED AND REPAIRED; SECOND DEV ACCEPTANCE PENDING — stays Open. Do not merge, do not deploy to PROD.** Branch `claude/issue-134` (worktree `D:\dev\afldb-issue-134`), pushed to `origin`; base `25c976d`; no migration. The settle posts its committed season to a loopback route (`/api/internal/revalidate-season`) which calls `revalidatePath()`, reposting on fresh connections until every worker ordinal answers, because Next 16 keeps page invalidation in per-process memory and `deploy/server-cluster.mjs` runs 2-4 workers. **First DEV acceptance (`streamanator`, 2026-09-04) found the route unconditionally `404`** — its gate required `x-forwarded-for`/`x-forwarded-host` to be ABSENT, but Next 16 synthesises both on every request (`node_modules/next/dist/server/base-server.js:606-612`), so the gate was always true; the unit tests missed it by calling `POST()` with synthetic `Request` objects that never pass through `base-server.js`. **Repaired (§11):** the gate now requires `x-forwarded-for` to RESOLVE TO LOOPBACK (`127.0.0.0/8`, `::1`, `::ffff:127.0.0.1`), refusing chains, non-loopback addresses and malformed values; `x-forwarded-host` is not a gate. Soundness rests on the tracked contract, not on the framework: both Caddyfiles set `header_up X-Forwarded-For {remote_host}` (overwrite, never append) on every `reverse_proxy` block and drop `X-Real-IP`/`Forwarded`, and `deploy/afldb.service` pins `HOSTNAME=127.0.0.1` so the app is loopback-bound — the same property `src/lib/auth/session.ts` already relies on. Secret, timing-safe comparison, failure-only limiter, integer-only body, server-composed path and 503-when-unconfigured all unchanged. Tests now send the shape Next really delivers, and a static suite asserts the Caddy/systemd/`??=` contract so an append/trust change cannot silently invalidate the model. Gates ALL GREEN: `tests/settle-season-revalidation.test.ts` 69/69, focused 287/287, integration `tests/integration/settle-afltables.test.ts` 64 passed / 1 pre-existing skip in 246.7 s against `afldb_test`, `tsc --noEmit` clean, ESLint clean. What already passed on the host: 4 workers each carrying a server-assigned `AFLDB_WORKER_ID` 1-4 plus `AFLDB_WORKER_COUNT=4`. DEV is currently on `main` @ `169d738` (build `p0Z_llbWECQxDN2Eo2YMC`, health 200, 4 workers, revalidate env names absent, settle timer `not-found`). PROD untouched; settle timer never installed. Key files: `src/lib/acquisition/season-revalidation.ts`, `src/app/api/internal/revalidate-season/route.ts`, `tools/current-season/settle-afltables.ts`, `deploy/server-cluster.mjs`, `src/middleware.ts`, `docs/deployment.md` §7c. **Exact next action:** redeploy the pushed branch to DEV by the reversible §10.1 procedure and re-run the acceptance sequence blocked at §10.2 — correct-secret loopback POST succeeds, wrong/missing secret and non-loopback identity fail, 4 distinct worker ordinals reached on fresh connections, `/seasons/2026` regenerated inside its ISR hour — then restore DEV. |
