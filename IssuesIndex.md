...
- **Next action:** **operator runs the Stage 1 validation, then reviews and commits.** In order: `npm run preflight -- --mode implementation --issue 162`; apply migration 097 to **`afldb_test` only**; `npx tsc --noEmit`; `npx vitest run tests/admin-fixture-actions.test.ts tests/data-overrides-source-contract.test.ts tests/db-promotion-check.test.ts tests/current-season-import.test.ts`; `npx vitest run tests/integration/admin-fixtures.test.ts` (**rerun first, expecting 36/36**; needs the worktree `.env` with `AFLDB_TEST_DATABASE_URL` for `afldb_test` through the local 55432 tunnel, §37.9, plus `.venv` with `psycopg` for the real replay); ISSUE-160/161 regressions; eslint over the changed files; `git diff --check`. **`npm run db:privileges` is required on any database that runs the app against `fixtures`** (app read is fail-closed since 039) — deploy order is binding: **migration 097 → `db:privileges` → code**. Stage 2 (the `/admin/fixtures` surface, capabilities, nav, batch UI, diagnostics panel) is a separate session. Do not merge ISSUE-160/161 first, do not update DEV, do not deploy, do not touch PROD. ISSUE-162 is **not resolved**.
