...
<!-- UPDATE 2026-09-04 (ISSUE-134 DEV acceptance): `AFLDB-ISSUE-134` is implemented,
     committed (`7c66ae7`) and pushed on `claude/issue-134`, with every repository gate green,
     but DEV acceptance on `streamanator` HALTED on a defect: the new loopback route answers
     `404` to every request, including correctly-authenticated ones, because its
     `x-forwarded-*` proxy gate rests on a premise Next 16 falsifies
     (`base-server.js:606-612` synthesises both headers on every request). DEV was restored to
     `main` @ `169d738` and PROD was not touched. The issue stays **Open**; it is NOT merged and
     must NOT be deployed. Evidence and the recommended fix are in
     `issues/open/AFLDB-ISSUE-134.md` §10. -->
...
| `AFLDB-ISSUE-134` | Low | Deployment / Operations / Frontend rendering (ISR) / Current-season settle | **IMPLEMENTED AND COMMITTED, BUT DEV ACCEPTANCE HALTED ON A DEFECT — stays Open. Do not merge, do not deploy.** Branch `claude/issue-134` (worktree `D:\dev\afldb-issue-134`), commit `7c66ae7`, pushed to `origin`; base `25c976d`; no migration. The settle now posts its committed season to a loopback route (`/api/internal/revalidate-season`) which calls `revalidatePath()`, reposting on fresh connections until every worker ordinal answers, because Next 16 keeps page invalidation in per-process memory and `deploy/server-cluster.mjs` runs 2-4 workers. Repository gates ALL GREEN: unit 49/49, focused 287/287, `tsc --noEmit` clean, ESLint clean, integration `tests/integration/settle-afltables.test.ts` 64 passed / 1 pre-existing skip (restricted importer role, needs `AFLDB_TEST_IMPORT_DATABASE_URL`) in 240.85 s against `afldb_test`. **DEV acceptance on `streamanator` 2026-09-04 found the route is unconditionally `404`, so the feature is inert on a real host.** Its first gate rejects any request carrying `x-forwarded-for`/`x-forwarded-host` on the premise that only Caddy adds them; Next 16 synthesises BOTH on every request in `node_modules/next/dist/server/base-server.js:606-612`, so the gate is always true. The unit tests missed it because they call `POST()` with synthetic `Request` objects that never pass through `base-server.js`. What DID pass on the host: 4 workers each carrying a server-assigned `AFLDB_WORKER_ID` 1-4 plus `AFLDB_WORKER_COUNT=4` from the cluster primary. DEV was restored to `main` @ `169d738` (rebuilt `p0Z_llbWECQxDN2Eo2YMC`, MainPID 1057872, health 200, 4 workers, both revalidate env names removed, settle timer still `not-found`). PROD untouched; settle timer never installed. Key files: `src/lib/acquisition/season-revalidation.ts`, `src/app/api/internal/revalidate-season/route.ts`, `tools/current-season/settle-afltables.ts`, `deploy/server-cluster.mjs`, `src/middleware.ts`, `docs/deployment.md` §7c. **Exact next action:** operator decides on `issues/open/AFLDB-ISSUE-134.md` §10.4 — invert gate 1 to require `x-forwarded-for` to BE a loopback address (sound because `deploy/Caddyfile.production:39` overwrites rather than appends, the property `src/lib/auth/session.ts` already relies on); replace the test that passed for the wrong reason; correct the false premise in `route.ts` and `docs/deployment.md` §7c; re-run the §5 gates; then re-run DEV acceptance from §10.1. |
