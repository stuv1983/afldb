...
`AFLDB-ISSUE-125`, `AFLDB-ISSUE-126`, `AFLDB-ISSUE-127`, `AFLDB-ISSUE-130`.
...
<!-- ALLOCATION WARNING 2026-09-01: `AFLDB-ISSUE-118` is allocated and is NOT free.
     ...
     **Next free issue ID is still `AFLDB-ISSUE-130`.**
     ...
     awaiting dev-host validation before closure.
...
| `AFLDB-ISSUE-130` | Medium | Deployment / Operations / Data acquisition | The tracked `afldb-settle-afltables.service` depends on an **untracked host systemd drop-in** to find its R library. On the dev host the unit failed at step 1 with `Package 'jsonlite' is required` (`acquire_core.R:43`) while `jsonlite` and `fitzRoy` 1.8.0 were installed and healthy in `/home/arm/R/library`: the unit declares no environment (`systemctl show ... -p Environment` printed `Environment=`), systemd sources no login shell, and `~/.Renviron` sets nothing (`R_LIBS_USER` showed its computed default), so that directory is on no `.libPaths()`. A hand-written `r-library.conf` drop-in setting `R_LIBS_USER` made the supervised settle succeed end to end (209 matches / 9,614 player-match rows / 0 unkeyed / `SOURCE COMPLETENESS: COMPLETE`) — which is exactly the defect: a fresh deployment or production would fail identically **with the packages installed**, and nothing in the repository would have warned first. `deploy/afldb-settle-afltables.sh` resolves `Rscript`, `python3` and `node` explicitly but resolves the R **library** not at all; `docs/deployment.md:443-462` prescribes `/usr/local/lib/R/site-library` and then verifies it only by eyeball in an interactive shell, which is not the failing environment. R also drops non-existent `.libPaths()` entries silently, so setting a path is not self-validating. **Decision taken (2026-09-03):** `/usr/local/lib/R/site-library` is the canonical library on every deployed host; the dev host's `/home/arm/R/library` is a temporary deviation to reconcile; `AFLDB_R_LIBS` is an optional escape hatch, not part of the normal install. **Stage 2 IMPLEMENTED on `claude/issue-130`, awaiting host validation.** Built: `deploy/afldb-r-env.sh` (sourced; prepends optional `AFLDB_R_LIBS` to `R_LIBS`, additive; **refuses, exit 1, if the directory does not exist** because R drops a missing entry silently), `deploy/afldb-r-preflight.sh` (one `Rscript` run: `.libPaths()` + `R_LIBS*` printed, configured `AFLDB_R_LIBS` proved present on `.libPaths()`, `jsonlite`/`digest`/`fitzRoy` visible with resolving library, fitzRoy == `pinned_version` **read from** `fitzroy-contract.json`, `~/.Renviron` warning, installs nothing, exit 1 on any failure), settle script sources the fragment, `docs/deployment.md` §7b/§9 (canonical library; override; interactive-shell vs service-equivalent `systemd-run` check; drop-in removal), 18 new assertions in `tests/current-season-import.test.ts`. Unit file untouched. Validation: `sh -n` ×3, `git diff --check`, vitest 246/246, eslint clean, preflight executed for real against local R 4.6.1. Key files: `issues/open/AFLDB-ISSUE-130.md` §9, `deploy/afldb-r-env.sh`, `deploy/afldb-r-preflight.sh`, `deploy/afldb-settle-afltables.sh`, `docs/deployment.md`. | **Stage 3, dev host only, runbook §9.6 in order:** deploy the branch → reinstall `jsonlite`/`digest`/`fitzRoy` into `/usr/local/lib/R/site-library` per §7b → remove the `r-library.conf` drop-in + `daemon-reload` → `systemctl show afldb-settle-afltables.service -p Environment` must print `Environment=` → `sh deploy/afldb-r-preflight.sh` **and** the documented `systemd-run` service-equivalent run, both ending `R PREFLIGHT: OK` → one supervised `systemctl start` completing with `SOURCE COMPLETENESS: COMPLETE` with the drop-in absent → read-only check of where production's fitzRoy lives. Only then `CHANGELOG.md` (Unreleased) and close. Production deployment is separate work, queued behind `AFLDB-ISSUE-128`/`-129`. |
