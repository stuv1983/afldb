...
| `AFLDB-ISSUE-126` | Medium | Database / Admin / Security / Audit trail / Operations | The 2026-09-02 production canonical DB cutover replaced production-only application state along with the football data. The real super admin (`auth_users` id 1) was recovered from the pre-cutover backup and admin login verified, but three sets of production-only rows were **not** restored and exist only in the recovery database `afldb_prod_auth_recovery`: `auth_audit_log` **92 rows**, `beta_access_codes` **1 row**, `site_settings` **11 rows**. Production runs with 0 rows in all three. Nothing breaks — `src/lib/site-settings.ts` falls back to compiled-in defaults — but 11 deliberate super-admin choices are silently reverted, one beta access code is gone, and the admin audit trail has a hard discontinuity at the cutover. `auth_sessions` (17) were deliberately not restored and must stay unrestored. Key files/state: `afldb_prod_auth_recovery`, `/home/arm/afldb_prod_pre_rebuild_20260902-200355.dump`, `src/lib/site-settings.ts`. **UPDATE 2026-09-04: Stages 1–3 done read-only on branch `claude/issue-126`; runbook `issues/open/AFLDB-ISSUE-126.md`; no production write.** Re-measured: audit 92 → 11 (ids 90–181 vs 16–26, no overlap), settings 11 → 0, code 1 (spent) → 1 (new), sessions 17 (all expired) → 7. Also found via the ISSUE-125 contract: **`staging_aflw.*` 51,018 rows → 0 (production AFLW is empty)**, `site_media` 1 → 0, `beta_join_requests` 1 → 0, `data_edits` 2 (player id 4375 now a different person; Kelly Robinson is prod 8065), player links 6/2, telemetry 88/1/2. | **UPDATE 2026-09-04 20:37 AEST: EXECUTED on PROD.** Operator approved §7.5 (a); `auth_audit_log_id_seq` reset 182 → 26 (rows fingerprint-identical), then T1 (92 audit rows, ids 90–181, seq → 181), T2 (7 settings + 1 media), T5 (`staging_aflw` 51,018 rows, `aflw` views 11/710/960) and T6 (`database.recovered` marker id 182) committed one at a time, each verified read-only; final `auth_audit_log` 104 rows, seq 182; `/api/health` ok. Database-level acceptance all PASS (runbook §8). **Open only on the operator's browser acceptance (§8.1: super-admin login + admin/home/AFLW checks)**, then close-out. Recovery DB, T0 backup and host artefacts retained. Earlier today: operator approved T1/T2/T5/T6, retired T4; T0 GREEN (backup `afldb_prod-20260904-201037.dump` sha256 `e8cfa912…d639`, restore-proven 9/9, off-host copy verified); every rehearsal GREEN; PAUSED before any commit — the T1 rehearsal's `setval` survived its ROLLBACK (sequences are non-transactional) and left `auth_audit_log_id_seq` at 182 (rows untouched, 11 / ids 16–26). Operator to choose runbook §7.5 (a) `setval(…, 26, true)` [recommended] or (b) relax T1's precondition, then resume the commit path.** Original plan: T1 restore 92 audit rows with original ids; T2 restore 7 operator-override settings + 1 media row (4 default-equal keys retired); T3 retire the spent beta code and the 17 sessions (no DML); T4 restore the 1 join request (optional); T5 restore `staging_aflw.*` per table from the pre-cutover dump; T6 `database.recovered` marker last. `data_edits`/player-link decisions retired, corrections redone via the admin UI. Scripts `issues/open/AFLDB-ISSUE-126-*.{sh,sql}`, commit-gated. **`afldb_prod_auth_recovery` MUST NOT be dropped until this is resolved.** |
