...
<!-- ALLOCATION WARNING 2026-09-01: `AFLDB-ISSUE-118` is allocated and is NOT free.
     ...
     **Next free issue ID is still `AFLDB-ISSUE-130`.**
     ...
     on branch `claude/issue-130` in worktree `D:\dev\afldb-issue-130`; **nothing is implemented**.
...
| `AFLDB-ISSUE-130` | Medium | Deployment / Operations / Data acquisition | The tracked `afldb-settle-afltables.service` depends on an **untracked host systemd drop-in** to find its R library. On the dev host the unit failed at step 1 with `Package 'jsonlite' is required` (`acquire_core.R:43`) while `jsonlite` and `fitzRoy` 1.8.0 were installed and healthy in `/home/arm/R/library`: the unit declares no environment (`systemctl show ... -p Environment` printed `Environment=`), systemd sources no login shell, and `~/.Renviron` sets nothing (`R_LIBS_USER` showed its computed default), so that directory is on no `.libPaths()`. A hand-written `r-library.conf` drop-in setting `R_LIBS_USER` made the supervised settle succeed end to end (209 matches / 9,614 player-match rows / 0 unkeyed / `SOURCE COMPLETENESS: COMPLETE`) — which is exactly the defect: a fresh deployment or production would fail identically **with the packages installed**, and nothing in the repository would have warned first. `deploy/afldb-settle-afltables.sh` resolves `Rscript`, `python3` and `node` explicitly but resolves the R **library** not at all; `docs/deployment.md:443-462` prescribes `/usr/local/lib/R/site-library` and then verifies it only by eyeball in an interactive shell, which is not the failing environment. R also drops non-existent `.libPaths()` entries silently, so setting a path is not self-validating. **Stage 1 (investigation + design) COMPLETE; nothing implemented.** Designed: `deploy/afldb-r-env.sh` (sourced; prepends an optional `AFLDB_R_LIBS` to `R_LIBS` — additive; `R_LIBS_SITE` rejected because it displaces the `apt` site library), `deploy/afldb-r-preflight.sh` (fails loudly at deploy time on a missing `jsonlite`/`digest`/`fitzRoy` or a `fitzroy-contract.json` pin mismatch, prints effective `.libPaths()`, installs nothing), `docs/deployment.md` §7b, and tests in the existing `readSource` style. The unit file, its hardening and `TimeoutStartSec` are deliberately **not** modified. Key files: `issues/open/AFLDB-ISSUE-130.md`, `deploy/afldb-settle-afltables.{service,sh}`, `docs/deployment.md`, `tests/current-season-import.test.ts`. | **Stage 2, fresh session, carry-over `issues/open/AFLDB-ISSUE-130.md`.** First take the one open operator decision (runbook §7): is `/home/arm/R/library` intended on the dev host, or should it be reconciled to the documented `/usr/local/lib/R/site-library` (recommended)? Either answer leaves the repository implementation identical — it decides the host action and one documentation paragraph. Then implement §5.1-§5.3 and §5.5-§5.6 exactly, no broader; run `sh -n` on the three shell files and `npx vitest run tests/current-season-import.test.ts`. Only then, on the **dev host only**: host action → deploy → **remove the temporary drop-in** → `daemon-reload` → `sh deploy/afldb-r-preflight.sh` → one supervised `systemctl start` proving the chain still completes with the drop-in absent. Confirm production's R library location while there; production deployment is separate work and is queued behind `AFLDB-ISSUE-128`/`-129`. |
