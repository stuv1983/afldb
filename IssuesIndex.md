...
**Open issues:** 4 tracked here — `AFLDB-ISSUE-110`, `AFLDB-ISSUE-118`, `AFLDB-ISSUE-137`, `AFLDB-ISSUE-138`.
...
| `AFLDB-ISSUE-118` | Medium | Grid Solver / External datasets / Data acquisition | **REOPENED 2026-09-05; Stages H2–H4 (heights), Family A (medals) and Family B (captaincies) done the same day** (runbook §23.16–§23.22). Heights: AFL Tables register canonical (12,487 on `afldb_test`), the AFL API season rosters (`acquire_rosters.R`, 2012–2026, 1,824 players) and a tracked Wikipedia adjudication set (83 players) loaded as corroborating evidence; precedence decided on evidence (§23.19 H3.6), no canonical height changed; the corpus classifies height cells from the evidence: `incorrect known answer` 299 → 0, `external source disagreement` 197 cells / 6 players (informational), `source conflict` 102 cells / 3 players (closed by Stage E1 below). Rebuild: three manifest-pinned height stages + five final gates in `db:test:rebuild`; **clean unattended 16-stage rebuild proven 2026-09-05 (22 min, FINAL VALIDATION 53/53, §23.23)**. Family A: 7 medals, 328 Wikipedia-cited winner rows, all linked on `afldb_test`; `unsupported` 375 → 309 (19 criteria). Family B: 399 captaincy rows for Geelong / Hawthorn / West Coast / Fitzroy / Brisbane Bears / University, 1,774 / 1,774 linked on `afldb_test`. Co-captaincy (§23.23): Gridley's `premcaptain` names one premiership captain per flag, AFLDB's `premiership_captain` every appointed captain who played and won the Grand Final — AFLDB's data/semantics not wrong, solver unchanged; the 5 Adam Goodes cells are classified `external source disagreement` from AFLDB's own co-captaincy evidence, `incorrect known answer` 5 → 0; strict run fails only on `unsupported` 309 + `dataset gap` 354 + `source conflict` 102. Stage D1 (§23.24–§23.25) DONE: AFL Tables all-time club lists acquired (`acquire_club_lists.R`, 21 pages, 16,731 rows, manifest-pinned `club-lists-20260905`); `birth-dates` rebuild stage after `heights-wikipedia` with the pin read fail-closed from `afltables-contract.json`, `--validate-only` preflight and five gates (13,255 with `dob`, 0 without evidence, 13,255 with club-list evidence, 0 conflicts, 2 kept fitzRoy/AFL Tables disagreements); loader writes batched (1,658 s → 2.7 s); **clean unattended 17-stage rebuild 2026-09-05 13:31–13:54 (23 min), FINAL VALIDATION 58/58**. Grid Solver `age_on_debut_min` (Aged X or older on debut, from canonical `dob` + `debut_date`); Gridley `debut22` mapped (4,416 players); data-absent criteria 19 → 18, occurrences 103 → 102; corpus `unsupported` 309 → 306 cells, cells solved 9,626 → 9,629, `incorrect known answer` 0; strict fails only on `unsupported` 306 + `dataset gap` 354 + `source conflict` 102. **Stage E1 (§23.26) DONE: the 3 height source conflicts adjudicated on evidence, no canonical height changed** — Nathan Brown 9771 tied to his Wikipedia article by the Stage D1 date of birth (183 cm, on AFLDB's side of the ≤180 bound; tracked Wikipedia set 83 → 84 rows), so his 40 cells are `external source disagreement` under the unchanged §23.19 rule; McCartin (194 vs AFL listing/Wikipedia 195) and Ugle-Hagan (194 vs 197) retained under §23.19 rule 1 in the new tracked, self-invalidating `data/players/height-adjudications.csv` (applies only while the canonical height and competing evidence are exactly those decided on), their 62 cells the new informational `adjudicated source conflict`; `source conflict` 102 → 0; strict fails only on `unsupported` 306 + `dataset gap` 354 (failing cells 762 → 660).  **Stage E2 (§23.27) investigated, model decided, not implemented:** the snapshot's per-match `Coach` column is one coach per (match, club) on all 129 seasons (32,034 of 33,676 team-matches; gaps only ≤1922 and 11 in 1940; 162 team-seasons with mid-season changes; 383 strings, stable spelling); identity comes from the AFL Tables coaches index (386 pages, 386 unique names, all snapshot strings resolve) whose coach pages link the player profile path `external_identities` already holds — so `coaches` (person, nullable `player_id` by that link, never by name) + `match_coaches` (match, club, coach), a manifest-pinned `coaches` acquisition/stage, `coached_by` + `premiership_coach` builders. **Stage E2 (§23.28) IMPLEMENTED:** coach pages acquired (`acquire_coaches.py`, 386 pages, manifest-pinned `coaches-20260905`, parsed artefacts tracked); migration `087_coaches.sql` (`coaches` + `match_coaches`, registered for app read / import write); loader `import_match_coaches.py` (exact-string join to the pages, profile-path-only player links, four evidence-dated `profile_link_corrections` for page hrefs AFL Tables does not serve, `--validate-only` / `--dry-run`, idempotent) — on `afldb_test` 386 coaches (368 linked, 18 coach-only incl. the operator list), 32,034 assignments, 15,817 / 400 / 621 matches with both / one / no coach; `coaches` rebuild stage after `birth-dates` with preflight and eight gates (18 stages, 66 checks); `coached_by` + `premiership_coach` (`GRID_BUILDERS` 154); `premcoach` + 7 `coachedBy*` mapped (data-absent 18 → 10 criteria, 102 → 86 occurrences); corpus `unsupported` 306 → 258 cells, cells solved 9,629 → 9,677, `incorrect known answer` 0 (Gridley's list-grain `coachedByGoodwin` cells for Heppell/Daniher are the documented `list membership` difference), strict fails only on `unsupported` 258 + `dataset gap` 354. **Unattended 18-stage rebuild proven 2026-09-05 16:55–17:17 (22 min), FINAL VALIDATION 66/66** (coaches stage batch 11 identical to the hand load). **Family F (§23.29) — father–son rule selections IMPLEMENTED:** Gridley `fathersonfather` = the FATHER of a son selected under the rule; the migration-006 `father_son_selections` and `player_relationships` tables are populated for the first time from the tracked normalised Wikipedia list (`data/players/father-son-selections.csv`, 127 selections 1988–2025, article *Father–son rule* rev 1370239415) by `tools/migration/father_son.py` — `normalize` resolves every person once by name + era + drafting-club lineage to an AFL Tables profile path, refuses ambiguity, and requires each of the 7 evidence-dated adjudications (`father-son-adjudications.csv`: Brad/Bradley Campbell, Billy/Bill Brownless, four WAFL/SANFL-qualified fathers tied by Wikipedia, Peter Morrison explicitly unlinked) to be needed exactly once; `load` (validate-only / dry-run / idempotent) links only through `external_identities`. On `afldb_test`: 127 selections, 99 sons + 123 fathers (107 distinct) linked, 28 non-player sons and 4 non-VFL/AFL fathers unlinked, 127 `parent_child` relationships; migration `088` adds the link CHECKs; `father-son` rebuild stage after `coaches` with preflight and six artefact-derived gates (19 stages, 72 checks, 6/6 by hand); `father_son_father` + `father_son_selection` (`GRID_BUILDERS` 156); data-absent 10 → 9 criteria / 86 → 83 occurrences; corpus `unsupported` 258 → 249, solved 9,677 → 9,686, `incorrect known answer` 0, the 9 father–son cells finding-free; strict fails only on `unsupported` 249 + `dataset gap` 354. **Unattended 19-stage rebuild proven (§23.30, 23 min, FINAL VALIDATION 72/72).** **Family F (§23.31) — siblings IMPLEMENTED:** the operator export of the legacy Wikipedia football-families tables (article rev 1365040810; 485 `sibling` rows, one per pair, 0 self) is normalised by `tools/migration/family_siblings.py` to the tracked `data/players/sibling-relationships.csv` — 498 pairs = 484 export pairs (one duplicated family merged) + 14 evidenced supplements (`sibling-supplements.csv`: Gary Ablett Jr ↔ Nathan Ablett and 13 more pairs the export named only as a father's sons, each on a quoted sentence from the people's own articles — absence of an export row is unknown coverage, never a negative); identity by name + listed-club lineage + article-title birth year against the canonical DOB, 8 evidence-dated adjudications, 0 ambiguous, no name-only link; on `afldb_test` 498 `sibling` rows, 389 both linked, 658 players with a linked brother, 0 self / duplicate pairs, load idempotent (second run 0 changed); `siblings` rebuild stage after `father-son` with preflight and seven gates (20 stages, 79 checks); `has_brother` (`GRID_BUILDERS` 157); Gridley `brother` mapped (data-absent 9 → 8 criteria, 83 → 30 occurrences); corpus `unsupported` 249 → 90 cells, solved 9,686 → 9,842, `incorrect known answer` 0, new `source coverage gap` category 21 cells / 1 player (Jarryd Lyons, whose brother was listed but never played); family card now labels sibling rows from `relationship_label`; **unattended 20-stage rebuild proven 2026-09-05 21:36–21:58 (22 min), FINAL VALIDATION 79/79**; every remaining brother gap reconciled against the upstream families page by section (only Lyons remains: the page states sons of one father only and Corey Lyons never played). | **Exact next action: ISSUE-118 remaining-scope review in a fresh session.** §23.35: (1) after-the-siren — model + artefact DONE §23.33 and **LOADED + RECONCILED §23.34** (migration 089 applied to `afldb_test`; `after_siren.py load` / `reconcile`; 126 canonical rows, 120 kickers linked over 111 distinct / 6 unresolved, 116 matches linked / 10 NULL, 64 qualifying over 62 kickers, 4 adjudications applied exactly once, reconciliation 38/38, second and third identical loads 0 churn): **SOLVER DONE §23.35** (`after_siren_winner`, `GRID_BUILDERS` 158; Gridley `winaftersiren` mapped, 63 rows / 61 linked kickers; data-absent 8 → 7 criteria / 30 → 26 occurrences; corpus `unsupported` 90 → 78, solved 9,842 → 9,854, `incorrect known answer` 0, the 5 after-siren findings all `time of board`, 0 new timeouts); the `after-siren` rebuild stage after `siblings` is still open; (3) International Rules (`data/reference/international-rules/`, establish `intrulesplayer`, inventory the scrape); (4) `season2024player`; (5) Tony Buhagiar All-Australian adjudication; `spoils5season` / `recruitedByDodoro` deferred, `irish` / `tasmanian` / `nfl` presumptively deferred; a fresh sibling extraction from the families article when a sibling follow-up is scheduled; DEV load of birth dates + coaches + father–son + siblings; production with the next deploy (ISSUE-137 sequencing). Follow-up recorded: Grand-Final-day captains of 1915/1958/1977/2004/2016 are not modelled. **Remaining-scope review DONE 2026-09-05 (seventeenth session, doc-only, runbook §23.36):** residual unsupported-valid set verified at **7 criteria / 26 occurrences** (`season2024player` 14, `intrulesplayer` 5, `irish` 2, `recruitedByDodoro` 2, `nfl` 1, `spoils5season` 1, `tasmanian` 1), each formally classified as an accepted deferral — redundant compatibility alias (`season2024player`), future/non-core canonical domain (`intrulesplayer`, `irish`, `tasmanian`, `nfl`, `recruitedByDodoro`), unavailable canonical statistic (`spoils5season`) — and a formal closure contract recorded (runbook §23.36). **Public UI exposure DONE 2026-09-06 (eighteenth session, runbook §23.37):** coach-only profiles (`/coaches/[slug]-id`, linked coach redirects to their player page, `/coaches` index), after-the-siren player-profile exposure (`getPlayerAfterSirenEvents`, plain-language wording, collapsed by default), coach search/discovery (`searchCoaches`, scoped `player_id IS NULL`), Family/Honours wrapped in `CollapsiblePanel`, Brownlow de-duplicated out of the generic Honours list, Career & Biography regrouped (identity first), disputed-DOB accessibility fixed (visible text, not `title`), final section hierarchy set; DB-free 3,193/3,198 (5 pre-existing unrelated failures) + `afldb_test`-backed integration tests (Leigh Matthews, Chris Fagan, Luke Shuey, David King, Kerry Good) all pass; `tsc`/`eslint` clean; no schema/canonical-data/Grid-Solver change. | **Exact next action: the final ISSUE-118 rebuild/corpus/closure proof, in a fresh chat** (§W.4's UI exposure audit is now satisfied); still open alongside it: the `after-siren` rebuild stage after `siblings`, DEV load, International Rules, Buhagiar adjudication. |
