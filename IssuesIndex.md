...
<!-- ALLOCATION WARNING 2026-09-01: `AFLDB-ISSUE-118` is allocated and is NOT free.
     ...
     `codex/issue-119` and applied to `afldb_test`. `082` was the next free migration number as of the 2026-09-01 scan; it is
...
|---|---|---|---|
| `AFLDB-ISSUE-119` | Medium | Admin / Security / NL search / Telemetry / Database | **Stage 2 in progress: the DATABASE SECURITY BOUNDARY IS VALIDATED on `afldb_test` (2026-09-01, runbook §24). Migration `081_nl_search_telemetry_clear.sql` is applied (§23), `npm run db:privileges:test` has run — `NOTICE` confirming `PUBLIC` revoked and `afldb_auth` granted `EXECUTE` — and `tests/integration/privileges.test.ts` passes 34/34, proving from the applied catalogue: owner `afldb_owner`, `SECURITY DEFINER`, `VOLATILE`, zero parameters, `search_path` pinned to `pg_catalog, pg_temp`, an ACL asserted outright as exactly `{afldb_auth, afldb_owner}` (excluding the NULL-ACL/EXECUTE-to-PUBLIC restore state), `afldb_app`/`afldb_import`/`afldb_backup` unable to execute, no dynamic SQL/CASCADE/TRUNCATE/unqualified relation in the stored body, and NO direct `DELETE`/`TRUNCATE` for `afldb_auth` on any of the three NL tables. BUT the function has still NEVER BEEN CALLED: `tests/integration/nl-search-telemetry-clear.test.ts` has never run, so the retained closure, ancestry, counts, lock cutoff, app-health detachment and sequence non-reset are all unevidenced. `AFLDB_TEST_AUTH_DATABASE_URL` is still undefined, so restricted-role end-to-end execution is PENDING, NOT PASSED — a skipped describe is not a pass.** The safe reset target is disposable `nl_search_log` only. Repository contracts make `nl_search_review` durable admin decisions and `nl_search_feedback` immutable reader facts, so both and their minimum log/parent-chain context must survive. Proposed mechanism: selective `DELETE` through one `afldb_auth`-executable, owner-defined fixed function; no direct DELETE/TRUNCATE grant, no sequence reset, mandatory same-transaction count-only `auth_audit_log` event, child-before-parent writer locks, typed confirmation, and test-only restricted-role proof. Runbook: `issues/open/AFLDB-ISSUE-119.md` (renumbered from 118 on 2026-09-01; `opus/gridley-corpus` holds committed ISSUE-118). Approval binds Stage 2 to the §5.1 obligation table and adds a mandatory recursive-ancestry test deeper than one parent (§13). The `SECURITY DEFINER` clear function, its owner pinning and its single `EXECUTE` grant are written and source-reviewed against §5–§8 (runbook §20); `privileges.sql` now restores owner, `PUBLIC` revoke and `afldb_auth` `EXECUTE` after a role-after-migration install or a `--no-privileges` restore, and revokes any direct DELETE/TRUNCATE on the three NL tables (runbook §21). The guarded tests (`tests/integration/nl-search-telemetry-clear.test.ts` new, `tests/integration/privileges.test.ts` extended) cover the mandatory deeper-than-one-parent ancestry fixture with a deleted mid-chain sibling, orphaned feedback, app-health `SET NULL`, sequence non-reset, rollback, lock-cutoff blocking and the restricted `afldb_auth` credential (runbook §22). **Next action: per runbook §24.8 — (1) establish the restricted `_test` auth DSN by setting `AFLDB_TEST_AUTH_DATABASE_URL` to the same endpoint and same `afldb_test` database with role `afldb_auth` and its own password (never the owner credential, never `afldb_dev` or production; needs an `afldb_auth` password on the test cluster); (2) then operator-run only `npx vitest run tests/integration/nl-search-telemetry-clear.test.ts` — the first call of the function ever — confirming from the output that the restricted describe RAN rather than skipped. Note `081` is checksum-locked in `afldb_meta.schema_migrations`, so any repair is a new migration or a test-DB rebuild, never an in-place edit. Only after that evidence passes: the query layer (casting the five `bigint` counts, which `postgres.js` returns as strings), the transaction-aware audit helper, the Server Action, the UI, `docs/search.md` and `CHANGELOG.md`, in that order. No Server Action or UI until then.** |
