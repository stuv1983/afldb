...
- **State:** **Planning complete 2026-09-11 — no code, no migration, no commit, no deployment; no database read or written.** Runbook `AFLDB-ISSUE-162.md`. Finding: `matches` requires NOT NULL scores/result/margin (003) and every ladder/season/venue/NL/Grid consumer reads "row exists" as "played", so an unplayed fixture cannot live there (S-1/S-5 fire for any `matches`-based option); AFLW already separates `staging_aflw.fixtures` from `staging_aflw.matches` (025). Chosen: new canonical registry table `fixtures` (schedule facts only, no score column), `matches` untouched, "played" derived by season + round code + club-pair resolution, identity = minted `fixture_key` under `manual_admin_edit`, whole-row `data_overrides` + fail-closed `replay_admin_overrides('fixtures')`, `data_edits` widened with a `fixture_key` lineage rule, never-delete lifecycle (`scheduled`/`cancelled`/`void`), no `seasons`/`clubs`/`club_seasons` write (window `max(seasons.year)..max+1`, clubs via `afldb_season_list_clubs()`), no settle/current-season/rekey change, no public exposure. One additive migration (next free 097, not allocated). Capabilities `data.fixtures.read`/`.edit`; routes `/admin/fixtures`, `/[season]`, `/[season]/new`, `/[season]/[fixtureKey]`; round-at-a-time batch entry with fingerprint-gated all-or-nothing confirm. Stacked on ISSUE-161 (`opus/issue-162-fixture-admin` from `34858ce`); deploys to DEV only with the Admin Centre batch.
- **Key files/subsystems:** `AFLDB-ISSUE-162.md` (runbook); evidence in `src/db/migrations/003_matches.sql`, `022`, `025_staging_aflw.sql`, `096_season_list_members.sql`; `src/db/queries/match-admin.ts` (`createMatch`, not reused), `src/lib/acquisition/{match-rekey,settle-afltables,canonical-apply}.ts`, `src/lib/external-afl/current-season-import.ts`, `tools/migration/{import_fitzroy_core.py,common.py,rebuild_derived.py}`, `tools/db/promotion-inventory.ts`; planned `src/db/queries/admin-fixtures.ts`, `src/app/admin/fixtures/**`, `tests/admin-fixture-actions.test.ts`, `tests/integration/admin-fixtures.test.ts`.
- **Next action:** **D-1…D-7 DECIDED 2026-09-11** (runbook §35; all as recommended, D-6 with the condition that the read-time played resolution is deterministic and fail-closed with ambiguity left unlinked, and the constraint that `fixture_key` is never replaced by `match_key`). Operator commits the planning files; then Stage 1 in a fresh high-effort session (`Execute AFLDB-ISSUE-162 Stage 1 according to AFLDB-ISSUE-162.md`), starting with the preflight and the migration-number re-check. Do not merge ISSUE-160/161 first, do not update DEV, do not deploy, do not touch PROD.
