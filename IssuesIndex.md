...
- **Next action:** operator validation, Stage 1 first (it is a precondition for Stage 2's) — apply migration 098 to **afldb_test only**, `npm run db:privileges` against it, then `npx tsc --noEmit`, `npx vitest run tests/data-overrides-source-contract.test.ts tests/db-promotion-check.test.ts tests/current-season-import.test.ts`, `npx vitest run tests/integration/admin-club-leadership.test.ts tests/integration/admin-season-lists.test.ts tests/integration/admin-fixtures.test.ts`, ESLint over the Stage 1 files. Then Stage 2: `npx vitest run tests/admin-club-leadership-actions.test.ts tests/auth.test.ts`, ESLint over the Stage 2 files, `git diff --check`. Then commit Stage 1 and Stage 2 separately and run the local completion audit mirroring 161/162. Stacked on ISSUE-162 at `f80a1df`; ships only with the combined 160+161+162+163 Admin Centre DEV batch (deploy order 096 → 097 → 098 → `db:privileges` → code). No DEV, no PROD, no merge, no deploy. ISSUE-163 is **not resolved**.
