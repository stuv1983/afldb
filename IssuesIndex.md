...
- **Next action:** ONE item blocks closing ISSUE-155 — add `brownlow_vote_entry_state` and `brownlow_season_authority` to `PROMOTION_CONTRACT` in `tools/db/promotion-inventory.ts` (`productionOnly`, `treatment: 'reinstate'`, `compare: 'equal'`, `restoreAfter: ['auth_users']`, `footballRefs`/`lineageRefs`/remediation for `match_id → matches` current-season id-remap via `match_key`, the player-id and season refs), update the hardcoded list at `tests/db-promotion-check.test.ts:349`, re-run `tests/db-promotion-check.test.ts` to green. Do NOT use `grant_import_write()`. This is the §27.28 / §27.22 ISSUE-151 promotion-lineage follow-up and a pre-deploy stop condition; no new issue ID. Then run the §27.27 browser acceptance, then the §27.21 deploy sequence. Do not absorb the §27.31 database-state policy question or the `external_grid` privilege mismatch.
