...
| `AFLDB-ISSUE-118` | Medium | Grid Solver / External datasets / Data acquisition | **REOPENED 2026-09-05; Stage AA3 done the same day.** The 1983/1986/1987/1988 VFL Teams of the Year are in `data/awards/all-australian.csv` (1,158 → 1,244 rows, 86 new `wikipedia` rows all linked, 7 players censused; loaded on `afldb_test` + `afldb_dev`). Oracle `AFLDB source missing required selection` 315 entries / 14 players → **11 / 1** (Greg Anderson, Gridley-side, recorded); David Clarke 1972 link confirmed correct; same-season double selection counts once (decided by the corpus). Still open: the corrected acceptance (every valid stored Gridley criterion answered exactly, 0 unsupported, 0 timeouts) fails on the height / draft / marquee / captaincy gaps. On the branch: All-Australian final-team builders, height builders (`height_min` / `height_max`), strict corpus regression. Height source: AFL Tables `player_details` (acquired, 95% HT, no stable id) — cross-source reconciliation, **Fable High decision pending**. Key files: `issues/open/AFLDB-ISSUE-118.md` §23 (§23.14 AA3, §23.15 next action), `data/awards/all-australian.csv`, `tools/migration/all_australian.py`, `src/search/gridley-compat.ts`, `tests/integration/gridley-aa-oracle.test.ts`. | Fable High session for Stage H2 (height acquisition and provenance) and the schema/model families (runbook §23.15). |
