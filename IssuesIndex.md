...
|---|---|---|---|
| `AFLDB-ISSUE-119` | Medium | Admin / Security / NL search / Telemetry / Database | **Stage 2: the DATABASE LAYER IS NOW FULLY VALIDATED END TO END on `afldb_test` (2026-09-01, runbook §23–§25).** Migration `081_nl_search_telemetry_clear.sql` is applied (§23); `npm run db:privileges:test` has run; `tests/integration/privileges.test.ts` passes 34/34 (§24), proving from the applied catalogue owner `afldb_owner`, `SECURITY DEFINER`, `VOLATILE`, zero parameters, `search_path` pinned to `pg_catalog, pg_temp`, an ACL asserted outright as exactly `{afldb_auth, afldb_owner}` (excluding the NULL-ACL/EXECUTE-to-PUBLIC restore state), `afldb_app`/`afldb_import`/`afldb_backup` unable to execute, no dynamic SQL/CASCADE/TRUNCATE/unqualified relation in the stored body, and no direct `DELETE`/`TRUNCATE` for `afldb_auth` on any NL table. **The clear function has now also been CALLED for the first time (§25):** `AFLDB_TEST_AUTH_DATABASE_URL` is established — derived from the existing `afldb_auth` credential by changing only the database to `afldb_test`, never the owner credential, never `afldb_dev` or production, with endpoint/database/`_test` parity asserted before connecting and a probe confirming `current_user=afldb_auth` before `.env` was written — and `npx vitest run tests/integration/nl-search-telemetry-clear.test.ts` passes **9/9 with 0 SKIPPED**, the restricted describe RUNNING rather than skipping. Proven by execution: the full four-level ancestor chain above a reviewed leaf survives while the mid-chain sibling and child-of-leaf are deleted; reviews and matched/orphaned feedback byte-unchanged; app-health rows kept with only the disposable link `NULL`ed; sequence not reset; an aborted clear rolls back deletes and detachments alike; a second-backend writer provably blocks on the lock cutoff via `pg_blocking_pids()`; and live as `afldb_auth`, EXECUTE succeeds while `DELETE`/`TRUNCATE` are refused at SQLSTATE `42501`. **Acceptance criterion 5 is fully evidenced and the restricted-role halves of 5 and 10 are PASSED.** **The QUERY LAYER IS NOW VALIDATED TOO (§26, §27):** `npx vitest run tests/nl-search-log.test.ts` passes **16/16** — the `clearNlSearchTelemetry count boundary` describe **11/11** — and `npm run typecheck` passes (final output `Types generated successfully`). **Risk R3 is therefore evidenced at the typed query boundary:** the `bigint` counts `postgres.js` hands back as strings are explicitly converted, or rejected outright (`null` and `''` would otherwise coerce to a false `0` and audit a real deletion as having deleted nothing), before they can enter the application result type or the §9 audit contract. **THE TRANSACTION-AWARE AUDIT HELPER NOW EXISTS AND IS VALIDATED TOO (§28):** `src/lib/auth/session.ts` extracts its single `auth_audit_log` INSERT into one private writer typed on `postgres.ISql` — the genuine common supertype of the `authSql` pool and a `begin()` handle, since `Sql` and `TransactionSql` are siblings and a `Sql` parameter would have forced a cast — so `audit()` keeps its exact signature, pool and behaviour while the new `auditInTransaction(tx, …)` writes the same row on the caller's transaction, with no cast, no suppression and no duplicated SQL. `npx vitest run tests/auth.test.ts` passes **31/31**, the new DB-free `auth_audit_log writer` describe **6/6** (row written on the given handle and never on the pool; actor id, email label, action, detail and last-hop `X-Forwarded-For` IP all preserved; nulls when absent; `audit()` unchanged; both forms emit byte-identical SQL; a failed insert propagates so the caller rolls back), and `npm run typecheck` passes, compiling all **67** existing `audit()` call sites against the refactored signature. Neither the query helper nor the audit helper yet has a **production caller** at that point. **The Server Action now exists (runbook §29, 2026-09-01):** `clearTelemetry()` in `src/app/admin/nl-search/actions.ts` calls `requireSuperAdmin()` before parsing the confirmation phrase or opening a transaction, checks the exact phrase `CLEAR SEARCH TELEMETRY` server-side, runs `clearNlSearchTelemetry(tx)` and `auditInTransaction(tx, …)` on one `authSql.begin()` handle with no try/catch, and on committed success only revalidates `/admin/nl-search` (`layout`) and `/admin/app-health`. `tests/admin-nl-search-actions.test.ts` (11 DB-free cases) is written and **now validated (runbook §29.7, 2026-09-01)**: `npx vitest run tests/admin-nl-search-actions.test.ts` passes **11/11** and `npm run typecheck` passes (final output `Types generated successfully`), evidencing guard ordering, exact confirmation-phrase gating, one shared transaction handle for clear+audit, a five-count-only audit payload, failure propagation with no revalidation, correct returned counts, and success-only two-path revalidation. **The UI, `docs/search.md` and `CHANGELOG.md` are now implemented too (runbook §30, 2026-09-01):** `ClearTelemetryForm.tsx` — typed-phrase client confirmation (`CLEAR SEARCH TELEMETRY`, matching the server's independent check), `DeleteMatchButton`-pattern reveal/confirm/cancel, pending-state protection, and a success report built from the returned `NlTelemetryClearCounts` — is wired into `/admin/nl-search` beside the period/export controls; the page's prior unqualified "append-only" subtitle claim is corrected. `docs/search.md` documents the retention/audit guarantees; `CHANGELOG.md` carries the `Unreleased` entry. `npx vitest run tests/admin-nl-search-actions.test.ts` (11/11) and `npm run typecheck` were re-confirmed. No new component test file was added, since this repository has no React-rendering test convention to extend. **Guarded Playwright UI confirmation remains the only unexecuted acceptance evidence** — the guarded PostgreSQL integration suites are not outstanding: `tests/integration/privileges.test.ts` (34/34, §24) and `tests/integration/nl-search-telemetry-clear.test.ts` (9/9, restricted describe proven to run, §25) already executed and passed against `afldb_test`. | The safe reset target is disposable `nl_search_log` only; `nl_search_review` (durable admin decisions) and `nl_search_feedback` (immutable reader facts) and their minimum log/parent-chain context must survive. Mechanism as built: selective `DELETE` through one `afldb_auth`-executable, owner-defined fixed function; no direct DELETE/TRUNCATE grant, no sequence reset, mandatory same-transaction count-only `auth_audit_log` event, child-before-parent writer locks, typed confirmation, test-only restricted-role proof. Runbook: `issues/open/AFLDB-ISSUE-119.md` (renumbered from 118 on 2026-09-01; `opus/gridley-corpus` holds committed ISSUE-118). | **Next action, per runbook §31 (2026-09-01):** the guarded Playwright harness is now **written and type-validated but unexecuted** (`npm run typecheck` → passed, `✓ Types generated successfully`, runbook §32) — `playwright.telemetry-clear.config.ts` + `tests/admin-nl-search-clear/` (layered destructive-target guard refusing any non-loopback / non-`_test` / unacknowledged target; two-account real-TOTP auth setup; deterministic `DELETE`-and-reseed fixture giving exactly deleted 5 / retained 6 / review 1 / feedback 2 / detached-link 1; acceptance spec covering reveal-cancel, exact-phrase gating, successful clear + returned counts, retained review/feedback messaging, Super-Admin-only access, and an in-browser proof the deployment reads the seeded `_test` DB). It is blocked on two operator prerequisites: (1) a real **non-super plain-admin** account (`AFLDB_E2E_PLAIN_ADMIN_*`); (2) a disposable **loopback `_test` deployment** of this branch (`AFLDB_BETA_GATE=off`, `DATABASE_URL`/`AFLDB_AUTH_DATABASE_URL` → `afldb_test`). Exact run command in runbook §31.7. If it passes, Stage 2 is ready for closure: resolve this issue, remove it from this table and `issues.md`'s Open Issues table. Open risks: R4, R5, R6, R7, R8. Migration `081` is checksum-locked — any repair is a new migration or a test-DB rebuild, never an in-place edit; `npm run db:privileges:test` stays idempotent and safe to re-run.** |
...
| `AFLDB-ISSUE-117` | Medium | Security / Production reliability / NL search / Telemetry | **Created 2026-08-31 by the full-codebase review.** The public NL `/search` pipeline has **no per-IP rate limit** — every GET with `q` runs the full NL pipeline plus one `nl_search_log` INSERT (`src/app/search/page.tsx:71`, `src/db/queries/nl/log.ts:138`) while autocomplete (60/min), feedback (12/15 min) and health-event (120/min) are all limited. Currently shielded by the beta gate (`middleware.ts:117`) — credible **pre-launch** risk, not a live defect. Secondary LOW hardening in the same runbook: `/api/health-event` unbounded `request.json()` body (`route.ts:64`), and `Object.hasOwn` missing on two prototype-key lookups (`records.ts:159`, `aflw.ts:786-793` — 500 on crafted key, provably not injection). Runbook: `issues/open/AFLDB-ISSUE-117.md`. **Next action:** implement F1 (per-IP limiter on the NL search entry, reuse `src/lib/auth/rate-limit.ts`, fail-open, friendly response) plus small F2/F3 edits in one focused session; **must be closed or re-adjudicated before `AFLDB_BETA_GATE` is disabled in production.** |
