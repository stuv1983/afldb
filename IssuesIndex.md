...
- **State:** Open / Stage 1 COMPLETE and VALIDATED (G0–G8 all PASSED 2026-09-11), committed `dbef4c2`, deployed to DEV, not merged to `main`. **Stage 2 (the `/admin/coaches` admin surface) is now IMPLEMENTED but NOT VALIDATED** — written 2026-09-11 against `AFLDB-ISSUE-159.md` §9/§14/§16.1 deliverable set: capabilities `data.coaches.read` (Admin-and-up) / `data.coaches.edit` (Super Admin only); nav entry; `/admin/coaches` (search/filter/create) and `/admin/coaches/[id]` (identity/provenance, editable metadata, player linkage, club+season-bounded coaching assignments); seven Server Actions (`createCoach`, `saveCoachMetadata`, `linkCoachToPlayer`, `unlinkCoach`, `setCoachAssignment`, `clearCoachAssignment`, `retireCoachOverride`), each `requireCapability('data.coaches.edit')`-first and each one import-role transaction writing the canonical row + `data_overrides` upsert + `data_edits` row together (§4.2); `src/db/queries/admin-coaches.ts` (public-client reads, plus a D-2 narrow SELECT-only import-role helper for `data_overrides`, which carries no `afldb_auth` grant); duplicate-prevention on create (§4.4, hard + soft-confirmable refusals); player linkage resolved through `external_identities` only, refusing a tracked `afltables-contract.json` `profile_link_corrections` rule (S-9); `setCoachAssignment` accepts one or many `(matchId, clubId)` pairs in one transaction for the "apply to every listed match" control (§9.4). **S-6 followed literally:** no coach Server Action calls `revalidatePath` — a dedicated `/admin/coaches/revalidate` route (its own `requireCapability('data.coaches.edit')` guard) does, invoked by the browser from `submit-helper.ts` only AFTER an action has already resolved, alongside `router.refresh()` for the admin surface itself; this is a deliberately different, more conservative mechanism than `brownlow`/`player-links` actions.ts currently use (both still call `revalidatePath` in-action). **NONE of this has been run**: no `npx tsc --noEmit`, no `npx vitest run`, no browser acceptance. `tests/auth.test.ts` extended (`EQUIVALENT_ROLE_GUARD` entries for both new capabilities — the nav/boundary/guard-order/pre-auth contracts are structural and should pass by construction, unverified); `tests/coach-slug.test.ts` extended (2 cases, `coachSlug` itself unchanged); new `tests/admin-coach-actions.test.ts` (DB-free: `entity_key` shapes + form-parsing validation); new `tests/integration/admin-coaches.test.ts` (needs `AFLDB_TEST_DATABASE_URL`, unrun). No `PROMOTION_CONTRACT`, `privileges.sql`, `src/lib/edit/spec.ts`, `coaches_link_ck` or migration 087 change (all correctly out of Stage 2 scope, per §16.2's Stage-1-only prohibitions carried forward in spirit). Coach reconciliation remains out of scope and P9-class.
- **Key files/subsystems:** Stage 1 (unchanged, validated): `src/lib/acquisition/manual-authority.ts`, `src/db/migrations/095_coach_admin_overrides.sql`, `tools/migration/common.py`/`import_match_coaches.py`, `tools/db/promotion-inventory.ts`, `src/db/queries/audit-log.ts`, `docs/production-promotion.md`. Stage 2 (new, unvalidated): `src/lib/auth/capabilities.ts`, `src/app/admin/nav-model.ts`, `src/db/queries/admin-coaches.ts`, `src/app/admin/coaches/**` (`page.tsx`, `[id]/page.tsx`, `actions.ts`, `validation.ts`, `submit-helper.ts`, `revalidate/route.ts`, `CreatePanel.tsx`, `MetadataPanel.tsx`, `LinkagePanel.tsx`, `AssignmentPanel.tsx`), `tests/auth.test.ts`, `tests/coach-slug.test.ts`, `tests/admin-coach-actions.test.ts` (new), `tests/integration/admin-coaches.test.ts` (new).
- **Next action:** Operator must run, in order: `npx tsc --noEmit`; `npx vitest run tests/auth.test.ts tests/coach-slug.test.ts tests/admin-coach-actions.test.ts`; `npx vitest run tests/integration/admin-coaches.test.ts` against `AFLDB_TEST_DATABASE_URL`; then browser acceptance on DEV at the widths and refused-action-focus check §9/§10.2 gate 12 specifies, across Super Admin / Admin / Contributor (gate 6, the permission matrix). Only after all of that passes does Stage 2 become "implemented and validated," and only then is `AFLDB-ISSUE-159` eligible for Resolved — nothing here has been claimed as validated. G4-D (Brownlow settle carry-forward) remains open and unrelated to Stage 2.
