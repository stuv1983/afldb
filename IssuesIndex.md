...
**ISSUE-102 / ISSUE-112 closeout checkpoint — 2026-09-02 Pass 17 (supersedes
the older Pass-15/16 gate summaries embedded in the long rows below):** both issues
remain **OPEN**. The PlayerResolver is fail-closed and now accepts only exactly
one adjudicated `unique`/`resolved` AFL Tables identity; one tracked DraftGuru
decision resolves Matthew Rendell, leaving 18 players / 33 manifest rows without
a repository-supported stable identity. **G3 PASS:** the full DB-backed file ran
**107/107, 0 skipped, 0 failed**, including all formerly skipped 21 fixtures.
**G5 PASS non-vacuously:** those fixtures exercised 9 decisions (8 linked + 1
confirmed-unlinked), with same-row/same-player replay, zero retained orphans or
mismatches, protected ownership, stable ids and cross-family isolation. G6 is
blocked on the absent accepted DraftGuru, fitzRoy core and ladder-witness bytes;
the runbook has no waiver. Exact next action: supply the exact retained accepted
snapshots and execute the canonical rebuild. See ISSUE-112 §26.

| Issue | Severity | Area | Current state |
|---|---|---|---|
| `AFLDB-ISSUE-102` | Medium | Data acquisition / Import architecture | **PARENT ARCHITECTURE / COORDINATION RECORD — scope revised 2026-08-30 by operator decision** from the former "record only; do not design the replacement" boundary, which is now superseded (lineage retained in `issues.md`). Runbook `issues/open/AFLDB-ISSUE-102.md`; continuation state `issues/open/AFLDB-ISSUE-102-HANDOFF.md`. ISSUE-102 owns the architecture, the `AFLDB_LEGACY_SQLITE` inventory, child coordination, the closure criteria and the final verification that `import_awards.py` no longer operationally requires legacy SQLite — **it does not implement loaders**. Confirmed: the dependency is **per-group** (`needs_legacy = any(key not in LEGACY_FREE_GROUPS ...)`; the predicate was generalised from the single `under_22` exemption by ISSUE-111 pass 1), `under_22` is legacy-free via the tracked `data/awards/22-under-22.csv` manifest and `coleman` is legacy-free by derivation, leaving **six of eight** groups still dependent; classified **`LEGACY_SOURCE_DEPENDENCY`**, a repeatable reload still required by `docs/deployment.md` §7. `tools/db/rebuild-test.ts` gained a `coleman` data stage in ISSUE-111, **validated by the 2026-08-30 canonical rebuild** (`AFLDB-ISSUE-111` **Resolved 2026-08-30**; its record stays at `issues/open/AFLDB-ISSUE-111.md` while this parent cites it); the other five awards/honours tables still have **no rebuild stage and no Stage-9 gate**, so a canonical rebuild leaves them at **zero rows** — the `club_seasons = 0` shape of ISSUE-095. Every integrity contract a replacement needs already exists in `reload_keyed` and **no privilege change is required**; but `awards-reload-links.test.ts:205-1247` is **currently unexecutable** (gated on `AFLDB_LEGACY_SQLITE`). Children: **`AFLDB-ISSUE-111`** (Coleman derivation), **`AFLDB-ISSUE-112`** (curated honours manifests), **`AFLDB-ISSUE-113`** (Brownlow season totals — **outside 102's closure boundary**; 102 may resolve while 113 stays open, and the resolution must say so). `AFLDB-ISSUE-110` is NL semantic-mapping work (merged into dev 2026-08-31, own row below) and is **not** a child. **Next action: ISSUE-112 implementation phasing** — G0 is **DONE 2026-09-01 (all nine families PASS**, measured read-only against `afldb_dev`; `AFLDB-ISSUE-112.md` §14.4/§14.6). All 7 slices (honour teams, Hall of Fame, captaincies, Rising Star, All-Australian, club best-and-fairest, named medals) are **IMPLEMENTED + DB-validated** (through 2026-09-02, Pass 14) — every family is manifest-backed and runs legacy-free individually. **ISSUE-112 closeout ATTEMPTED 2026-09-02 (Pass 15, runbook §24) — ISSUE-112 STAYS OPEN.** G2/G4/G7/G8 PASS, G5 PASS-in-shape, **G3 PARTIAL**, **G6 BLOCKED** (the accepted fitzRoy/DraftGuru snapshot bytes are absent from every reachable checkout; re-acquiring is a scrape ISSUE-112 §12 forbids). The §7 AWARDS/HONOURS rebuild stage now exists (`awards-honours`, between DraftGuru and DERIVED, Stage-9 per-family row gates), the last two shared definitions and the 33 previously-unowned `rising-star` winner rows are tracked, and a **correctness defect was found and fixed**: the manifests' carried `player_id` denoted a different footballer after a canonical rebuild in **12,392 of 12,392** cases, so links now resolve through the tracked AFL Tables profile identity (`data/awards/player-identity.csv`), fail-closed. **ISSUE-102 cannot close until ISSUE-112 does.** Extraction-source prerequisite (ISSUE-112 §11.1) is **DECIDED 2026-09-01**: the legacy-loaded AFLDB PostgreSQL state, not a fresh scrape. One systemic pre-merge operator decision remains for ISSUE-112 — `source_citation` granularity (no per-row page citation survives in PostgreSQL; no scrape proposed). ISSUE-111's gate is closed: it is Resolved. **No implementation is authorised under ISSUE-102 itself.** |
| `AFLDB-ISSUE-112` | Medium | Data acquisition / Import architecture / Data integrity | **Child of ISSUE-102.** Replace the legacy SQLite input of the six legacy-dependent `import_awards.py` groups with checked-in, validated, reviewable **curated manifests** under `data/awards/`, in the 22 Under 22 mould. Runbook `issues/open/AFLDB-ISSUE-112.md`. Seven families: All-Australian, Hall of Fame, honour teams, captaincies, Rising Star, club best-and-fairest, named medals — plus the award definitions and the `person_links` bridge (`import_awards.py:1364-1376`, replace with the tracked DraftGuru ledger). **Scraping, HTML parsing, paid APIs and undocumented endpoints are NOT authorised.** Only the **input** changes: every `reload_keyed` call keeps its key, columns, ownership scopes and flags, so id/manual-link/`confirmed_unlinked` preservation, the name-change guard, collision refusal and the `(717275, 1)` lock all carry over. `data/reference/` is the wrong home (`load_reference_data.py` TRUNCATEs, no link handling). Grains differ — do not force one schema on all seven. Watch: `hall_of_fame` and `honour_team_members` reload on **natural** keys and have no `source_record_id`; do not add one (migration 059). Extraction source **DECIDED 2026-09-01** (runbook §11.1): the legacy-loaded AFLDB PostgreSQL state, not a fresh scrape. **Gate G0 — DONE 2026-09-01 (runbook §14.4/§14.6): all nine families PASS**, measured read-only against `afldb_dev` via the streamanator dev server (connection proven, transaction rolled back). Every `season`-bearing family has `source_record_id` complete and unique; every natural-key probe returned zero true collisions; the 74-row awards-scoped `player_link_resolutions` ledger is orphan-clean; `player_link_suggestions` empty. Structure-only lossless assessment (§14.3) confirmed by measurement — exactly three predicted gaps, no new one; the one systemic gap was `source_citation` (no `source_url` on any of the five link-target tables; recoverable only at source granularity — `draftguru`/`wikipedia`/`footywire`) — **DECIDED 2026-09-01 for the honour-teams slice: source-granularity `wikipedia`, not a per-row page citation.** **Honour-teams slice (§11.2 phase 1) — IMPLEMENTED 2026-09-01 (Pass 7, runbook §16).** A dedicated SSH key (`~/.ssh/afldb_dev`) unblocked Pass 6's connection failure; the 113-row bootstrap was extracted read-only and proven, matching G0 exactly. `data/awards/honour-teams.csv` (113 rows, minted `honourteam:<slug>:<seq>` ids) and `tools/migration/honour_teams.py` (validating DB-free parser) exist; `import_honour_teams()` in `import_awards.py` now reads the manifest instead of legacy SQLite `team_selections`, with `reload_keyed`/advisory-lock/collision semantics byte-identical to before; `"honour_teams"` is now in `LEGACY_FREE_GROUPS`. DB-free tests 22/22 pass, `npx tsc --noEmit` clean, `git diff --check` clean. **Real DB-backed integration validation EXECUTED and GREEN 2026-09-01 (Pass 8, runbook §17):** the new `honour-teams manifest reload (AFLDB-ISSUE-112)` describe block ran for real (not skipped) against `afldb_test` under the restricted `afldb_import` role — 6/6 focused tests passed: 113-row parity, 3x idempotent reload byte-identical fingerprint, the one explicit `player_link_resolutions` decision (Ted Whitten) survives, all 24 unlinked rows stay unlinked, a `manual_admin_edit` row survives untouched, and `hall_of_fame`/`captaincies`/`award_winners`/`award_nominations` row counts are unchanged. Run from the Windows worktree via a temporary SSH tunnel to streamanator's PostgreSQL only (no file touched on the streamanator checkout); `AFLDB_TEST_IMPORT_DATABASE_URL` derived ephemerally in-process (not persisted) since it remains unconfigured anywhere reachable. Carried-forward risk, not a slice blocker: linked manifest rows carry `afldb_dev` `player_id` verbatim, not rebuild-stable (ISSUE-111 G5) — recorded against the deferred §7 canonical-rebuild stage; `source_key`, not `player_id`, is the manifest's durable identity. **Hall of Fame slice (§11.2 phase 2) — IMPLEMENTED and DB-validated 2026-09-01 (Pass 9, runbook §18).** 343-row bootstrap extracted read-only from `afldb_dev` (proven `db=afldb_dev`, `txn_read_only=on`), matching G0 exactly. `data/awards/hall-of-fame.csv` (343 rows, minted `hof:<seq>` internal keys — not the DB reload key `(name, inducted_year)`, ordered `inducted_year NULLS LAST, name COLLATE "C"`), `tools/migration/hall_of_fame.py` (validating DB-free parser, `source_citation = wikipedia` for all rows per the same source-granularity policy). `import_hall_of_fame()` now reads the manifest instead of legacy SQLite; `reload_keyed` key/columns/scope/`refuse_out_of_scope_key`/advisory behaviour byte-identical; `"hall_of_fame"` added to `LEGACY_FREE_GROUPS` + `BATCH_SOURCE_KEYS`. DB-free 24/24, `npx tsc --noEmit` clean, `git diff --check` clean; the new `hall-of-fame manifest reload (AFLDB-ISSUE-112)` integration block ran for real against `afldb_test` under the restricted `afldb_import` role — **6/6 green** (343-row parity, 3x idempotent fingerprint, all five `player_link_resolutions` decisions survive, all 97 name-only rows stay unlinked, `manual_admin_edit` row untouched, other families' row counts unchanged); honour-teams block re-run alongside, still 6/6. **Captaincies slice (§11.2 phase 3) — IMPLEMENTED and DB-validated 2026-09-01 (Pass 10, runbook §19).** 1,375-row bootstrap extracted read-only from `afldb_dev` (proven), matching G0 exactly. `data/awards/captaincies.csv` — **`source_key` is the preserved `source_record_id` carried verbatim, NOT re-minted** (captaincies already has a stable id and reloads on `(source_id, source_record_id)`); `club` is the canonical `clubs.name` re-resolved season-aware by `ClubResolver` (all 1,375 rows proved to round-trip → a rebuild-stable club identity, not a frozen `club_id`). `tools/migration/captaincies.py` (validating DB-free parser). `import_captaincies()` reads the manifest and **lost its `lite` parameter**; `_refuse_captaincy_natural_key_collisions()` and `captaincies_natural_uq` semantics unchanged; `reload_keyed` call byte-identical; `"captaincies"` added to `LEGACY_FREE_GROUPS` + `BATCH_SOURCE_KEYS`. DB-free 22/22, `npx tsc --noEmit` clean, `git diff --check` clean; new `captaincies manifest reload (AFLDB-ISSUE-112)` integration block **8/8 green** against `afldb_test` under `afldb_import` (1,375-row parity, era re-resolution, 3x idempotent fingerprint, resolved-link id-stability, all rows stay linked, `manual_admin_edit` protection + natural-key collision fail-closed — both carrying the retired synthetic-SQLite fixture's AFLDB-ISSUE-085 coverage manifest-driven — other families unchanged); combined ISSUE-112 regression (honour teams + HoF + captaincies) **20/20**; whole `awards-reload-links.test.ts` **50 passed / 21 skipped / 0 failed** (skips are the pre-existing `AFLDB_LEGACY_SQLITE`-gated blocks). **Rising Star slice (§11.2 phase 4) — IMPLEMENTED and DB-validated 2026-09-01 (Pass 11, runbook §20).** 766-row bootstrap extracted read-only from `afldb_dev` (proven), matching G0 exactly. `data/awards/rising-star.csv` — **`source_key` is the preserved `source_record_id` (24-hex digest) carried verbatim, NOT re-minted** (Rising Star reloads on `(source_id, source_record_id)`); `club`/`opponent` are canonical `clubs.name` re-resolved season-aware by `ClubResolver` (all 765/763 non-null rows round-trip; 1 NULL club + 3 NULL opponent preserved); `stat_line` jsonb carried losslessly (763 present / 3 NULL; parser rejects malformed/wrong-shape rather than coercing). `tools/migration/rising_star.py` (validating DB-free parser). `import_rising_star()` reads the manifest and **lost its `lite` parameter**; `reload_keyed` call (key `(source_id, source_record_id)`, `scope_column="award_id"`, AFLDB-ISSUE-080 `source_id` provenance scope) byte-identical; `"rising_star"` added to `LEGACY_FREE_GROUPS` + `BATCH_SOURCE_KEYS`, and its `GROUP_REQUIRES → {"awards"}` entry removed so `--groups rising_star` runs legacy-free (reverse `awards`-refresh closure kept). DB-free 33/33 (+ orchestration-contract suites 83/83), `npx tsc --noEmit` clean, `git diff --check` clean; new `rising-star manifest reload (AFLDB-ISSUE-112)` integration block **9/9 green** against `afldb_test` under `afldb_import` (766-row parity, era re-resolution ×6, NULL club/opponent preserved, `stat_line` byte-round-trip, 3× idempotent fingerprint, resolved-link id-stability, link dropped only for unresolvable `player_id` — 13 such 2026-debut rows against a staler `afldb_test`, all `unmatched` — `manual_admin_edit` protection, other families unchanged); combined ISSUE-112 regression **29/29**; whole `awards-reload-links.test.ts` **59 passed / 21 skipped / 0 failed**. **All-Australian slice (§11.2 phase 5) — IMPLEMENTED and DB-validated 2026-09-02 (Pass 12, runbook §21).** G0 re-measured read-only against `afldb_dev` (proven): the "2,158" figure is the legacy raw-table sum — the post-merge `award_winners` count is **1,158** (1953–2025, 53 seasons, linked 1,078 / unlinked 80, `source_record_id` complete+unique, **draftguru 906 + wikipedia 252**, 10 `(season,player)` dup pairs = 9× 1984 club/state dual + Josh Kennedy 2016) — every authoritative earlier fact re-confirmed exactly. `data/awards/all-australian.csv` (1,158 rows) — **`source_key` is the preserved `source_record_id` verbatim** (`aa:YYYY:n` / `aah:YYYY:player:club`, not re-minted); **two provenance sources kept distinct** (per-row `source` selects `source_id`, `source_citation == source`); `club` is the source's verbatim string re-resolved season-aware by `ClubResolver` (`club_id` reconstructed, `club_name_raw` byte-identical; 149 state-league/interstate sides → NULL deterministically). `tools/migration/all_australian.py` (validating DB-free parser; enforces `source_key` + `(season,player,club)` uniqueness but **not** `(season,player)`, so the 1984 dual + Kennedy rows survive distinct). `import_all_australian()` reads the manifest and **lost its `lite` and `person_links` parameters** (the two-table merge is gone); `reload_keyed` call + post-reload span `UPDATE` byte-identical; `"all_australian"` added to `LEGACY_FREE_GROUPS` + `BATCH_SOURCE_KEYS`, `GROUP_REQUIRES → {"awards"}` removed so `--groups all_australian` runs legacy-free (reverse closure kept, definition guard kept). DB-free 40/40 (191/191 across touched suites), `npx tsc --noEmit` clean, `git diff --check` clean; new `all-australian manifest reload (AFLDB-ISSUE-112)` integration block **8/8 green** against `afldb_test` under `afldb_import` (1,158-row parity with the 906/252 split, era re-resolution ×6 incl. "Western Bulldogs"→Footscray/1986 and "North Melbourne"→Kangaroos/1999 clamps, 1984 dual + Kennedy rows distinct, 3× idempotent fingerprint, carried link state + G5-shape decision integrity, `manual_admin_edit` protection, other families/honours tables unchanged); whole `awards-reload-links.test.ts` **67 passed / 21 skipped / 0 failed** (+8 over Pass 11, slices 1–4 not regressed). **Club best-and-fairest slice (§11.2 phase 6) — IMPLEMENTED and DB-validated 2026-09-02 (Pass 13, runbook §22).** G0 re-measured read-only against `afldb_dev` (proven) — every authoritative figure re-confirmed exactly (752 rows, 1980–2025, 46 seasons, 19 `bf-*` slugs, linked 744 / unlinked 8, `source_record_id` complete+unique, draftguru for all, `votes` empty on all). **Two tracked manifests:** `data/awards/club-best-and-fairest.csv` (752 winner rows; `source_key` = preserved `source_record_id` verbatim; `club` re-resolved season-aware by `ClubResolver` → rebuild-stable `club_id` + byte-identical `club_name_raw`; `votes` refused; `source_citation` = `draftguru`) and `data/awards/club-best-and-fairest-definitions.csv` (19 `bf-*` award rows — this is where the family becomes *definitionally* legacy-free). `tools/migration/club_best_and_fairest.py` (validating DB-free parser for both + a `validate_family()` span cross-check; guards `(award_slug, season, player)` not `(award_slug, season)`, so the 25 tied B&F seasons survive as multiple rows). **Award-definition decoupling:** a new legacy-free `club_bf` group reconciles the 19 definitions from its own manifest via a slug-scoped id-preserving `reload_keyed`; the legacy `awards` group's shared `build_definitions()` and its `reload_keyed` scope are left **byte-identical** (`bf-*` still emitted there, two id-stable writers agree, like the `all-australian` definition) — so **named-medal definition semantics are provably untouched**. `import_awards()` adds the 19 `bf-*` `award_id`s to `other_group_awards` (the `under_22`/`all_australian`/`coleman` mechanism). `"club_bf"` added to `LEGACY_FREE_GROUPS` + `BATCH_SOURCE_KEYS` + `GROUP_ORDER` + the `GROUP_REQUIRES["awards"]` closure (not the reverse). No migration, no privilege change. DB-free 37/37 (240/240 across touched suites), `npx tsc --noEmit` clean, `git diff --check` clean; new `club best-and-fairest manifest reload (AFLDB-ISSUE-112)` integration block **9/9 green** against `afldb_test` under `afldb_import` (752-row + 19-definition parity, each definition span == its winners', era re-resolution ×7, no NULL `club_id`, tied 2003 Adelaide season loads as two rows, 3× idempotent fingerprint, carried link state + G5-shape decision integrity, link dropped only for unresolvable `player_id`, `manual_admin_edit` protection, other families unchanged); whole `awards-reload-links.test.ts` **76 passed / 21 skipped / 0 failed** (+9 over Pass 12, slices 1–5 not regressed). **Named medals slice (§11.2 phase 7, the last) — IMPLEMENTED and DB-validated 2026-09-02 (Pass 14, runbook §23).** G0 re-measured read-only against `afldb_dev` (proven) — every authoritative figure re-confirmed exactly (979 rows, 1976–2025, 50 seasons, 17 slugs [16 `award` + `national-draft-pick-1`], linked 863 / unlinked 116, `source_record_id` complete+unique, draftguru for all, 299 rows without a club, Brownlow medallist 53 rows/46 seasons with 7 tie extras). New this family: 53 Brownlow rows carry a `votes` tally (`NN.00`) and no other slug does; one `(slug,season,player)` collision (2013 40-Man Squad's two "Josh Kennedy"s) → collision-free identity is `(award_slug,season,player,club)`. **Two tracked manifests:** `data/awards/named-medals.csv` (979 winner rows; `source_key` = preserved `source_record_id` verbatim; `club` re-resolved season-aware by `ClubResolver`, empty → NULL; `votes` carried for Brownlow rows, refused elsewhere; `source_citation` = `draftguru`) and `data/awards/named-medals-definitions.csv` (17 award rows). `tools/migration/named_medals.py` (validating DB-free parser for both + `validate_family()` span cross-check). **Award-definition decoupling:** new legacy-free `named_medals` group reconciles the 17 definitions from its own manifest via slug-scoped id-preserving `reload_keyed`; the legacy `awards` group's `build_definitions()` and its `reload_keyed` scope left **byte-identical** (co-emission, like club_bf / all-australian) — so the legacy `awards` group **keeps its genuine remaining job: creating the `all-australian` / `rising-star` / `coleman` (+ 2nd `honour_team`) definitions, which no manifest family owns**. `import_awards()` adds the 17 named-medal `award_id`s to `other_group_awards` by slug; its `build_winners()` now emits zero rows (a documented no-op). `"named_medals"` added to `GROUPS` / `LEGACY_FREE_GROUPS` / `BATCH_SOURCE_KEYS` / `GROUP_ORDER` / `GROUP_REQUIRES["awards"]` closure (not the reverse). No migration, no privilege change. DB-free 44/44 (284/284 across touched suites), `npx tsc --noEmit` clean, `git diff --check` clean; new `named-medals manifest reload (AFLDB-ISSUE-112)` integration block **10/10 green** against `afldb_test` under `afldb_import` (979-row + 17-definition parity, each definition span == its winners', era re-resolution ×7, `club_id` 680 / NULL 299 exact, 2003 Brownlow loads as 3 distinct rows each with its votes tally, votes on exactly the 53 Brownlow rows and 0 others, 3× idempotent fingerprint incl. `votes`/`note`, carried link state + G5-shape decision integrity, link dropped only for unresolvable `player_id`, `manual_admin_edit` protection, other families/honours tables unchanged); whole `awards-reload-links.test.ts` **86 passed / 21 skipped / 0 failed** (+10 over Pass 13, slices 1–6 not regressed). **All seven families are now manifest-backed and each runs legacy-free individually. Next action: the ISSUE-112 CLOSEOUT** (not done Pass 14) — (1) G2/G3 at ISSUE-112 level: one `import_awards.py` run over all seven manifest families with `AFLDB_LEGACY_SQLITE` unset + the full `awards-reload-links.test.ts` matrix with no legacy gate; (2) the §7 canonical-rebuild AWARDS/HONOURS stage in `tools/db/rebuild-test.ts` — give the `all-australian` / `rising-star` / `coleman` **definitions** a tracked home so the legacy `awards` group leaves the rebuild, with Stage-9 per-family gates (979/752/1,158/766/1,375/343/113); (3) G5 link audit, G7 (`docs/deployment.md §7`), G8 (`privileges.test.ts` unchanged). **CLOSEOUT ATTEMPTED 2026-09-02 (Pass 15, runbook §24) — ISSUE-112 STAYS OPEN on two operator inputs.** The recorded boundary was wrong: the legacy `awards` winner reload was NOT a no-op — `rising-star` was still in its scope and `afldb_dev` holds **33 `rising-star` `award_winners` rows** no manifest owned; and it was the sole creator of exactly **two** definitions (`all-australian`, `rising-star`), not four. Both now tracked: `data/awards/award-definitions.csv` (one shared file, two DISJOINT slug-scoped writers, no double ownership) and `data/awards/rising-star-winners.csv` (33 rows, `source_key` preserved verbatim, `draftguru` provenance not the nominations' `footywire`, cross-checked against the nominations on player identity per decided season). `RISING_STAR_SLUG` added to `other_group_awards`, so the legacy winner reload now genuinely matches nothing; the legacy `awards` group is retained **compatibility-only** and still needs `AFLDB_LEGACY_SQLITE`. **CORRECTNESS DEFECT FOUND AND FIXED:** `players.id` is not merely unstable across a canonical rebuild — the carried ids were WRONG. Against the canonically rebuilt `afldb_test`, **0 of 12,392** shared ids denote the same footballer, and the loaders' `player_id in valid_players` guard kept every link, so **5,141 of 5,194 links would have been silently attached to a different player**; Hall of Fame / honour teams / captaincies had no guard at all and were already loading wrong links in Passes 8–14. Fix: `data/awards/player-identity.csv` (1,738 censused players) + `tools/migration/player_identity.py` + `import_awards.PlayerResolver`, resolving through the normalised AFL Tables profile path in `external_identities` and failing closed — refuses an uncensused id, loads unlinked and *reports* a player with no identity, never falls back to the bootstrap id, never matches on a name. No manifest or manifest parser rewritten. Result: **5,138/5,194 links preserved (98.9%)**, 56 lost and every one enumerated (captaincies 1,375/1,375, AA 1,065/1,078, Rising Star 750/766, club B&F 735/744, named medals 856/863, HoF 239/246, honour teams 85/89). **§7 canonical-rebuild stage implemented:** `awards-honours` in `tools/db/rebuild-test.ts` between DraftGuru and DERIVED, running the eight manifest groups with no legacy source; Stage-9 per-family ROW gates (113/343/1,375/766/33/1,158/752/979/330, 39 definitions ex-Coleman, 0 source-less winners); Coleman keeps its own later derived stage (ISSUE-111 ordering/semantics/ownership untouched). **Gate results:** G1 PASS · **G2 PASS** (one `import_awards.py` run over all eight manifest groups with `AFLDB_LEGACY_SQLITE` unset, 61 s, no self-skip) · **G3 PARTIAL** (`awards-reload-links.test.ts` **86 passed / 21 skipped / 0 failed**; the 21 skips are the pre-existing legacy-*fixture* blocks that build a synthetic SQLite file — not ISSUE-112 paths) · **G4 PASS** (3× byte-identical fingerprints over `award_winners` 3,298 / `award_nominations` 766 / `hall_of_fame` 343 / `honour_team_members` 113 / `captaincies` 1,375 / `awards` 40) · **G5 PASS in shape, vacuous in ledger** (`afldb_test` holds no `player_link_resolutions`) · **G6 BLOCKED** · **G7 PASS** (`docs/deployment.md` §6/§7 legacy-free; no production deploy claimed) · **G8 PASS** (`privileges.test.ts` 34/34 unchanged). Also: DB-free **606 passed / 6 skipped** over 15 suites, `db-test-rebuild.test.ts` 223/223, release-gates 51 passed/14 skipped, `npx tsc --noEmit` clean, `git diff --check` clean, ISSUE-111 not regressed (Coleman 46 rows intact). **BLOCKER 1 (G6): the canonical rebuild cannot be executed** — `import_draftguru.py --validate-only` REFUSES (`data/sources/draftguru/annual-html-20260826` absent) and `validate_ladder_witness.py` REFUSES (`full-history-20260827` bytes absent); neither snapshot is in the worktree or on streamanator (only `trial-2024`), and re-acquiring is a scrape §12 forbids. **BLOCKER 2: 19 players (37 manifest rows) have no rebuild-stable identity** and load unlinked, enumerated in the census; accepting or adjudicating them is an operator decision. **Next action:** (1) operator supplies the accepted fitzRoy/DraftGuru snapshot bytes and runs `npm run db:test:rebuild -- --fitzroy-label full-history-20260827 --acknowledge-destroy afldb_test`, confirming Stage 8 and the new Stage-12 gates → closes G6; (2) operator decides the 19 unidentified players; (3) decide whether G3's 21 legacy-fixture blocks are inside this issue's closure contract at all; (4) only then resolve ISSUE-112. |
...
<!-- No open rows follow. Everything below is retired lineage only: ISSUE-106 (retired
     2026-08-29) and ISSUE-093 (retired 2026-08-27). The open issues are ISSUE-102, -110, -112,
     -113, -116 and -104, listed above. -->
