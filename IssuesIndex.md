...
| `AFLDB-ISSUE-153` | Medium | Public UI / record pages / NL deferred semantics / `src/search/nl/*` | **OPEN — STAGES 1–5 AND 7 ARE IMPLEMENTED AND NOT MERGED. Branch `opus/issue-153-nl-deferred-semantics` (worktree `D:\dev\afldb-issue-153`) holds 4 commits through `b493a48` and 3,441 insertions `main` does not have (`src/search/nl/{parser,plan,vocab,describe}.ts`, `src/db/queries/nl/father-son-summary.ts`, `src/db/queries/family-records.ts`, both record pages, `tests/family-records-surfaces.test.ts`, `tests/integration/nl-answers-relationships.test.ts`, the Stage-0 evidence SQL, runbook `issues/open/AFLDB-ISSUE-153.md`). That worktree ALSO holds ~690 lines of UNCOMMITTED tracked changes (16 files) recording a 2026-09-10 rendered acceptance of the re-pinned corpora (`current` 319/319, `next` 349/349, every transport counter at zero) — at risk until committed. STAGE 6 (C1/D6/C5/C6) IS NOT STARTED AND NOT AUTHORISED (D6 unlocked). Known conflict, deliberately unresolved: five rows pinned as DECLINES in the ISSUE-152 frozen corpora (`rel_dec_003`, `rel_dec_004`, `rel_dec_005`, `rel_dec_006`, `xd_dec_014`) now legitimately answer. Nothing merged, nothing deployed.** Split out of `AFLDB-ISSUE-152` Stage-0 Finding F1 by operator decision 2026-09-08. **(1)** `/records/father-son` â†’ `getFatherSonRecords` (`src/db/queries/family-records.ts:130`) reads `player_relationships WHERE relationship = 'parent_child'` and **never touches `father_son_selections`**, while the Grid Solver's `father_son_selection` / `father_son_father` builders (`src/db/queries/grid-solver.ts:1279`) read that AFL draft-rule table â€” two meanings of "father-son" in one product, over two tables, with no cross-reference. **(2)** `/records/family` â†’ `getFamilyRecords` (`src/db/queries/family-records.ts:39`) groups **every** `relationship_type` by `family_key` with no `relationship` filter, while `src/app/records/family/page.tsx:15,99` says "a linked family of **siblings**" â€” a family linked only by a cousin, in-law or spouse row is counted and described as siblings. The stricter evidenced reading already exists in the codebase as `has_brother` (`relationship = 'sibling'` AND `relationship_label IN ('brothers','twin brothers')` AND the other side linked with games > 0). **SIZED AGAINST DATA 2026-09-08** (`ISSUE-152-nl-evidence-output.txt` Â§3.1/Â§3.2/Â§3.5/Â§4.1, `afldb_test`, read-only): **both halves are LATENT, not active.** `player_relationships` holds only `sibling` (498 rows, 378 `family_key`s) and `parent_child` (127 rows, **0 `family_key`s**) â€” zero cousin/in-law/spouse/grandparent/aunt-uncle/other rows â€” so `getFamilyRecords`' `family_key` grouping cannot presently return a non-sibling family, and `/records/family` is correct in fact but wrong by construction. `father_son_selections` (127 / 96 linked / 31 not) and `parent_child` (127 / 96 / 31) are **the same population**, every `parent_child` row carrying the label `father and son (AFL fatherâ€“son rule selection)`, so `/records/father-son` is not serving a wrong set today â€” but nothing enforces it, and **no witness exists that distinguishes the two readings**. Labels measured: `brothers` 467, `siblings` 13, `sisters` 8, `twin brothers` 7, `twins` 3. **Unmeasured (M5):** combined career games per family â€” the metric the page actually orders by (Â§3.5 ranked by member count; largest family `ablett-0004` has 5 linked players but only 4 distinct names, ids 4700/4701 both "Gary Ablett"). This is a **semantic** decision, not a code-only fix: correct each page's prose, narrow each query, or split the boards â€” three defensible resolutions producing different public pages, and narrowing the father-son board would also need the unlinked-father rule decided. `AFLDB-ISSUE-152` depends on this for C1/FS1â€“FS3/FS6 and decisions D6/D8, but is **not blocked** by it for Phases B, C or E. Key files: `src/db/queries/family-records.ts`, `src/app/records/family/page.tsx`, `src/app/records/father-son/page.tsx`, `src/db/queries/grid-solver.ts`. | **Operator:** the sizing run is done and Stages 1–5 + 7 are implemented. In order: (1) commit the at-risk uncommitted work in `D:\dev\afldb-issue-153`; (2) settle the five-row corpus reclassification against the ISSUE-152 frozen sets (a product decision, not a code fix); (3) lock D6 before any Stage 6 work; (4) merge, and resolve only when both pages' prose and queries agree and the boards are eyeballed on DEV. |
