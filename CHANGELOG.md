...
### Staged promotion accepts a contract-permitted empty table on stage-completion evidence (AFLDB-ISSUE-247) (Open, uncommitted) - 25 September 2026
...
- **Validation.** DB-free: typecheck clean, `tests/db-promotion-check.test.ts` 201/201, ESLint
  ...
  rollback-only, with zero residue, on 2026-09-26. The A5 rerun is pending.
...
- **Acceptance (live DEV, operator-run, 2026-09-25, at `4bb23a8f`).**
  - A mandatory `afldb_dev` backup was taken first.
  - Validate-only returned WOULD_RETIRE. `--apply` returned RETIRED, with retirement audit
    `auth_audit_log` 983 and 2 writes. The rerun returned ALREADY_RETIRED on the same 983, with 0
    writes.
  - Override `id 1` is inactive, with its payload, author and `created_at` preserved. The canonical
    fixture match is still absent.
  - `data_edits` and the `data_overrides` count are unchanged, and `auth_audit_log` grew by
    exactly one row.
...
- **Read-only pre-import PLAN / post-import VERIFY gates** for the person-page bridge import into `afldb_test`: `tools/rebuild/draftguru/bridge_import_gate.py`. Reads only `AFLDB_TEST_DATABASE_URL` (database must be exactly `afldb_test`, confirmed server-side), forces and asserts `default_transaction_read_only=on` / `transaction_read_only=on`, sends `SELECT` only through a refusing cursor wrapper, rolls back and closes unconditionally, writes no artefact and never prints the DSN. It replays the importer's own `apply_authority()` over the accepted snapshot with seeding forbidden, classifies every person (newly linked / already agreeing / conflicting / missing / extra / duplicate-ambiguous / unexpected state) and refuses on anything but the planned link changes; prints exact post-import totals and ordered content hashes (`after_state`, `picks_after`, `newly_linked` manifest, `changes`, unrelated-table `baseline` digests). `verify` proves only the planned rows changed, each bridge resolves to its intended player, the 1,589 withheld persons stay unlinked, the two rejected identities cannot re-enter, no duplicate person or target, the link vocabulary, the batch row and the baseline digests. Contract `tests/python/draftguru_import_gate_contract.py`; Vitest wrappers in `tests/draftguru-acquisition.test.ts` and `tests/draftguru-import.test.ts`.
- **`tools/maintenance/backup-afldb-test.ps1`** — workstation twin of `backup.sh` for the one database that script cannot be pointed at (it sources `.env`, whose backup DSN names `afldb_dev`): same `pg_dump` contract, partial-then-rename, `pg_restore --list` verification, SHA-256, password via `PGPASSWORD` only, refuses any DSN not naming `afldb_test`, writes outside the repository.
- **`afldb_test` now carries the bridge (operator-executed 2026-09-19).** After a catalogue-verified backup, the read-only plan, a dry run and a re-plan, `import_draftguru.py --no-seed --bridge` linked 3,465 DraftGuru persons through the person-page bridge (authority ledger 6 / bridge 3,465 / unmatched 1,587 / seeded 0); `bridge_import_gate.py verify` passed twice with an identical digest: 3,470 linked persons, 5,115 of 6,810 picks linked (75.11%, up from 5), the 1,589 withheld persons unlinked, the two rejected identities absent, every unrelated-table digest unchanged. Grid Solver draft criteria therefore have a real population on `afldb_test` (the `draftLinks` probe is crossed natively). DEV and PROD are unchanged; the 94 `target_not_registered` persons are AFLDB-ISSUE-224.
- **`--dry-run` message corrected.** The importer said "nothing was written", but `import_batch()` retains a committed `failed` batch row (error `DryRunComplete: `) on every dry run; it now states that the data writes were rolled back and that audit row is retained (`DRY_RUN_MESSAGE`, pinned by the atomicity contract).
- **Read-only validation of the linked population:** `tests/integration/draft-linkage.test.ts` (SELECT-only; expectations derived from the child, ledger and Stage A manifest; proves §6.1/§6.4 and the six draft builders through the production compiler) and `tools/rebuild/draftguru/afldb_test_draft_linkage_checks.sql` (one rolled-back `READ ONLY` transaction). The mutating suites (`grid-solver.test.ts` fixtures, `draftguru-import.test.ts`) are documented as not being verification steps. The bridge-link invariant lives in `tests/draft-linkage-invariants.ts` and is asserted child-target → registration → player; its first version joined `draft_persons.player_id` to every AFL Tables identity and fanned out onto the continuing path of the four ISSUE-136 renumbered players (a false failure on `afldb_test`, no wrong link) — `tests/draft-linkage-invariants.test.ts` pins that shape DB-free.
- **Gridley corpus on the linked population (operator-run 2026-09-19, read-only; report sha256 `7f14ff2c…`).** The six draft criteria scored natively for the first time: 1,163/1,166, 99 `incorrect known answer` cells — 62 on nine linked players (Brad Crouch's 2011 Mini-Draft pick 2, which Gridley's "National Draft" TOP 5/TOP 10 counts and `national_draft_pick_between` deliberately does not; eight players Gridley lists as Rookie-drafted whose linked DraftGuru pages carry no Rookie event) and 37 non-draft cells that pre-date the import. Nothing was reclassified. The corpus suite now appends a triage from the player's own trusted-linked `draft_picks` rows to every draft-criterion finding (`tests/gridley-corpus-support.ts`, pure), and its player resolver distinguishes a debutant the player register does not yet hold (2026 matches present, register at the 2025 baseline — AFLDB-ISSUE-224) from a bad override instead of failing the criteria test. `normalisePlayerName` now folds a no-break space to a space before stripping (it used to fuse "Jagga Smith" into one token). Pinned DB-free in `tests/gridley-compat.test.ts`. Phase 4a acceptance is not claimed.
- **Corpus decisions (operator, 2026-09-19).** A National Draft pick-range cell where Gridley lists a linked player whose only trusted-linked pick inside the range is a non-national selection (Brad Crouch, 2011 Mini-Draft pick 2, TOP 5 / TOP 10) is now classified `external source disagreement` with the row named — one evidence-keyed rule, no change to the national-only builder and no player exception. The 37 pre-existing non-draft cells are tracked as AFLDB-ISSUE-225; the two unresolved 2026 embedded players are confirmed by a read-only query to be AFLDB-ISSUE-224 registration gaps (2026 matches present, player register at 2025, no NBSP names).
- **The eight `pickrookie` disagreements resolved by independent-source review (2026-09-19).** Adam Schneider, Lachie Henderson, Phil Davis, Sydney's Sam Reid, Lewis Taylor, Paul Seedsman, Michael Rischitelli and Bryce Gibbs are each linked to a DraftGuru page carrying no Rookie Draft event; an independent source was opened and read for every one (six primary AFL/club sources; Phil Davis and Bryce Gibbs on reputable specialist/secondary sources, flagged as such; Michael Rischitelli's pick number from a secondary source only) confirming a Rookie Draft selection or administrative re-listing (Gibbs: pick 1, inactive, salary-cap purposes only) that DraftGuru's linked page omits — a DraftGuru source coverage gap, never a Gridley-key, trusted-link or draft-kind mapping error. Recorded per person in a new tracked outcome artefact keyed by AFL Tables profile (`data/players/rookie-relisting-outcomes.csv`) and read by one pure, evidence-keyed rule (`rookieSourceCoverageGap`, `tests/gridley-corpus-support.ts`) that fires only for a reviewed `gridley_supported` player on the `pickrookie` axis whose triage cause is `no_linked_row_matches` — no player named in the classifier itself, no general "missing Rookie row" rule, the Crouch (D1) rule untouched. The corresponding cells now classify `source coverage gap` (`tests/integration/gridley-corpus.test.ts`) instead of `incorrect known answer`, still failing strict acceptance and counted in diagnostic mode like the pre-existing `has_brother` case. Full review, citations and verdicts: `docs/rebuild-manifests/draftguru/rookie-relisting-independent-review-20260919-v1.md`.
- **Full Gridley corpus rerun confirms the linked population clean on every draft criterion (2026-09-19, operator-run, read-only; report sha256 `659e29ed…`).** With D1 and D2 both implemented, `incorrect known answer` collapsed from 99 to exactly **37** — the pre-existing AFLDB-ISSUE-225 residue, verified record-for-record against that issue's table (player, axis and cell count all match, no addition, no residue) with zero draft-criterion cells among them. `external source disagreement` 422, `source coverage gap` 77, `dataset gap` 48, `parse` 0, all matching the §11.19.13 projection exactly. **Scope decision:** AFLDB-ISSUE-222's core trusted-linkage objective is satisfied on this evidence; the §7.4 `afldb_test` reversal/rollback exercise is scoped to Phase 4a (the gate for a future Phase 4b DEV/PROD promotion of this dataset), not to the linkage objective itself, and is deferred alongside the rest of Phase 4a. No database write, rollback or Git action occurred this pass; the issue is kept open pending the operator's commit and the project's standard close-out (`merge:ready` → deploy → DEV smoke → close). Record: `AFLDB-ISSUE-222.md` §11.19.14.
- **`bridge_import_gate.py` generalised to `--target {test, dev}`; the §7.4 `afldb_test` rollback exercise defined but not run (2026-09-19).** The gate now takes an explicit `--target` (default `test`, every pre-existing invocation unchanged); `dev` reads a new, dedicated `AFLDB_DEV_DATABASE_URL` — never the importer's write DSN or the migration owner DSN — requires the connected database to be exactly `afldb_dev`, has no default `--bridge` (mandatory for `dev`, refused if it resolves to the `afldb_test` child), and refuses any target outside `{test, dev}` before any DSN is read; there is no PROD entry. Because this introduces a new sensitive DSN name, it was added to `.env.example`, `docs/deployment.md`'s credential table, and the `UnsetEnvironment=` deny list of all three units that load `.env` (`deploy/afldb.service`, `deploy/afldb-email-intake.service`, `deploy/afldb-settle-afltables.service`) so it is never exposed to a running service process — the ISSUE-220 credential-boundary test (`tests/deploy-web-unit.test.ts`) enforces this for the web unit. `tools/rebuild/draftguru/s74-snapshot.sql` (new, read-only `\copy` script) and the exact operator command sequence for the §7.4 S0–S3 rollback proof on `afldb_test` are recorded in `AFLDB-ISSUE-222.md` §11.19.15 — defined, not executed. `tests/python/draftguru_import_gate_contract.py` extended with explicit test/dev/unknown-target coverage. No database, Git, network or deployment command ran this pass.
- **`validate_person_bridge_child.py` generalised to `--target {test, dev}`; `afldb_dev` deployment child generated and independently validated (2026-09-19).** The child validator's `EXPECT` block and pinned lineage were hard-coded to the `afldb_test` child, so a run that omitted `--child` silently validated the `afldb_test` child against the DEV hash instead of validating DEV. `--target` is now explicit and closed: `test` (default) is unchanged byte-for-byte and still reproduces the pinned Phase F digest `5bc5336b…`; `dev` requires `--child`/`--expect-sha256`, refuses the `afldb_test` child by path, name and bytes, and requires the child's own `target` field to match the exporter's real DEV label. `data/reference/draftguru-person-bridge-20260918-v2.afldb_dev.json` (`a9652e4a…`, 3,468 bridges / 1,589 withheld) passes all 48 checks under `--target dev` (`summary_sha256 cd8d6da2…`). **Governance:** Phase F stays accepted at its original frozen source hash `35c41602…` (evidence `63c89265…`); this generalisation is Phase 4b deployment tooling and its DEV validation is additional evidence, not a Phase F regeneration — `validate_validation_review.py`'s check 2.5 tool-hash-drift result is the expected consequence of the post-acceptance tool change, not a newly failed review. No database import or deployment occurred; the DEV database is not yet imported.
- **`bridge_import_gate.py`: the child's deployment-target label and the physical database are now separate fields in the closed target specification (2026-09-19).** The first real DEV pre-import attempt got past `--validate-only` and a fully rolled-back `--dry-run` and was then refused by the read-only `plan` gate with `REFUSED: the child targets 'dev', not afldb_dev` — a fail-closed refusal on a wrong check. `load_child()` was comparing a deployment child's own `target` field with the **physical database name**, but that field is the exporter's `--resolve-against` label, and the vocabulary is asymmetric by accepted contract (`export_person_bridge.TARGET_DSN_ENV`: `afldb_test` → `afldb_test`, `dev` → `afldb_dev`), so the check only ever worked for the `test` target, where the two names coincide. `TARGETS` now carries `database` (the DSN path check and the `current_database()` assertion) and `child_target` (the artefact label) as distinct fields — `test` → `afldb_test`/`afldb_test`, `dev` → `afldb_dev`/`dev`, matching `validate_person_bridge_child.py`'s own labels — and `load_child()` takes `expect_child_target`, never a database name. The `afldb_test` child is now refused under any non-`test` target by resolved path, by the `.afldb_test.json` name **and** by its pinned bytes under any other name (`refuse_test_child_under`), so a rename cannot launder it into a DEV run. No alias was added and no child artefact was edited; the default `test` target, the gate's printed output and its hashed summary payload are unchanged, and every existing safety property (mandatory DEV `--bridge`, the separate `AFLDB_DEV_DATABASE_URL`, no import/owner DSN fallback, no PROD target, session read-only, `SELECT`-only cursor, unconditional rollback-and-close, no write of any kind) still holds. `tests/python/draftguru_import_gate_contract.py` pins the separated model, both cross-target refusals, a database-named (`afldb_dev`) child refused under `dev`, a server reporting `current_database() = 'dev'` refused, the `child_target` → `database` mapping read from the exporter's own source, and an end-to-end CLI `plan --target dev` over the real committed DEV child. **No real DraftGuru import has run against `afldb_dev`**; the pre-attempt `afldb_dev` backup is intact and the dry run's one retained `status=failed` / `DryRunComplete` audit row is the tool's designed audit trail, not a data change.

### Grid Solver: draft criteria review — honest "No data" squares, draft-kind vocabulary, list moves are not drafts, bounded parameters, form reset (AFLDB-ISSUE-221) - 17 September 2026

- **The reported failure is a data-linkage gap, not a compiler defect.** "Top 10 draft pick" (Gridley `picktop10` → `national_draft_pick_between(1, 10)`) compiles correctly, but on every current database only **5 of 6,810** `draft_picks` rows carry a trusted player link (the tracked human decisions; the legacy automatic linker was retired under AFLDB-ISSUE-093 and AFLDB-ISSUE-164's decision D-9 keeps unattended draft linking suspended), and none of the five is a top-10 national pick. Every `draft_picks` builder therefore returned an empty set and the page rendered "No answer" / "No player satisfies both axes" — a false statement about a pair the database never evaluated. The Grid Solver now tells the two apart: a square whose row or column question matches **no player at all** renders **"No data"** with the question named, both on the board and in the drill-down (`GridCellSummary.emptyAxis`; no extra query — the six axis sets are already fetched once per board). Linking the draft picks themselves stays governed by ISSUE-164 D-9 and is recorded as a follow-up on the issue.
- **Draft type is now the normalised `draft_kind`, not the source's raw label.** The dropdown offered "National" and "National Draft" as two separate choices because `draft_type_is` compared `draft_picks.draft_type` verbatim; the 1981/1982/1987 pages carry no Draft column and store those 113 rows as `'National Draft'`, so "National" silently missed three drafts while `national_draft_pick_between` (which already read `draft_kind`) did not. `GRID_DRAFT_TYPES` is now the ten frozen DraftGuru kinds (`data/reference/draftguru-event-kinds.json`) with form labels; the compiler resolves either a kind or a legacy raw label (`resolveDraftKind`) so existing share links and the Gridley rules (`pickrookie` → `rookie`, `freeagent1` → `free_agency`) keep their meaning, and an unrecognised value is refused rather than bound.
- **A trade or free-agency signing is not a draft selection.** `draft_picks` is the whole recruitment history and 929 trade / 128 free-agency rows carry the *receiving* club as `club_id`, so "Drafted by club", "Drafted by club, never played there" and "Drafted between years" would have counted a player traded to a club as drafted by it. Those three builders now exclude `draft_kind IN ('trade', 'free_agency')` (`IS DISTINCT FROM`, so a legacy row with no kind still counts); "Traded X+ times" and "Draft pick between" are unchanged (trades carry no pick number).
- **Gridley "FATHER SON PICK" reads the tracked father–son list.** The `fatherson` rule mapped to `recruited_via('Father-Son')`, i.e. `draft_picks.signing_kind`, which depends on the same 5-of-6,810 links, while its partner `fathersonfather` already read the fully linked `father_son_selections`; it now maps to `father_son_selection`, the source AFLDB-ISSUE-118 §7 named for it. (Corpus effect recorded on the issue.)
- **A bad number in a share link or the form no longer takes the whole page down.** A season of `199999` or a threshold of `99999999999` reached PostgreSQL as a bound value it rejects (SQLSTATE 22003, out of range for smallint/integer) — not a timeout, so the route rendered the error boundary. Every request number is now bounded at the compiler (thresholds, seasons, picks, votes, margins and heights to 0–32,767, the tightest column type any of them meets; ids to the integer range), the failure is a typed `GridAxisError`, and `guardCellTimeout` confines it to the squares that use the axis ("Invalid value" with the parameter's own message) while everything else — a permission error, a dead pool, a data exception the bounds should have made unreachable — still throws. Draft kinds and statistic keys refuse the same way.
- **Reset now resets the filters.** The form keeps the board in client state; on a soft navigation React preserved that state, so "Reset" cleared the board but left the six axis editors showing the old questions. The form is now keyed by the board token, so Reset and a shared link both remount it on the board the URL names.
- **Tests.** Two Grid Solver integration oracles that had been failing on `afldb_test` since the real 2026 Wildcard Final rows arrived (`is_final` where the solver deliberately reads `is_finals_series`, AFLDB-ISSUE-129 §8.4 — 3,658 vs 3,644 and 283 vs 282) now state the solver's own contract; the after-siren precondition on an unlinked winner is reported rather than required. New coverage: a committed, namespaced draft fixture (one player; a 1982 `'National Draft'` pick, a rookie pick, two trades and a free-agency signing on different clubs) proving every draft builder's reading; `GridAxisError` bounds and confinement; the draft-kind vocabulary pinned to the frozen contract; the Gridley draft rules pinned; `emptyAxis` on real data; page-source pins for the invalid and no-data renderings and the keyed form.

### NL stress corpus: corrected the three stale `verified_finals_without_premiership` rows (V5 → V6) and closed the Fable NL code review - 17 September 2026

- The frozen 12,000-row regression corpus's only failing rows under parser v66 (parse+execute) were a hand-verified answer that had gone stale against the data, not a parser defect: "most finals without a premiership" expected Nick Dal Santo alone (24, tie count 1), but Dane Rampe has since played his 24th finals-series match without a premiership (Sydney, 2026-09-05). Proven from current canonical data three ways — stored `player_career_stats`, an independent recount from `player_match_stats` ⋈ `matches` under the exact `rebuild_derived.py` rule, and the production `premierships = 0` leaderboard — with nobody else at 24, and a parser-v65 control producing the identical failures.
- Corrected with a new checked-in, fail-closed correction script, `tools/nl/fix-stale-finals-without-premiership-tie.ts` (same pattern as the AFLDB-ISSUE-199/201/204/205 scripts; unit-tested DB-free): exactly rows 41–43, `expected_answer_primary` → `Dane Rampe|Nick Dal Santo` (the corpus's own any-of-these lead form, in the query's deterministic tie order) and `expected_tie_count` 1 → 2, value 24 preserved, 0 non-target rows touched. V6 (`~/nl-stress-corpus-v6.csv`) scores **12000/12000 clean / 0 soft / 0 failed / 0 errors** on parser v66 and supersedes V5 as the stable baseline. No production NL code changed; `PARSER_VERSION` stays 66.
- With this, the Fable NL code review (AFLDB-ISSUE-187..197 and their 198..219 hardening lineage) received its final acceptance: **PASS**.

### NL search: recognised trailing-apostrophe plural club-alias possessives across five NL families (AFLDB-ISSUE-219) - 17 September 2026

- "What was Bombers' biggest victory against Pies", "Pies' highest seasonal wins in 2017", and "What is Bombers' longest winning streak against Pies" all declined outright with `unsupported_term: bombers'`/`pies'`, even though the club itself was already resolving correctly — `canonicalise()`'s existing possessive-stripping rule only matched an apostrophe immediately BEFORE a trailing "s" ("richmond's" → "richmond"), never a bare apostrophe AFTER an already-plural noun ("bombers'"). The defect was not in club matching at all: `extractClubs`'s word-boundary regex already matched correctly inside the punctuation-bearing token, and `scope.clubFor`/`clubAgainst` were already set — the divergence was entirely in the final leftover-token reconciliation, where the whitespace-split token `"bombers'"` never equalled the matched/consumed `"bombers"`, and the apostrophe-bearing token survived as an unexplained `unsupported_term`, declining a question the parser had otherwise fully understood.
- Confirmed a genuinely shared, cross-family `canonicalise()` defect rather than something owned by any one builder: five distinct grain families' exploratory-corpus templates use a plural-aware possessive helper that produces this exact form — `team_match_result`, `team_checkpoint_collision`, `q3_comeback_near_miss`, `club_season_rank` (the AFLDB-ISSUE-214 residual left unfixed for exactly this reason) and `team_streak` (the AFLDB-ISSUE-218 `team_match_result/0` cluster, independently found and deliberately deferred to this same future issue).
- Fixed with one new, generic strip in `canonicalise()` (`src/search/nl/vocab.ts`): a trailing apostrophe immediately before whitespace/end-of-string is now stripped the same way the existing `'s` rule already is — mirroring that rule rather than special-casing any club, venue, or alias. Apostrophes are not made globally ignorable: a mid-word apostrophe (`o'brien`) is untouched by construction, and an unknown word with a trailing apostrophe still declines exactly as before (now on the stripped spelling rather than the punctuated one). `PARSER_VERSION` 65 → 66.
- Operator-validated locally (`tests/nl-parser.test.ts` 605/605, broader NL gates 402/402, `typecheck` clean) and on `streamanator` against a temporary isolated `git worktree` (no established mechanism transported uncommitted code to a host before this issue; one was defined for it — a `git diff` patch applied to a worktree off `origin/dev`, with a clean `git apply --check` itself serving as the code-identity proof — the live dev checkout and the retained git stash were untouched throughout, and the temp worktree was removed afterward). Host suites reconfirmed 605/605 + 402/402 + clean typecheck. The exploratory V2 corpus (29,030 rows, the AFLDB-ISSUE-218-corrected file, parse+execute against the current database) moved 20512 → 20876 clean (+364 clean / -364 soft / 0 failed / 0 hard regressions), reconciling exactly across the five target families (`club_season_rank` 210, `team_streak` 75, `team_match_result` 56, `team_checkpoint_collision` 16, `q3_comeback_near_miss` 7) — every one of the 364 improved questions confirmed, by direct inspection, to carry a trailing-apostrophe alias, and none that don't. The frozen V1/"V5" 12,000-row corpus's only three failing rows (a stale `verified_finals_without_premiership` fact check, unrelated to club aliases) were proven, by an identical parser-v65-vs-v66 control against the same current database, to be pre-existing data drift rather than a regression introduced by this fix — recorded as a stale-corpus-expectation candidate for a future correction, not opened as its own issue. AFLDB-ISSUE-218's own resolved record and 19 of its 20 tests were left unmodified; its 20th test (a deliberately-deferred decline this issue exists to resolve) was updated in place to assert the new correct behaviour rather than deleted, preserving the cross-issue history. Implemented on `sonnet/issue-219-possessive-club-aliases` (unmerged).
