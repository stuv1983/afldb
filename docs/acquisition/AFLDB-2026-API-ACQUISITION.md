...
| S5 player provider bootstrap bridge | **COMPLETE**, 398/400 providers linked (afldb_test). *(Annotated 2026-09-22, ISSUE-244 F021.)* Current DEV full-season evidence, 2026-09-21 snapshot: **577 linked, 92 unresolved**. The `afldb_test` link set (398/400) and the DEV full-season evidence (577 linked / 92 unresolved) are **different datasets / evidence sets**, not a regression or a correction of one by the other; the earlier figure is retained as history. The 92 unresolved providers are owned by ISSUE-224. *(Annotated 2026-09-22, D-9b/§20: the 577/92 split was measured on the now-superseded snapshot `-011148`. It is **lineage, not S9 acceptance evidence**. S9's accepted bridge must re-resolve **all 669 providers in one run against `-031725`**, after ISSUE-224 registration and the DEV AFL Tables 2026 settle change the canonical substrate the bridge is built from; the 577 must not be carried forward.)* |
...
| S9 DEV validation (real-feed dry-run/apply) | **IMPLEMENTED / NOT ACCEPTED — PARTIALLY EXECUTED** *(reconciled 2026-09-22; supersedes "PAUSED before any real-feed acquisition")*. **ACCEPTANCE SNAPSHOT: D-9b RESOLVED 2026-09-22, option B — `afl-api-2026-2026-09-21-011148` is FORMALLY SUPERSEDED** (its bytes are absent from every local root) **by the retained, hash-verified `afl-api-2026-2026-09-21-031725`** (manifest sha256 `5018a3d6…`, 652/652 files byte-exact; zero measurable drift against the `-011148`-built bridge's retained 217-match / 669-provider / 9,983-row census — `issues/open/AFLDB-ISSUE-228.md` §20). Consequence: the accepted bridge must be **rebuilt as one coherent 669-provider population against `-031725`**, and the **577** links already on DEV are **lineage, NOT acceptance evidence** (no mixing with the 92). Historically: a real 2026 acquisition occurred on the now-superseded snapshot `-011148` (manifest sha256 `dcbd0626…`, 217 concluded matches, 9,983 player-match rows, 0 bundle build failures). The full-season bridge's 577 links were imported to DEV and a full DEV settle **dry-run** was **INCOMPLETE** (`unresolvedIdentityMatch 0`, `unresolvedIdentityPlayer 830`); it predates ISSUE-244 and is **not** current acceptance evidence. **No committed canonical `afl_api` apply, no S9 DEV smoke, no timer installation/enablement.** All committed S9 acceptance work must run on the post-ISSUE-244 lineage. **Route A (operator, 2026-09-22):** ISSUE-224 registration/linkage must let the same snapshot settle with `unresolvedIdentityPlayer = 0` and pass the F004 completeness gate; a partial-player apply by removing `--require-complete-source` is **rejected for S9 acceptance** (the flag's absence stays technically possible, just not the selected route). |
...
- **Scheduled chain may commit nothing until ISSUE-224 + S9** (ISSUE-244 F017, CLOSED / DEFERRED
  ...
  rebuilt/re-resolved on the authoritative immutable snapshot, and the re-settle must reach
  `unresolvedIdentityPlayer = 0`. **D-9b (RESOLVED 2026-09-22, option B; `issues/open/AFLDB-ISSUE-228.md`
  §20):** that snapshot is **`afl-api-2026-2026-09-21-031725`** (manifest `5018a3d6…`), **not** the
  superseded `-011148`, whose bytes are absent from every local root. The rebuild must re-resolve
  **all 669 providers as one coherent population**; the 577 links already imported to DEV are lineage,
  **not** acceptance evidence, and must not be mixed with the 92. Removing `--require-complete-source` to force a partial-player
