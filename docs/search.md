...
`nl_search_log` and `nl_search_review` together are the raw material a later tuning pass reads to adjust `NL_CONFIDENCE` and grow the vocabulary from what people actually type. No `/admin/nl-search` UI exists yet — everything above is queryable via `psql` today (the original plan and the review both list it as a distinct, later increment) — but the schema and the write path are what that page would read from.
