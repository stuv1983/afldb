...
**The plan.** `NlQueryPlan` names one of ten grains (`player_career`, `player_game`, `player_season`, `team_match`, `club_season`, `team_streak`, `achievement_summary`, `head_to_head`, `coach_record`, `after_siren`), a closed metric where the grain ranks one, an aggregation, and every named scope. Clean `A v B` wording is stored as matchup scope, not as a subject/opponent side filter. Team-result lists carry a separate grouped threshold and optional per-match margin filter; validation requires the selected compiler to consume both. Career predicates continue to reuse the Grid Solver's named builders. Unsupported combinations decline instead of dropping fields.

**The parser.** `parseNlQuestion(question, ctx)` is synchronous except for player resolution. It canonicalises text, gates genuinely unsupported data such as playing positions and averages, resolves club/venue aliases, extracts aggregation, metric, match type, round, period, checkpoint, streak, grouped threshold, per-match margin, season and negation slots, resolves any player mention, and then elects the grain.
...
**Every grain is compiled.** Career supports multi-condition trivia, allowlisted rankings, award counts and boundaries; the remaining grain-specific behaviour is described below.
...
**`after_siren`** is the one EVENT grain: it reads AFLDB's curated, cited list of kicks after the siren (`after_siren_kicks`, migration 089) and never recomputes an event from scores or `player_match_stats`. Three dimensions stay independent and are ANDed rather than merged — what the kick registered (goal / behind / nothing), what it did to the result (won / drew / nothing), and the match result from the kicker's own side — so "a goal after the siren" and "a goal after the siren to win" are provably different populations, and one measured event is a kick that scored nothing, changed nothing, and whose side won anyway. An absent outcome means every kick, misses included. A question is answered either as the events themselves or as a leaderboard of the kickers, which are different row shapes; every superlative in the family is a tie, because the measured ceiling is two. A row with no canonical match link is counted, listed, attributed and classified — `premiership_season` is never the gate — and is excluded only from the two things `matches` owns: ordering and finals scope, with the answer naming how many were left out. Finals scope reads `matches.round_type`, never the source's own round text, because one non-premiership 1980 event is recorded with the round "GF"; "after the siren in a Grand Final" is an honest empty result. Every answer carries a permanent caveat that this is a curated list of individual events, not a systematic record of every kick after every siren.
