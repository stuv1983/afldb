...
**The plan.** `NlQueryPlan` names one of seven grains (`player_career`, `player_game`, `player_season`, `team_match`, `club_season`, `team_streak`, `achievement_summary`), a closed metric where the grain ranks one, an aggregation, and every named scope. Team-result lists carry a separate grouped threshold and optional per-match margin filter; validation requires the selected compiler to consume both. Career predicates continue to reuse the Grid Solver's named builders. Unsupported combinations decline instead of dropping fields.

**The parser.** `parseNlQuestion(question, ctx)` is synchronous except for player resolution. It canonicalises text, gates genuinely unsupported data such as coaching and positions, resolves club/venue aliases, extracts aggregation, metric, match type, round, period, streak, grouped threshold, per-match margin, season and negation slots, resolves any player mention, and then elects the grain.
