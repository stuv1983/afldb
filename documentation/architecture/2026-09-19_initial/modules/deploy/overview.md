<!-- DOC | Bootstrap module stub for the deploy module of AFLDB -->
...
- **Path(s):** `deploy`
- **Apparent purpose:** systemd, Caddy, cluster supervisor, deployment (CLAUDE.md §6).
- **Tracked files:** 27 (0 examined, 27 not examined)
- **Baseline entries:** 0 (extractor: `tsc-api`)

## Extraction coverage

The API baseline covers `.ts`/`.tsx` and `.sql` only. **27 file(s) in this module
were NOT examined:** 7 `.webp`, 4 `.sh`, 3 `.service`, 2 `.timer`, 1 `.none`, 1 `.production`, 1 `.rules`, 1 `.js`, 1 `.svg`, 1 `.html`, 1 `.txt`, 1 `.xml`, 1 `.css`, 1 `.mjs`, 1 `.ps1`.
...
- Behavioural contracts (null-versus-throw, ordering, idempotency): **not captured** — these
  belong in `documentation/registry/deploy.md`, which is deliberately empty until someone
