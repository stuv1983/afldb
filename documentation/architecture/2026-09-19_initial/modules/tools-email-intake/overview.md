<!-- DOC | Bootstrap module stub for the tools-email-intake module of AFLDB -->
...
- **Path(s):** `tools/email_intake`
- **Apparent purpose:** IMAP fetch/staging (CLAUDE.md §6).
- **Tracked files:** 2 (0 examined, 2 not examined)
- **Baseline entries:** 0 (extractor: `tsc-api`)

## Extraction coverage

The API baseline covers `.ts`/`.tsx` and `.sql` only. **2 file(s) in this module
were NOT examined:** 2 `.py`.
...
- Behavioural contracts (null-versus-throw, ordering, idempotency): **not captured** — these
  belong in `documentation/registry/tools-email-intake.md`, which is deliberately empty until someone
