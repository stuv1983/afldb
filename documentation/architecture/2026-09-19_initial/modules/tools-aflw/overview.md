<!-- DOC | Bootstrap module stub for the tools-aflw module of AFLDB -->
...
- **Path(s):** `tools/aflw`
- **Apparent purpose:** AFLW parse/staging load (CLAUDE.md §6).
- **Tracked files:** 4 (0 examined, 4 not examined)
- **Baseline entries:** 0 (extractor: `tsc-api`)

## Extraction coverage

The API baseline covers `.ts`/`.tsx` and `.sql` only. **4 file(s) in this module
were NOT examined:** 3 `.py`, 1 `.md`.
...
AFLW: AFLW app/query code → tools/aflw → docs/aflw.md if modelling rules needed.
...
- Behavioural contracts (null-versus-throw, ordering, idempotency): **not captured** — these
  belong in `documentation/registry/tools-aflw.md`, which is deliberately empty until someone
