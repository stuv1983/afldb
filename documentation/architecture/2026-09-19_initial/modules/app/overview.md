<!-- DOC | Bootstrap module stub for the app module of AFLDB -->
...
- **Path(s):** `src/app`
- **Apparent purpose:** Next.js pages, layouts, server components, route handlers (CLAUDE.md §6).
- **Tracked files:** 280 (279 examined, 1 not examined)
- **Baseline entries:** 770 (extractor: `tsc-api`)

## Extraction coverage

The API baseline covers `.ts`/`.tsx` and `.sql` only. **1 file(s) in this module
were NOT examined:** 1 `.svg`.
...
- `src/app/admin/records/labels.ts` — 21 entries
- `src/app/admin/awards/actions.ts` — 15 entries
- `src/app/admin/awards/labels.ts` — 15 entries
- `src/app/admin/draft/actions.ts` — 11 entries
- `src/app/admin/brownlow/labels.ts` — 10 entries
- `src/app/admin/fixtures/actions.ts` — 10 entries
- `src/app/admin/records/actions.ts` — 10 entries
- `src/app/admin/records/validation.ts` — 10 entries
- `src/app/coaches/compare/state.ts` — 10 entries
- `src/app/admin/data-editor/actions.ts` — 9 entries

## Task routing (from `CLAUDE.md` §7)

Frontend/UI: exact route → directly imported components. Admin: admin route → imported component → helper/query.

## Existing test homes

Extend the closest existing suite; do not create a new test file by default (`CLAUDE.md` §10).

`tests/admin-*.test.ts, tests/match-sheet.test.ts, tests/admin-nav/`
...
- Behavioural contracts (null-versus-throw, ordering, idempotency): **not captured** — these
  belong in `documentation/registry/app.md`, which is deliberately empty until someone
